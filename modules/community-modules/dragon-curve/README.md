...
| --- | --- | --- | --- |
| `sparse_infill_density` | float | 20.0 | Mirrors rectilinear-infill (canonical percent; spacing = `line_width / (density/100)`). |
| `infill_direction` | float | 45.0 | Rotates the whole tiling about the region centre. |
| `sparse_infill_speed` | float | 100.0 | Mirrored; becomes the path speed factor. |
| `line_width` | float | 0.0 | Mirrored. Spacing is `line_width / (sparse_infill_density/100)`. `0` means *auto* and falls back to 0.45 mm. |
...
`sparse_infill_density`, `line_width` and `tiling_depth` are overridable per
region.
...
1. Build the Heighway dragon polyline: `2^tiling_depth` segments of length
   `line_width / (sparse_infill_density/100)`, using the standard turn rule (the
   `i`-th turn is left when `((i & -i) << 1) & i` is zero). Rotate it by
   `infill_direction`.
