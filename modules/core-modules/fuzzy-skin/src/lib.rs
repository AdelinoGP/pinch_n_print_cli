...
//! are perturbed regardless of per-vertex `fuzzy_skin` feature flags.
//!
...
use slicer_ir::{
    ConfigValue, ConfigView, ExtrusionPath3D, LoopType, Point3WithWidth, WallFeatureFlags, WallLoop,
    WidthProfile,
};
    ...
    fn run_wall_postprocess(
        ...
        for region in regions {
            for (wall_index, wall) in region.wall_loops().iter().enumerate() {
                ...
                let should_apply = self.apply_to_all
...
///
/// Walks each segment of the path, subdivides at `point_distance` intervals,
/// and displaces each subdivision point perpendicular to the segment direction
/// by a random amount scaled by `thickness`.
///
...
/// - Output must have at least 3 points
///
...
fn apply_fuzzy_skin(
    ...
    let seed = layer_index.wrapping_mul(65537).wrapping_add(wall_index.wrapping_mul(257)).wrapping_add(1);
    let mut rng = Rng::new(seed);

    let min_dist = point_distance * 0.75;
    let range = point_distance * 0.5;

    let mut out_points: Vec<Point3WithWidth> = Vec::new();
    let mut out_flags: Vec<WallFeatureFlags> = Vec::new();
    let mut out_widths: Vec<f32> = Vec::new();
    let mut dist_left_over: f32 = 0.0;

    for seg_idx in 0..points.len() - 1 {
        let p0 = &points[seg_idx];
        let p1 = &points[seg_idx + 1];
        let seg_flag = if seg_idx < feature_flags.len() {
            &feature_flags[seg_idx]
        } else {
            &feature_flags[feature_flags.len() - 1]
        };
        let seg_width = if seg_idx < width_profile.widths.len() {
            width_profile.widths[seg_idx]
        } else {
            width_profile.widths[width_profile.widths.len() - 1]
        };
        ...
        if !should_fuzz {
            // Copy the start point unchanged
            if seg_idx == 0 || out_points.is_empty() {
                out_points.push(*p0);
                out_flags.push(seg_flag.clone());
                out_widths.push(seg_width);
            }
            // Reset leftover for next segment
            dist_left_over = 0.0;
            continue;
        }
        ...
        if seg_len < 1e-8 {
            // Degenerate segment — copy start point
            if seg_idx == 0 || out_points.is_empty() {
                out_points.push(*p0);
                out_flags.push(seg_flag.clone());
                out_widths.push(seg_width);
        ...
        // Walk along segment at point_distance intervals
        let mut dist = dist_left_over;
        while dist < seg_len {
            let t = dist / seg_len;
            ...
            let r = rng.next_f32() * thickness;
            let new_x = base_x + perp_x * r;
            let new_y = base_y + perp_y * r;
        ...
        dist_left_over = dist - seg_len;
    }

    // Ensure at least 3 points (per OrcaSlicer)
    while out_points.len() < 3 && points.len() >= 2 {
        let idx = if points.len() >= 2 + out_points.len() {
            points.len() - 2 - out_points.len()
        } else {
            0
        };
        let fallback_flag = if idx < feature_flags.len() {
            &feature_flags[idx]
        } else {
            &feature_flags[0]
        };
        let fallback_width = if idx < width_profile.widths.len() {
            width_profile.widths[idx]
        } else {
            width_profile.widths[0]
        };
        out_points.push(points[idx]);
        out_flags.push(fallback_flag.clone());
        out_widths.push(fallback_width);
    }
