...
impl LayerModule for TreeSupport {
    fn on_print_start(config: &ConfigView) -> Result<Self, ModuleError> {
        let enabled = match config.fields.get("support_enabled") {
            Some(ConfigValue::Bool(b)) => *b,
            _ => false,
        };

        let density = match config.fields.get("support_density") {
            Some(ConfigValue::Float(d)) => *d as f32,
            _ => 0.2,
        };

        let base_angle = match config.fields.get("support_angle") {
            Some(ConfigValue::Float(a)) => *a as f32,
            _ => 0.0,
        };

        let support_speed = match config.fields.get("support_speed") {
