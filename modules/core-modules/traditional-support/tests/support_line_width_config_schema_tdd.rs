//! Manifest schema guard: `support_line_width` must stay declared on the
//! traditional support renderer.
//!
...
fn manifest() -> Value {
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("traditional-support.toml");
    let text = std::fs::read_to_string(&path).unwrap_or_else(|error| {
        panic!(
            "traditional-support.toml must be readable at {}: {error}",
            path.display()
        )
    });
    text.parse::<Value>()
        .expect("traditional-support.toml must parse as TOML")
}
...
fn schema_entry<'a>(manifest: &'a Value, key: &str) -> &'a Value {
    manifest
        ...
        .unwrap_or_else(|| panic!("traditional-support.toml is missing [config.schema.{key}]"))
}
