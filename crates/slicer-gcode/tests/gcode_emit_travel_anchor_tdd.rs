...
//! TDD red tests for packet `39_stable-entity-ids` â€” gcode emit travel anchor resolution.
//!
...
fn travel_emitted_at_entity_id_endpoints() {
    // Entity A: line from (0, 0) to (10, 0) at z=0.2  â€” entity_id = 1
    // Entity B: line from (20, 5) to (30, 5) at z=0.2 â€” entity_id = 2
    ...
    // The first G0 line must have X â‰ˆ 20.0 and Y â‰ˆ 5.0
    let g0 = g0_lines[0];
    ...
    assert!(
        (x - 20.0).abs() < 0.01,
        "G0 X should be â‰ˆ 20.0 (start of entity B), got {} in '{}'\nfull gcode:\n{}",
        x,
        g0,
        text
    );
    assert!(
        (y - 5.0).abs() < 0.01,
        "G0 Y should be â‰ˆ 5.0 (start of entity B), got {} in '{}'\nfull gcode:\n{}",
        y,
        g0,
        text
    );
...
fn travel_survives_entity_reorder() {
    // Entity A: (0, 0) â†’ (10, 0), entity_id = 1
    // Entity B: (20, 5) â†’ (30, 5), entity_id = 2
    // Entity C: (40, 10) â†’ (50, 10), entity_id = 3
    //
    ...
    // The G0 must have X â‰ˆ 0.0 and Y â‰ˆ 0.0 (the TravelMove destination)
    let g0 = g0_lines[0];
    let x = parse_gcode_param(g0, 'X')
        .unwrap_or_else(|| panic!("G0 line has no X param: '{}'\nfull gcode:\n{}", g0, text));
    let y = parse_gcode_param(g0, 'Y')
    ...
    assert!(
        (x - 0.0).abs() < 0.01,
        "G0 X should be â‰ˆ 0.0 (TravelMove destination), got {} in '{}'\nfull gcode:\n{}",
        x,
        g0,
        text
    );
    assert!(
        (y - 0.0).abs() < 0.01,
        "G0 Y should be â‰ˆ 0.0 (TravelMove destination), got {} in '{}'\nfull gcode:\n{}",
        y,
        g0,
        text
    );
