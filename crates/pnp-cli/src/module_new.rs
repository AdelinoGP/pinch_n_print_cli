//! Implementation of the `pnp_cli module new` subcommand.
//!
//! Scaffolds a new module project directory with the correct structure,
//! Cargo.toml, manifest template, and a passing test suite.
//! Extends the scaffold with `.cargo/config.toml` (WASM alias) and `README.md`.

use std::fmt;
use std::fs;
use std::path::Path;

use slicer_schema::{stage_by_id, STAGES};
...
/// Execute the `pnp_cli module new` command, creating the module under `dir/<name>`.
///
/// This is the core implementation used by both the CLI entry point and tests.
pub fn execute_in(dir: &Path, name: &str, stage: &str) -> Result<(), NewError> {
    ...
    // Write .cargo/config.toml with the wasm build alias.
    let cargo_dir = base.join(".cargo");
    fs::create_dir_all(&cargo_dir).map_err(NewError::Io)?;
    fs::write(
    ...
    // Write README.md with a short module-author intro.
    ...
    let readme_text = format!(
        "# {}\n\n\
         ...
         ```sh\n\
         cargo build-wasm\n\
         wasm-tools component new target/wasm32-unknown-unknown/release/{}.wasm -o target/slicer/{}.wasm\n\
         ```\n",
        name, name_underscore, name,
    );
    fs::write(base.join("README.md"), readme_text).map_err(NewError::Io)?;
    ...
    println!("  {name}/.cargo/config.toml");
...
fn generate_manifest(name: &str, stage: &str) -> String {
    // Precondition, kept for its panic: the stage must be in the canonical
    // table. Its columns are no longer read here — the qualified export below
    // is the only schema-derived value the manifest advertises.
    stage_by_id(stage).expect("is_valid_stage() already checked");
    let parallel_safe = stage != "PostPass::LayerFinalization";

    // Packet 164: every WASM stage owns a versioned per-stage package, and the
    // func is `run` for all of them — so the bare export name no longer
    // identifies a contract. Advertise the qualified spelling
    // (`slicer:<pkg>/<iface>@<ver>#run`), which does.
    let expected_export =
        slicer_schema::qualified_export_for_stage_id(stage).unwrap_or_else(|| {
            // Host-built-in stages (PrePass::PaintSegmentation, packet 97) have no
            // WIT contract; `module new` rejects them upstream, but keep the
            // scaffold honest rather than printing a misleading export.
            "(none — this stage is host-built-in and has no WIT export)".to_string()
        });

    format!(
        r#"# Generated by `pnp_cli module new`; metadata derived from slicer-schema.
# Expected WIT export for stage `{stage}`:
#   {expected_export}
# (the stage-specific export emitted by #[slicer_module])
[module]
id           = "com.example.{name}"
version      = "0.1.0"
...
/// The full method signature, keyed by stage id.
...
mod tests {
    ...
    fn manifest_postpass_stage() {
        ...
        assert!(manifest.contains(r#"id = "PostPass::GCodePostProcess""#));
    }
    ...
    #[test]
    fn lib_rs_has_from_config_constructor() {
        let lib = generate_lib_rs("my-infill", "Layer::Infill");
        assert!(
            lib.contains("fn from_config"),
            "generated lib.rs must include from_config constructor"
        );
    }
    ...
    #[test]
    fn every_generated_manifest_omits_the_legacy_world_key() {
        let legacy_key = format!("{}-{}", "wit", "world");
        for spec in STAGES {
            ...
            assert!(
                parsed
                    .get("module")
                    .and_then(|m| m.get(legacy_key.as_str()))
                    .is_none(),
                "generated manifest for stage {} must omit the legacy key",
                spec.stage_id
            );
        }
    }
    ...
    fn generated_manifest_comments_advertise_schema_derived_exports() {
        // Asserting `contains(spec.wit_export)` was near-vacuous after packet
        // 164: `wit_export` is the bare `"run"` for every migrated stage (and
        // `""` for the host-built-in one), so it matched almost any text. Pin
        // the qualified spelling, which is the actual contract.
        let mut checked = 0;
        for spec in STAGES {
            let Some(qualified) = slicer_schema::qualified_export_for_stage_id(spec.stage_id)
            else {
                // PrePass::PaintSegmentation is host-built-in (packet 97).
                continue;
            };
            let manifest = generate_manifest("test-mod", spec.stage_id);
            assert!(
                manifest.contains(&qualified),
                "generated manifest for {} must advertise its qualified WIT \
                 export `{qualified}`, got:\n{manifest}",
                spec.stage_id,
            );
            checked += 1;
        }
