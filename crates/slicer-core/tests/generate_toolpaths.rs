...
//! The cases below use source polygons and assert toolpath topology and
//! spacing-domain invariants rather than serialized output from one run.
//!
...
fn simple_square_fixture() -> ExPolygon {
    expoly(vec![p(0.0, 0.0), p(10.0, 0.0), p(10.0, 10.0), p(0.0, 10.0)])
}
