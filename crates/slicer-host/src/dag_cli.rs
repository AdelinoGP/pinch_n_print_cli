//! Implementation of the `slicer-host dag <subcommand>` JSON output.
//!
//! All entry points here operate on `&[LoadedModule]` produced by
//! `load_modules_from_roots` — manifest TOML parsing only, no WASM
//! compilation. Designed for agent tooling that polls module wiring at sub-
//! 100 ms latency regardless of module count.
//!
...
use crate::dag::{build_global_dag, GlobalEdge};
use crate::instrumentation::{compute_serial_edges_for_stage, EdgeReason, SerialEdge};
...
pub fn run_dag_stages(modules: &[LoadedModule]) -> StagesOut {
    // BTreeMap gives deterministic stage ordering by canonical id.
    let mut by_stage: BTreeMap<&str, Vec<&LoadedModule>> = BTreeMap::new();
    for m in modules {
...
/// `dag stage <id>` — full detail for one stage, or `None` if no module is
/// in that stage.
pub fn run_dag_stage(modules: &[LoadedModule], id: &StageId) -> Option<StageOut> {
    let stage_modules: Vec<&LoadedModule> = modules
        .iter()
        .filter(|m| m.stage() == id.as_str())
        .collect();
    if stage_modules.is_empty() {
        return None;
    }

    let owned_modules: Vec<LoadedModule> = stage_modules.iter().map(|m| (*m).clone()).collect();
    let edges = compute_serial_edges_for_stage(&owned_modules);

    let module_views = stage_modules
        .iter()
        .map(|m| ModuleOut {
            id: m.id().to_string(),
            claims: m.claims().to_vec(),
            ir_reads: m.ir_reads().to_vec(),
            ir_writes: m.ir_writes().to_vec(),
            requires_modules: m.requires_modules().to_vec(),
    ...
    Some(StageOut {
        ...
        serial_edges: edges.iter().map(edge_to_stage_edge_out).collect(),
    })
}
...
pub fn run_dag_depends(
    ...
    if !modules.iter().any(|m| m.id() == target.as_str()) {
        return None;
    }
    let global = build_global_dag(modules);
...
pub fn run_dag_claims(modules: &[LoadedModule]) -> ClaimsOut {
...
mod tests {
    ...
    fn stages_groups_modules_and_counts_claims() {
        ...
        let out = run_dag_stages(&modules);
    ...
    fn stage_returns_serial_edges_with_flat_reasons() {
        ...
        let out = run_dag_stage(&modules, &"Layer::Infill".to_string()).unwrap();
    ...
    fn stage_returns_none_for_unknown_stage() {
        ...
        assert!(run_dag_stage(&modules, &"Layer::Nope".to_string()).is_none());
    }
    ...
    fn depends_splits_upstream_and_downstream() {
        ...
        let out = run_dag_depends(&modules, &target, None).unwrap();
    ...
    fn depends_returns_none_for_unknown_module() {
        ...
        assert!(run_dag_depends(&modules, &missing, None).is_none());
    }
    ...
    fn depends_includes_object_ids_when_supplied() {
        ...
        let out = run_dag_depends(&modules, &target, Some(&ids)).unwrap();
    ...
    fn claims_flags_interchangeable_holders() {
        ...
        let out = run_dag_claims(&modules);
