...
use slicer_host::model_loader::{load_model, ModelLoadError};
...
<model unit="millimeter" xmlns="http://schemas.microsoft.com/3dmanufacturing/core/2015/02">
  ...
  <build>
    ...
    //   (0, 10, 0) → (10,   0, 0)
    //   (0,  0, 0) → ( 0,   0, 0)
    // This is a Z-rotation by -90° (clockwise in XY viewed from +Z).
    ...
    assert!(
        ...
        "Z-rotation by -90° must send (20,0,0)→(0,-20,0) and (0,10,0)→(10,0,0); got {:?}",
        verts
    );
}
...
fn y_rotation_45deg_orients_correctly() {
    // benchy_4color.3mf's component-1 transform is a Y-rotation by -45°
    // (3MF row-vector: `0.7071 0 0.7071 0 1 0 -0.7071 0 0.7071 0 0 0`).
    // Test with a Y-rotation by +45° applied at <build><item> so the
    ...
    // sends (1, 0, 0) → (0.7071, 0, -0.7071) and (0, 0, 1) → (0.7071, 0, 0.7071).
    let xml = r#"<?xml version="1.0" encoding="UTF-8"?>
<model unit="millimeter" xmlns="http://schemas.microsoft.com/3dmanufacturing/core/2015/02">
  <resources>
    <object id="1" type="model">
  ...
  <build>
    ...
    assert!(
        have_x_image,
        "Y+45° must send (1,0,0) → (1/√2, 0, -1/√2); got {:?}",
        verts
    );
    assert!(
        have_z_image,
        "Y+45° must send (0,0,1) → (1/√2, 0, 1/√2); got {:?}",
        verts
    );
}
