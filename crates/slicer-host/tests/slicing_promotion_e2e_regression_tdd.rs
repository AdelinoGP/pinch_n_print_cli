...
//!   3. The `TopSurfaceIroning` `LayerModule` — emits ironing strokes
...
use slicer_host::{commit_shell_classification_builtin, commit_slice_builtin, Blackboard};
use slicer_ir::{
...
///
/// - Step A: z in [0, 0.4], 20×20 mm footprint (centered at origin)
/// - Step B: z in [0.4, 0.6], 12×12 mm footprint (centered)
/// - Step C: z in [0.6, 0.8], 6×6 mm footprint (centered)
///
...
fn layer_plan() -> LayerPlanIR {
    let layers: Vec<GlobalLayer> = (0..4)
        .map(|i| {
            ...
            //   z=0.4 (i=1): step-a (its slice at z=0.4 is empty → exposed top)
            //   z=0.6 (i=2): step-b only (its slice at z=0.6 is empty → exposed top)
            //   z=0.8 (i=3): step-c only (its slice at z=0.8 is empty → exposed top)
...
fn staircase_prepass_classifies_each_step_top_as_exposed() {
    ...
    // Layer 0 (z=0.2): step-a interior — top is layer 1 (z=0.4 where slice is empty)
    ...
    // layer 1's upper neighbor is layer 2 (step-b only) → step-a has no upper layer
    // in its timeline → layer 1 is step-a's exposed top.
...
fn staircase_topsurface_ironing_emits_for_exposed_layers() {
    ...
    // a meaningful point count (≥ 30 — staircase is small but each exposed
    // top should produce a snake at 0.1mm spacing).
    assert!(
        total_paths >= 1,
        "expected ≥ 1 ironing path across staircase; got {total_paths}"
    );
    assert!(
        total_points >= 30,
        "expected ≥ 30 ironing points across staircase; got {total_points}"
    );
}
