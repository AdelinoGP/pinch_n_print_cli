...
// ─── AC-1: cantilever_emits_overhang_speed ────────────────────────────────

/// Two-layer cantilever scene: layer 0 is a 20×20mm square (no overhang),
...
/// must emit `F600` (10 mm/s × 60 = 600 mm/min).
#[test]
fn cantilever_emits_overhang_speed() {
    ...
    // Layer 0: 20×20mm square outer wall, no overhang.
    ...
    // Layer 1: 20×30mm rectangle; the +y extension (y > 20.0) is cantilever
    // territory and its points are classified as overhang quartile 1.
    let mut layer1 = make_layer(1, 0.4);
    let pts1 = vec![
        // Supported region (y <= 20) — no quartile
        Point3WithWidth {
            x: 0.0,
            y: 0.0,
            z: 0.4,
        ...
        // Cantilever extension (y > 20) — quartile 1
        Point3WithWidth {
            x: 20.0,
            y: 30.0,
            z: 0.4,
    ...
    // After implementation, that move must use F600 (10 mm/s × 60).
...
// ─── AC-2: zero_config_byte_identical_baseline ────────────────────────────
...
fn zero_config_byte_identical_baseline() {
    use slicer_host::overhang_classifier::classify_layers;

    let zero_config = slicer_host::gcode_emit::FeedrateConfig {
        outer_wall_speed: 60.0,
    ...
    // — every byte of the emitter output is now pinned to this snapshot for
    ...
    // ── Half 1 of AC-2: byte-identical G-code under zero config ─────────────
    //
    ...
    // ⇒ emitter output must be byte-identical to the all-None baseline.
    ...
    // equal the outer-wall base speed (60 mm/s × 60 = 3600 mm/min). Any
    ...
    let active_config = slicer_host::gcode_emit::FeedrateConfig {
        outer_wall_speed: 60.0,
        overhang_1_4_speed: 10.0,
        ..Default::default()
    };
    ...
    // ── Half 2 of AC-2: classifier writes no `Some(_)` under zero config ───
...
// ─── AC-3: non_wall_roles_ignore_overhang_quartile ────────────────────────
...
// ─── AC-4: first_layer_quartile_is_none ───────────────────────────────────
...
fn first_layer_quartile_is_none() {
    use slicer_host::gcode_emit::FeedrateConfig;
    ...
    slicer_host::overhang_classifier::classify_layers(&mut layers, &config);
...
fn quartile_to_key_mapping() {
    ...
    // Expected: quartile 1→F600, 2→F1200, 3→F1800, 4→F2400
...
// ─── AC-N1: quartile_zero_is_reserved ─────────────────────────────────────
...
fn quartile_zero_is_reserved() {
    ...
    // 1000-point random sweep: scatter points across [-15, 25] × [-15, 25],
    ...
    // of Some(1)..=Some(4). Any other value (Some(0), Some(5), …) is a
    // bucketization bug.
