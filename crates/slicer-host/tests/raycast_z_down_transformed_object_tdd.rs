...
fn raycast_transformed_object_returns_world_space_z_across_all_worlds() {
    ...
    let layer_hit = lhs::Host::raycast_z_down(&mut layer_ctx, "translated-plate".to_string(), 10.0, 12.0, 25.0)
        .expect("layer transformed raycast should not error")
        .expect("layer transformed raycast should hit");
    let prepass_hit = phs::Host::raycast_z_down(&mut prepass_ctx, "translated-plate".to_string(), 10.0, 12.0, 25.0)
        .expect("prepass transformed raycast should not error")
        .expect("prepass transformed raycast should hit");
    let finalization_hit = fhs::Host::raycast_z_down(&mut finalization_ctx, "translated-plate".to_string(), 10.0, 12.0, 25.0)
        .expect("finalization transformed raycast should not error")
        .expect("finalization transformed raycast should hit");
    let postpass_hit = pphs::Host::raycast_z_down(&mut postpass_ctx, "translated-plate".to_string(), 10.0, 12.0, 25.0)
