...
use slicer_host::{execute_prepass_slice_single_layer, Blackboard, ExecutionPlan};
...
/// for all layers in the plan — matching the invariant assumed by
/// `execute_single_layer` (`slice_vec.get(layer.index as usize)`).
///
/// Panics on failure — callers are unit tests, so a panic is the right signal.
