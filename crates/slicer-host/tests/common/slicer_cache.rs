//! Process-local cache for `slicer-host` binary invocations.
//!
...
//! call [`run_slicer_host_uncached`] instead.
...
/// Captured outcome of a slicer-host invocation. `success == false` is a
...
/// driver, not a slicer-host failure). Regular non-zero exits are
...
/// the determinism tests (which call [`run_slicer_host_uncached`]
...
fn execute_slicer(model: &Path, module_dir: &ModuleDirKind, config: Option<&Path>) -> RunOutcome {
    ...
    let proc_out = run_slicer_host_uncached(model, &modules_path, &out_path, config);
...
pub fn run_slicer_host_uncached(
    model: &Path,
    module_dir: &Path,
    output: &Path,
    config: Option<&Path>,
