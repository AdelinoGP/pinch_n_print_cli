...
//!
//!   1. Identity transform   → world Z == vertex Z (no accidental offset).
//!   2. Z translation        → world Z = vertex Z + translate_z (transform is applied).
//!   3. Non-trivial mesh     → world Z covers the full vertex span after transform.
//!
...
//!
//! Acceptance criterion: `cargo test -p slicer-host --test world_z_canonical_surface_tdd`

#![allow(missing_docs)]

use std::collections::HashMap;

use slicer_host::model_loader::object_world_z_extent;
...
fn translation_matrix(tx: f64, ty: f64, tz: f64) -> [f64; 16] {
    // Column-major 4×4: translation in column 3.
...
// ---------------------------------------------------------------------------
// AC-1: identity transform — world Z equals vertex Z
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// AC-2: Z translation — world Z includes the translation offset
// ---------------------------------------------------------------------------
...
fn z_translation_shifts_world_z_extent() {
    ...
    assert!(
        (z_min - 15.0).abs() < 1e-5,
        "translated: world z_min should be 15.0, not local 0.0 — got {z_min}"
    );
    assert!(
        (z_max - 20.0).abs() < 1e-5,
        "translated: world z_max should be 20.0, not local 5.0 — got {z_max}"
    );
}
