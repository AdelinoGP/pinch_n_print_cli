...
//! This is the intended red state — tests graduate to green as production code lands.
//!
...
/// (and any other modules present — that's fine, only the postpass stage
...
fn slice_with_raw(raw: HashMap<ConfigKey, ConfigValue>) -> String {
    ...
    // Seed machine-gcode-emit schema defaults — separated by type:
    //   - string defaults  → binding_source only (real value for module)
    //                        pipeline_source gets empty-string sentinel (safe for CONFIG_BLOCK)
    //   - int/float/bool   → both sources (numeric values are safe for CONFIG_BLOCK)
    let machine_schema = machine_binding.module.config_schema().entries.clone();
    for (key, entry) in &machine_schema {
        if binding_source.contains_key(key) {
            // User explicitly set this key — let it flow through both sources unchanged.
            continue;
        }
    ...
    //    pipeline_source has int/float defaults and empty-string sentinels — no multiline
    //    template values — so the CONFIG_BLOCK won't contain M190 or PRINT_END.
    ...
    let config = PipelineConfig {
        ...
        runners: PipelineStageRunners {
            ...
            emitter: Box::new(
                DefaultGCodeEmitter::new("slicer-host-test 0.1.0".into())
...
fn start_block_position_before_extrusion_mode_and_first_g1() {
    ...
    let header_end_offset = gcode
        ...
        .expect("HEADER_BLOCK_END must be present — not yet emitted (red)");

    let m190_offset = gcode
        .find("M190 S60")
        .expect("M190 S60 must be present in gcode — not yet emitted (red)");
    ...
    let extrusion_mode_offset =
        extrusion_mode_offset.expect("M82 or M83 must be present — not yet emitted (red)");
    ...
    let first_g1_e_offset = gcode
        ...
        .map(|(offset, _)| offset)
        .expect("at least one G1 with non-zero E must exist — not yet emitted (red)");
...
fn end_block_position_after_last_g1_before_config_block() {
    ...
    let print_end_offset = gcode
        .find("PRINT_END")
        .expect("PRINT_END must be present — not yet emitted (red)");

    let config_start_offset = gcode
        .find("; CONFIG_BLOCK_START")
        .expect("CONFIG_BLOCK_START must be present — not yet emitted (red)");
    ...
    let last_g1_offset = gcode
        ...
        .into_iter()
        .last()
        .expect("at least one G1 must exist — not yet emitted (red)");
...
/// AC-9: fallback — each of the four config keys appears as a comment line
...
fn module_manifest_registers_four_keys_with_expected_types_and_defaults() {
    ...
    let config_end = gcode
        ...
        .expect("CONFIG_BLOCK_END must be present — not yet emitted (red)");
...
fn unclosed_bracket_treated_as_literal() {
    ...
    // Must appear as a standalone gcode line (no leading "; ") — fails in red state
...
fn start_block_not_inside_other_blocks() {
    ...
    // If M190 doesn't exist at all, the feature is missing — assert it's present.
    assert!(
        gcode.contains("\nM190"),
        "M190 must appear somewhere in gcode (start block) — not yet emitted (red)"
    );
}
