...
//!
//! Verifies the live-path ConfigView contract (docs/02 §Pre-filtered config;
...
fn arc_wrapped_view_cannot_be_mutated_by_consumers() {
    ...
    // inner view — this is the documented read-only guarantee on the live
    ...
    // but by this point no consumer thread is still reading — this is the
...
// ── Live plan-build wiring + guardrail ─────────────────────────────────
...
fn main_production_entry_path_routes_through_build_live_execution_plan() {
    // Guards the live plan/build wiring: the slicer-host binary must
    // construct its ExecutionPlan via `build_live_execution_plan` (the
    ...
    assert!(
        ...
        "main.rs must parse --config through parse_cli_config_source so the live-path source is real, not a placeholder"
    );
}

// ── build_live_execution_plan wiring ──────────────────────────────────

use slicer_host::{build_live_execution_plan, LiveModuleBinding};
...
use slicer_host::{parse_cli_config_source, ConfigSourceParseError};
...
fn parse_cli_config_source_output_feeds_bind_module_config_view_cleanly() {
    // End-to-end: parsed JSON → bound ConfigView → only declared keys visible.
