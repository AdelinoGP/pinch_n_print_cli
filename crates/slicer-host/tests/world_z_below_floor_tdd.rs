...
//!
//! Acceptance criteria:
//!   AC-1: object at Z=0 or above           → Ok(())
//!   AC-2: object translated -5mm in Z      → Err(WorldZBelowFloor { z_min: -5.0 })
//!   AC-3: object translated +10mm in Z     → Ok(())
//!   AC-4: object straddles Z=0 (min < 0)   → Err(WorldZBelowFloor)
//!   AC-5: empty mesh                        → Ok(()) (no geometry to check)
//!   AC-6: error message contains "WORLD_Z_BELOW_FLOOR"
//!   AC-7: identity object at Z ≥ 0         → Ok(())
//!   AC-8: z_min field in error is accurate  → z_min matches expected value
//!
...
use slicer_host::model_loader::{validate_world_z_floor, ModelLoadError};
...
// ---------------------------------------------------------------------------
// AC-1: object sitting on the floor (Z ≥ 0) → Ok(())
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// AC-2: object translated -5mm in Z → Err(WorldZBelowFloor)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// AC-4: object straddles Z=0 (local min below 0) → Err(WorldZBelowFloor)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// AC-5: empty mesh → Ok(()) (no geometry to violate the floor)
// ---------------------------------------------------------------------------

/// A mesh with no vertices cannot have a world-space Z below the floor.
/// The validator treats this as Ok — later stages catch empty meshes separately.
#[test]
...
// ---------------------------------------------------------------------------
// Extra: large negative Z translation → Err(WorldZBelowFloor)
// ---------------------------------------------------------------------------
