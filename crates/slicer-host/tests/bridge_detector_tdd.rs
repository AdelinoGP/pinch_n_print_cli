...
#[derive(Clone, Copy)]
enum WallPlacement {
    /// Walls on the two long sides (C0–C3 and C1–C2).
    ...
    LongSides,
    /// Walls on the left long side and bottom short side (C0–C3 and C0–C1),
    ...
    /// projection-span anchor width equals (width × length) /
    /// √(width² + length²).
    LeftAndBottom,
}

/// Build a `MeshIR` with a `width_mm × length_mm` cluster of down-facing
...
/// triangles elsewhere — they must NOT be picked up as bridge candidates.
...
fn make_rotated_bridge_mesh_walls(
    ...
    // slope = z_tilt / width_mm; for width >= 2 mm, z_tilt=2 gives nz ≈ -0.7 to -0.93.
    let z_tilt = 2.0_f32;
    ...
    // Down-facing bridge tris (CW → normal_z < 0, Overhang class).
    ...
    match walls {
        WallPlacement::LongSides => {
            // Left wall (C0–C3): bridge Tri1's edge 0→3 gets a wall neighbor.
            indices.extend_from_slice(&[0, 3, 7]);
            indices.extend_from_slice(&[0, 7, 4]);
            // Right wall (C1–C2): bridge Tri0's edge 2→1 gets a wall neighbor.
        ...
        WallPlacement::LeftAndBottom => {
            ...
            indices.extend_from_slice(&[0, 3, 7]);
            indices.extend_from_slice(&[0, 7, 4]);
            // Bottom wall (C0–C1): bridge Tri0's edge 1→0 gets a wall neighbor.
...
fn make_topfacing_only_mesh() -> MeshIR {
    // Flat roof: a 10×10 mm square lying in the XY plane at z=1.0, CCW winding.
    let pt3 = |x: f32, y: f32, z: f32| Point3 { x, y, z };
    let vertices = vec![
        pt3(0.0, 0.0, 1.0),
        pt3(10.0, 0.0, 1.0),
    ...
    // CCW from +Z → normal = +Z (TopSurface).
...
fn bridge_cluster_seeded_from_downfacing_facets_only() {
    ...
    assert!(
        ...
        "must produce at least one bridge region for 5×20mm down-facing cluster"
    );

    // Every facet in every bridge region must have normal_z ≤ 0.
    ...
    for br in &obj_data.bridge_regions {
        for &fi in &br.facet_indices {
            ...
            assert!(
                ...
                "bridge region facet {} has class {:?} (TopSurface) — must not appear in bridge regions",
                fi,
                class
            );
        }
...
fn valid_bridge_passes_min_length_filter() {
    ...
    assert!(
        ...
        "must produce at least one valid bridge region for 5×20mm panel"
    );
    ...
    // the run vertices onto that axis gives a span of (width × length) /
    // √(width² + length²) = 100 / √425 ≈ 4.8507 mm.
    let width = 5.0_f32;
    let length = 20.0_f32;
...
fn anchor_width_from_anchor_edge_run_not_bbox() {
    ...
    // Must NOT be the rotated AABB short side ≈ 14.33 mm (bbox regression).
    let bbox_short = 5.0_f32 * 30.0_f32.to_radians().cos() + 20.0_f32 * 30.0_f32.to_radians().sin();
    assert!(
        (br.anchor_width_mm - bbox_short).abs() > 0.5,
        "anchor_width_mm ({}) must NOT match the rotated bbox short side ({}) — bbox regression",
        br.anchor_width_mm,
        bbox_short
    );
...
fn xy_footprint_is_facet_projection_not_aabb() {
    ...
    // True facet-projection area = 5 * 20 = 100 mm².
    assert!(
        (area - 100.0).abs() / 100.0 <= 0.05,
        "xy_footprint area ({:.2} mm²) must be within 5% of 100 mm²",
        area
    );

    // Rotated AABB area ≈ (5*cos30+20*sin30) * (20*cos30+5*sin30) ≈ 14.33 * 19.82 ≈ 284 mm².
    // Definitely not that.
    assert!(
        (area - 240.0).abs() / 240.0 > 0.05,
        "xy_footprint area ({:.2} mm²) must NOT match rotated AABB area (~240 mm²)",
        area
    );
}
...
/// orientation. For rotation_deg=30° the long sides run at 30°+90°=120°,
/// so bridge_direction_deg ≈ 120°.
/// The critical negative assertion: must NOT be 0° (bbox aspect-ratio default).
#[test]
fn bridge_direction_follows_anchor_edge_orientation() {
    ...
    // Must NOT be 0° (bbox default) — the direction must track the actual
    ...
    // Must be within ±2° of 30.0 (anchor edge orientation for 30°-rotated bridge).
    assert!(
        (d - 30.0).abs() <= 2.0,
        "bridge_direction_deg ({}) must be within ±2° of 30.0",
        d
    );
}

/// AC-5 (rotated min-length filter).
/// Long-side anchor walls give bridge_direction = long-axis direction (120° for
/// 30°-rotated bridge), so bridge_length = projection along 120° = 20mm (correct).
#[test]
fn rotated_short_bridge_fails_min_length_filter() {
    ...
    // not the rotated AABB diagonal (≈14.33 mm from a 0° bbox shortcut).
    assert!(
...
fn expansion_margin_grows_polygon_observably() {
    // footprint: [0,0]–[20,5] mm
    let footprint = rect_expoly_mm(0.0, 0.0, 20.0, 5.0);
    // infill: [-3,-3]–[23,8] mm (3 mm border on every side)
    ...
    for ba in &sliced_region.bridge_areas {
        ...
        assert!(
            ...
            "bridge_area (area {:.4} mm²) must be contained in infill_areas (intersection area {:.4} mm²)",
            output_area,
            intersected_area
        );
    }
...
fn invalid_bridge_excluded_from_slice_areas() {
    ...
    assert_eq!(
        valid_count, 0,
        "5×20mm bridge must be invalid under min_bridge_length_mm=25.0"
    );
