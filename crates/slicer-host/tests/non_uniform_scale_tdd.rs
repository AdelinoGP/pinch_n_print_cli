...
//! along one or two axes without compensating — so non-uniform scale is a fatal
//! load-time error.
//!
//! Acceptance criteria:
//!   AC-1: uniform scale       → Ok(())
//!   AC-2: non-uniform X≠Y    → Err(NonUniformScaleUnsupported)
//!   AC-3: non-uniform Y≠Z    → Err(NonUniformScaleUnsupported)
//!   AC-4: all axes different  → Err(NonUniformScaleUnsupported)
//!   AC-5: identity transform  → Ok(())
//!   AC-6: zero matrix         → Ok(()) (treated as identity)
...
use slicer_host::model_loader::{validate_non_uniform_scale, ModelLoadError};
...
/// Scale 2× uniformly on all three axes is valid.
#[test]
fn uniform_scale_2x_is_valid() {
    ...
    assert!(
        result.is_ok(),
        "uniform scale 2× should be Ok, got: {result:?}"
    );
}

/// Scale 0.5× uniformly on all three axes is valid.
#[test]
fn uniform_scale_half_is_valid() {
    ...
    assert!(
        result.is_ok(),
        "uniform scale 0.5× should be Ok, got: {result:?}"
    );
}
...
// ---------------------------------------------------------------------------
// AC-4: all three axes different → Err(NonUniformScaleUnsupported)
// ---------------------------------------------------------------------------

/// A transform that stretches X by 1.5×, Y by 2.0×, Z by 3.0× is rejected.
#[test]
...
// ---------------------------------------------------------------------------
// AC-6: zero matrix treated as identity → Ok(())
// ---------------------------------------------------------------------------
...
/// A combined scale+translate transform (uniform scale 2×, translate 5mm Z)
...
fn uniform_scale_with_z_translation_is_valid() {
    ...
    assert!(
        result.is_ok(),
        "uniform scale 2× with Z translation should be Ok, got: {result:?}"
    );
}
