...
fn parses_benchy_4color_sidecar() {
    use slicer_host::model_loader_sidecar::{parse_3mf_sidecar, PartSubtype};
    let path =
...
fn malformed_sidecar_falls_back_to_normal_part() {
    use slicer_host::model_loader_sidecar::parse_3mf_sidecar;
    // Invalid XML — mismatched closing tag causes a parse error in quick_xml.
...
fn load_3mf_invokes_sidecar_parser_before_archive_drop() {
    // Two-part assertion:
    // 1. parse_3mf_sidecar returns non-empty data for benchy_4color.3mf — proves the
