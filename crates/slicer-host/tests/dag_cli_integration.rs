//! Integration tests for `slicer-host dag` and `slicer-host diagnose`.
//!
//! Drives the actual compiled `slicer-host` binary via
...
fn bin() -> PathBuf {
    PathBuf::from(env!("CARGO_BIN_EXE_slicer-host"))
}

fn workspace_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("crates/slicer-host has a parent")
        .parent()
...
fn run_dag(args: &[&str]) -> (Value, i32) {
    ...
    let output = cmd.output().expect("spawn slicer-host");
...
#[test]
fn dag_stages_with_empty_module_dir_returns_empty_list() {
