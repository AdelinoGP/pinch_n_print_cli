...
//! `slicer-host` binary, produces GCode that observably differs from
...
//!   - `crates/slicer-host/src/model_loader.rs:150` unconditionally
...
//!   - The CLI surface on `slicer-host` (and on the dev `slicer-cli`)
...
fn painted_benchy_3mf_reaches_paint_segmentation() {
    ...
    assert_ne!(
        ...
        "DEV-044 RED: painted-Benchy GCode is byte-identical to \
         ...
         crates/slicer-host/src/model_loader.rs. Closure: Packet 50 \
