...
//! intended red state — tests graduate to green as Steps 2–5 implement the
...
use slicer_host::{
    Blackboard, DefaultGCodeEmitter, DefaultGCodeSerializer, FinalizationError, FinalizationOutput,
    FinalizationStageRunner, LayerArena, LayerStageError, LayerStageOutput, LayerStageRunner,
    NoopLayerProgressSink, PostpassError, PostpassOutput, PostpassStageRunner,
    PrepassExecutionError, PrepassStageOutput, PrepassStageRunner,
};
use slicer_host::{CompiledModule, ExecutionPlan};
...
fn default_runners() -> PipelineStageRunners {
    PipelineStageRunners {
        ...
        emitter: Box::new(DefaultGCodeEmitter::new("slicer-host-test 0.1.0".into())),
...
fn sentinels_present_no_thumbnail() {
    ...
    assert_eq!(
        ...
        1,
        "HEADER_BLOCK_START must appear exactly once — got:\n{}",
...
/// AC-11: Every line in THUMBNAIL_BLOCK starts with "; " and base64 portion ≤ 76 chars.
#[test]
fn thumbnail_base64_chunking_orca_parity() {
    ...
    for line in thumb_region.lines().filter(|l| !l.trim().is_empty()) {
        ...
        assert!(
            b64_part.len() <= 76,
            "base64 chunk must be ≤ 76 chars, got {} chars: {b64_part:?}",
            b64_part.len()
        );
    }
}

/// AC-12: Block ordering — HEADER before first ;TYPE:, CONFIG after last ;TYPE:.
#[test]
...
/// Negative-1: Verifies our sentinel assertions are meaningful — a gcode string
...
/// real pipeline does not emit the sentinel — so the assertion in
...
fn rejects_missing_sentinel_block() {
    ...
    // The real pipeline output must contain the sentinel — this fails now (red state)
    let real_gcode = slice_no_thumb();
    assert!(
        real_gcode.contains("; HEADER_BLOCK_START"),
        "HEADER_BLOCK_START sentinel is absent from real pipeline output — not yet implemented"
    );
}
...
/// (Same assertion as AC-1 for the config sentinels — fails now since
/// CONFIG_BLOCK_START is not yet emitted.)
#[test]
fn empty_config_view_still_emits_sentinels() {
    // Run with zero extra config flags — defaults only
    let mesh_ir = Arc::new(load_model(&stl_fixture_path()).expect("fixture load"));
