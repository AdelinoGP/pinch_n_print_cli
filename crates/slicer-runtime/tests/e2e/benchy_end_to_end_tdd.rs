...
//! `resources/benchy.stl` (a binary STL â€” the only form the
...
fn benchy_mvp_produces_full_height_layer_progression() {
    ...
    assert!(
        outcome.success,
        "pnp_cli must succeed on full-height Benchy run. Stderr:\n{stderr}"
    );
    ...
    assert!(
        max_z >= 40.0,
        "Benchy max emitted Z = {max_z}; expected ~47â€“48 for a full-height \
...
/// AC-6: benchy_with_support_enabled â€” runs Benchy with tree-support
...
/// AC-8: benchy_support_marker_present â€” asserts .gcode contains
...
/// AC-8: benchy_support_deterministic â€” runs identical command twice,
