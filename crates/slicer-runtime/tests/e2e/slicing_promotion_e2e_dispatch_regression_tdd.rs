...
fn staircase_gcode_contains_ironing_block() {
    ...
    assert!(
        outcome.success,
        "pnp_cli must succeed against the staircase fixture. Stderr:\n{}",
        outcome.stderr
    );
...
fn staircase_ironing_g1_lines_within_top_fill_extents() {
    ...
    let printable_area = slicer_ir::ResolvedConfig::default().printable_area;
    let (centre_x, centre_y) = slicer_model_io::bed_center_mm(&printable_area)
        .expect("the default printable_area must describe a plate");
    ...
    for (line_no, line) in gcode.lines().enumerate() {
        ...
        for (x, y) in parse_xy_g1_lines(line) {
            checked += 1;
            if (x - centre_x).abs() > max_extent + tolerance
                || (y - centre_y).abs() > max_extent + tolerance
            {
