...
fn make_gcode_ir_with_mode(e_values: &[f32], absolute_e_mode: bool) -> GCodeIR {
    ...
    // command list â€” see `DefaultGCodeEmitter::emit` â€” so the serializer
...
fn rejects_xyzf_drift_across_modes() {
    // Same as xyzf_unchanged_across_modes â€” the negative framing verifies
    ...
    for (rel_line, abs_line) in rel_lines.iter().zip(abs_lines.iter()) {
        ...
        assert_eq!(
            rel_xyzf, abs_xyzf,
            "X/Y/Z/F MUST be identical â€” drift detected.\n  relative: {rel_line}\n  absolute: {abs_line}"
        );
    }
}
