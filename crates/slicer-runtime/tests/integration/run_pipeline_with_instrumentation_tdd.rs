...
use slicer_runtime::{
    ...
    EdgeReason, ExecutionPlan, FinalizationError, FinalizationOutput, FinalizationStageRunner,
    GCodeEmitter, GCodeSerializer, IrAccessMask, LayerArena, LayerStageError, LayerStageOutput,
    LayerStageRunner, LoadedModuleBuilder, NoopInstrumentation, NoopLayerProgressSink,
    PipelineInstrumentation, PostpassError, PostpassStageRunner, PrepassExecutionError,
...
//    test files is the codebase norm) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
impl PostpassStageRunner for NoopPostpassRunner {
    ...
    ) -> Result<slicer_runtime::PostpassOutput, PostpassError> {
        Ok(slicer_runtime::PostpassOutput::GCodeSuccess)
    }
    ...
    fn run_text_postprocess(
        ...
        Ok(slicer_runtime::PostpassOutput::TextSuccess { text })
    }
}
...
/// Assertions:
/// - `report.layers.len() == 2`
/// - `report.slice_meta.module_count >= 2` (one per-layer module Ã— 2 layers)
...
fn run_with_collector_records_phase_and_layer_brackets() {
    ...
    // Two global layers â†’ two layer records.
    assert_eq!(
        report.layers.len(),
        2,
    ...
    // One per-layer module Ã— 2 layers = 2 module calls (at minimum).
    assert!(
        report.slice_meta.module_count >= 2,
        "expected at least 2 module calls (1 module Ã— 2 layers), got {}",
