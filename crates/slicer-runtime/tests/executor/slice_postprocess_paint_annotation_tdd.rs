...
// â”€â”€ Step 15: degraded-warning determinism + propagation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// docs/02 Â§Paint Region Resolution Contract; docs/04 Â§770-775 (degraded
// success); docs/10 Â§Degraded success row; docs/11 Â§73-75.

#[test]
fn paint_annotation_repeated_runs_emit_byte_identical_warnings() {
    use slicer_runtime::execute_slice_postprocess_paint_annotation;
...
fn paint_annotation_warnings_ordered_region_then_semantic_then_polygon_then_point() {
    ...
    // Same (region, polygon, contour_point) â€” warnings must follow the input
...
fn paint_annotation_warnings_propagate_to_slice_event_collector_as_degraded() {
    use slicer_runtime::progress_events::SliceEventCollector;
    use slicer_runtime::{
...
fn paint_annotation_fallback_value_is_deterministic_for_each_semantic() {
    use slicer_runtime::execute_slice_postprocess_paint_annotation;
    use slicer_runtime::SlicePostProcessPaintAnnotationWarning as W;
    use slicer_runtime::SlicePostProcessPaintAnnotationWarningReason as R;
    ...
    // default (Materialâ†’ToolIndex(0); flag-typed semanticsâ†’Flag(false)).
    let result =
        execute_slice_postprocess_paint_annotation(SlicePostProcessPaintAnnotationRequest {
            slice_ir: slice_fixture(
                7,
...
// Regression: prior to the EPSILON_UNITS tightening (10 â†’ 1), a contour point
// that sat several integer units (sub-Âµm but well above grid quantization) from
...
// fire â€” points further away are silently assigned a region-level default.
#[test]
fn paint_annotation_does_not_warn_for_points_outside_grid_quantization_window() {
    ...
    // Contour point at (10.0, 0.0005) is 5 integer units (= 0.5 Âµm) above the
    ...
    assert!(
        !result.degraded,
        "5-unit (0.5 Âµm) offset is above grid-quantization noise and must not degrade"
    );
    assert!(
        result.warnings.is_empty(),
        "5-unit (0.5 Âµm) offset must not fire a code-504 warning; got {} warnings: {:?}",
        result.warnings.len(),
        result.warnings,
    );
...
fn paint_annotation_warnings_aggregate_into_one_event_per_polygon_semantic_group() {
    use slicer_runtime::paint_annotation_warnings_to_progress_events;
    ...
    // 4 warnings on the same (layer/obj/region/semantic/polygon) group â†’ 1 event.
    // 2 warnings on a different semantic â†’ 1 event.
    // 1 warning on a different polygon â†’ 1 event.
    ...
    // Third group: FuzzySkin/polygon=3, 1 point â€” singleton uses the original
