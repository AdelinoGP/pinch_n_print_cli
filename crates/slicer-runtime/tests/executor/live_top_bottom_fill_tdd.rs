...
//!
//! Verifies that the live host path (dispatch â†’ commit â†’ assembly) preserves
...
//!   `commit_layer_outputs` â†’ `convert_infill_output` â†’ `InfillIR` â†’
//!   `arena.set_infill` â†’ `assemble_ordered_entities` â†’ `ordered_entities`
//!
...
//!
//! We verify this by checking `arena.infill()` after commit â€” confirming
//! the role survived the WIT boundary â†’ `InfillIR` â†’ arena commit path.
...
fn commit_layer_outputs_preserves_top_solid_infill_role() {
    ...
    // No origin tags â€” untagged path goes to the one synthetic region.
    ...
    assert!(
        !solid.is_empty(),
        "solid_infill must not be empty after TopSolidInfill commit"
    );

    assert_eq!(
        solid[0].role,
        IrExtrusionRole::TopSolidInfill,
        "convert_infill_output must preserve TopSolidInfill role through the WIT boundary"
    );
    assert_eq!(
        solid[0].points.len(),
        2,
        "path point count must be preserved"
    );
    assert_eq!(
        solid[0].points[0].width, 0.4,
        "path width must be preserved"
    );
}
...
fn commit_layer_outputs_preserves_bottom_solid_infill_role() {
    ...
    assert!(
        !solid.is_empty(),
        "solid_infill must not be empty after BottomSolidInfill commit"
    );

    assert_eq!(
        solid[0].role,
        IrExtrusionRole::BottomSolidInfill,
...
fn commit_layer_outputs_preserves_mixed_infill_roles() {
    ...
    let has_bottom = region
        .solid_infill
        .iter()
        .any(|p| p.role == IrExtrusionRole::BottomSolidInfill);
    assert!(has_top, "ordered_entities must contain TopSolidInfill");
    assert!(
        has_bottom,
        "ordered_entities must contain BottomSolidInfill"
    );
}
...
fn commit_layer_outputs_infill_postprocess_replaces_correctly() {
    ...
    let after = arena
        .infill()
        .expect("must have infill after postprocess commit");
    let solid = &after.regions[0].solid_infill;
    assert_eq!(
        solid.len(),
        1,
        "InfillPostProcess replaces rather than appends"
    );
    assert_eq!(
        solid[0].role,
        IrExtrusionRole::TopSolidInfill,
