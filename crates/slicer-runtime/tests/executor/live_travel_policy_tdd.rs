...
///   Retract { length: 0.8 } â†’ ZHop { after: 0, height: 0.2 } â†’ Move(travel) â†’ Unretract { length: 0.8 }
...
fn retracting_travel_populates_matching_z_hop_and_retract_pair() {
    ...
    .expect(
        "commit must succeed â€” Retract/ZHop/Unretract from PathOptimization must not be rejected",
    );
...
/// Retractâ†’ZHopâ†’Travelâ†’Unretract sequence as a coherent block after the last entity.
#[test]
fn z_hop_anchor_aligns_with_retract_anchor_when_entities_present() {
    ...
    assert_eq!(
        ...
        "TravelMove entity_id must be the entity_id of the last entity (index 2 â†’ id 3), got {}",
