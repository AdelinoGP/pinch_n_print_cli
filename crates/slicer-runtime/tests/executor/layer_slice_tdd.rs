...
fn per_layer_executor_stages_host_built_in_slice_on_real_path() {
    ...
    // No per-layer stages are scheduled, so the runner isn't invoked â€” but
...
/// every low-Z layer â€” adjacent triangles sharing an edge emitted lines
/// in opposite orientation, so the old directed `a â†’ b` walker
...
fn per_layer_executor_surfaces_layer_slice_failure_structured() {
    ...
    // for surfacing per-layer slice errors as structured diagnostics â€” when
