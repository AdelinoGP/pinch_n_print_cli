...
//! world-space Z extent (z_max == z_min â†’ None).
//!
//! When a vertical rod (oriented along +Z in local space) is rotated 90Â°
...
//!
//! Verification: `cargo test -p slicer-runtime --test rotated_object_world_extent_tdd -- --nocapture`
...
/// giving a truly degenerate world Z extent after rotate_x(90Â°).
/// After rotation: (0,0,0)â†’(0,0,0) and (0,0,10)â†’(0,-10,0).
/// Both have world Z = 0 â†’ degenerate (z_max == z_min).
...
fn rotated_object_world_extent_is_degenerate() {
    // Mesh: vertical rod rotated 90Â° about X axis.
...
///
/// Rotation formula: (x, y, z) â†’ (x, -z, y)
/// The new Y = -old Z and new Z = old Y.
/// Column-major storage: matrix[col * 4 + row]
///   col 0: (1, 0, 0, 0) â†’ [0]=1,  [4]=0,  [8]=0,  [12]=0
///   col 1: (0, 0, 1, 0) â†’ [1]=0,  [5]=0,  [9]=1,  [13]=0   (+Zâ†’Y)
///   col 2: (0,-1, 0, 0) â†’ [2]=0,  [6]=-1, [10]=0, [14]=0   (-Zâ†’Y? no!)
///   col 3: (0, 0, 0, 1) â†’ [3]=0,  [7]=0,  [11]=0, [15]=1
/// Correction: new_z = old_y, so col 1 row 2 = +1 (not col 2).
/// col 1 = (0, 0, 1, 0) â†’ [1]=0, [5]=0, [9]=1, [13]=0   â†’ new_y = +z
/// Wait: (x,y,z)â†’(x,-z,y): new_y = -z = -1*z + 0*y â†’ col1[1]=0, col2[1]=-1
/// new_z = y = 0*x + 1*y + 0*z â†’ col0[2]=0, col1[2]=1, col2[2]=0
/// So: m[9]=+1 (col1,row2=new_y=+z? no that's wrong)
/// Let me redo: output.y = m[1]*x + m[5]*y + m[9]*z + m[13]*1
/// For new_y = -z: m[9] = -1, m[5] = 0
/// For new_z = y: m[2]*x + m[6]*y + m[10]*z + m[14] = y â†’ m[6]=1
fn rotate_x_90() -> [f64; 16] {
    let mut m = [0.0f64; 16];
    m[0] = 1.0; // col 0 row 0: X â†’ X
