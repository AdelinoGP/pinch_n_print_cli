...
//!
//! Specifically:
//! - Logging is forwarded to the host's WIT `log` import on `wasm32`
//!   (the guest target). On native targets it is routed through an
//!   installable thread-local sink that defaults to writing to `stderr`;
//!   tests install a capture sink to assert log behavior.
...
///
/// On native targets: if a capture sink is installed via
/// [`test_support::install_log_capture`], the message is appended there;
/// otherwise the message is written to `stderr`. Module unit tests run on this
/// path and need no runtime.
///
/// On wasm32 the call is forwarded to the host through the
/// `slicer:common/host-services#log` WIT import — mirroring [`medial_axis`]'s
/// own native-vs-wasm32 split. Guests build for `wasm32-unknown-unknown` (no
/// WASI), so the native `eprintln!`/thread-local path is a total no-op inside a
/// guest; without this arm the WIT import was never called and every module log
/// was silently discarded before reaching `hs::Host::log` on the host side.
pub fn log(level: LogLevel, message: &str) {
    ...
    #[cfg(target_arch = "wasm32")]
    {
        // Self-contained mini WIT world scoped to just this one function, as
        // with `medial_axis`'s equivalent inline binding: it does not depend on
        // the full world generated by the `#[slicer_module]` macro (private
        // inner module, not accessible from this crate). Component imports
        // resolve structurally against the real host runtime's
        // `slicer:common/host-services` interface, so only the wire shape has
        // to match the canonical `log-level` enum and `log` signature in
        // `crates/slicer-schema/wit/deps/common.wit`.
        #[allow(dead_code)]
        mod __sdk_host_log_import {
            ::wit_bindgen::generate!({
                inline: r#"
package slicer:sdk-log-helper;
...
"#,
        ...
        let wit_level = match level {
            LogLevel::Trace => {
                __sdk_host_log_import::slicer::common::host_services::LogLevel::Trace
            }
            LogLevel::Debug => {
                __sdk_host_log_import::slicer::common::host_services::LogLevel::Debug
            }
            LogLevel::Info => __sdk_host_log_import::slicer::common::host_services::LogLevel::Info,
            LogLevel::Warn => __sdk_host_log_import::slicer::common::host_services::LogLevel::Warn,
            LogLevel::Error => {
                __sdk_host_log_import::slicer::common::host_services::LogLevel::Error
            }
        };
        __sdk_host_log_import::slicer::common::host_services::log(wit_level, message);
    }
}
...
package slicer:types {
    interface geometry {
        record point2 { x: s64, y: s64 }
        record point3 { x: f32, y: f32, z: f32 }
        record point3-with-width { x: f32, y: f32, z: f32, width: f32, flow-factor: f32, overhang-quartile: option<u8>, dist-to-top-mm: f32, overhang-distance-mm: option<f32> }
