...
fn echo_command(command: &GcodeCommand, output: &GcodeOutputBuilder) -> Result<(), ModuleError> {
    match command {
        ...
        GcodeCommand::ToolChange(cmd) => {
            output.push_tool_change(cmd.after_entity_index, cmd.from_tool, cmd.to_tool)
        }
    ...
    fn run_gcode_postprocess(
        ...
        match config.get_string("postpass_mode") {
            ...
            Some(mode) if mode == "emit-sample" => {
                ...
                output
                    .push_retract(0.8, 35.0, RetractMode::Gcode)
                    .map_err(|message| ModuleError {
                        code: 3,
                        message,
                        fatal: true,
                ...
                output
                    .push_unretract(0.8, 35.0, RetractMode::Gcode)
                    .map_err(|message| ModuleError {
                        code: 4,
                        message,
                        fatal: true,
                    })?;
                output.push_fan_speed(200).map_err(|message| ModuleError {
                    code: 5,
                    message,
                    fatal: true,
                })?;
                output
                    .push_temperature(1, 215.0, false)
                    .map_err(|message| ModuleError {
                        code: 6,
                        message,
                        fatal: true,
            ...
            Some(mode) if mode == "emit-z-hop" => {
                output.push_z_hop(0, 0.4).map_err(|message| ModuleError {
                    code: 10,
                    message,
                    fatal: true,
                })?;
