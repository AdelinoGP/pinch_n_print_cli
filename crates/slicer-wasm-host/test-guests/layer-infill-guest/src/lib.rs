...
impl Guest for Component {
    ...
    fn run_slice_postprocess(
        ...
        for r in &regions {
            ...
            output
                .set_polygons(&key, &[poly])
                .expect("set_polygons failed");
        }
        Ok(())
    }
    fn run_perimeters(
    ...
    fn run_support(
        ...
        output
            ...
            .expect("push support path failed");
        Ok(())
    }
    ...
    fn run_path_optimization(
        ...
        let comment = format!(
            ...
            region_count, total_walls, total_infill
        );
        ...
        for i in 0..region_count as u32 {
            output
                .push_tool_change(i, i, i + 1)
                .expect("push tool_change failed");
        }
