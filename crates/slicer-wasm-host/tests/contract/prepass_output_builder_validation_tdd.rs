...
// ─── HostMeshSegmentationOutput ────────────────────────────────────────────

#[test]
fn mesh_segmentation_mark_triangle_paint_positive_control() {
    let mut ctx = HostExecutionContextBuilder::new("test.prepass", 0.2, 0.2).build();
    let handle = ctx.push_mesh_segmentation_output().unwrap();
    let result = prepass::HostMeshSegmentationOutput::mark_triangle_paint(
        &mut ctx,
        handle,
        "obj-a".to_string(),
        0,
    ...
    let inner = result.unwrap();
    assert_eq!(inner, Ok(()), "valid mark should be accepted");
    assert_eq!(
        ctx.mesh_segmentation_marks().len(),
        1,
...
#[test]
fn mesh_segmentation_mark_triangle_paint_empty_obj() {
    let mut ctx = HostExecutionContextBuilder::new("test.prepass", 0.2, 0.2).build();
    let handle = ctx.push_mesh_segmentation_output().unwrap();
    let result = prepass::HostMeshSegmentationOutput::mark_triangle_paint(
        &mut ctx,
    ...
    assert_eq!(
        inner,
        Err("mesh-segmentation-output: obj must be a non-empty object id".to_string()),
    );
}

#[test]
fn mesh_segmentation_mark_triangle_paint_empty_semantic() {
    let mut ctx = HostExecutionContextBuilder::new("test.prepass", 0.2, 0.2).build();
    let handle = ctx.push_mesh_segmentation_output().unwrap();
    let result = prepass::HostMeshSegmentationOutput::mark_triangle_paint(
        &mut ctx,
    ...
    assert_eq!(
        inner,
        Err("mesh-segmentation-output: semantic must be a non-empty string".to_string()),
    );
}
