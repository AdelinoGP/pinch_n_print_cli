...
- **AC verification strings must accept name-resolution-equivalent forms of every symbol they reference.** A literal-grep verification like `rg -q 'slicer_sdk::host::offset_polygons'` is brittle: a Rust call site that resolves the same symbol via the `slicer_sdk::prelude` re-export (`host::offset_polygons`) is semantically equivalent but fails the literal grep. The canonical pattern for any AC verification that names a symbol path is: `rg -q '<fully-qualified-path>' <file> || rg -q '<prelude-resolved-prefix>::<symbol>' <file> || rg -q '<in-scope-short-form>' <file>`. If the packet's AC string is too literal to accept all name-resolution-equivalent forms, treat it as a preflight defect and fix the AC string in `refine-draft` before activation. Do not silently rewrite the call site to satisfy a brittle grep — that corrupts the implementation to fit a broken test.
- **Ledger facts in packet docs are mutable shared state — re-derive them at preflight time.** Whenever a packet's `design.md`, `requirements.md`, `implementation-plan.md`, `task-map.md`, or `packet.spec.md` quotes a specific `TASK-###` ID, a `docs/07` row count, a "next free" ID, a "highest used ID", an "N open deviations" count, or any other ledger fact, the planner MUST re-derive that fact at the moment of preflight via a direct `rg` / `wc` / `git ls-files` against `docs/07_implementation_status.md` (or whichever ledger is authoritative). A `task_ids: []` in `packet.spec.md` is acceptable; a quoted ledger value the planner cannot reproduce from disk at preflight time is a `preflight FAIL`. This rule exists because ledger facts change while you work (parallel agents, rebases, fresh commits) and a quoted value is *true when written* but *false when used* — exactly the failure a careful reader cannot catch.
