...
3. Apply matching modifiers in priority-ascending order, last writer wins
   (`stamp_modifier_sub_region_configs` in
...
merging: global config → object config → modifier config. The merge is
ordered and deterministic, last writer wins per key, and a modifier delta only
affects the keys it explicitly provides; see § "Modifier Resolution Contract"
for how modifiers resolve. `Option<T>` fields are contributed by optional
modules and are `None` when the module is disabled.
...
- `tool_config:<tool_index>:<key>` — per-tool/extruder override keyed by the integer `tool_index`, resolved by `resolve_per_tool_configs`. A clean additive axis enabled by the region_id↔tool split (`PrintEntity.tool_index` is now a first-class selector). Consumed in **two** places, because a tool can be known at two different points:
  ...
  2. **Every tool — at G-code emit** (`crates/slicer-gcode/src/emit.rs`, `retract_length_for_tool`): emit-time settings (e.g. the wipe-tower-owned `retract_length`) are overlaid by the entity's resolved `tool_index`, the one place *every* entity's tool is known.
...
on `CURRENT_SLICE_IR_SCHEMA_VERSION` carries the full running history.
...
record, defined alongside `SliceIR` in `crates/slicer-ir/src/slice_ir.rs`.

- Raft layers occupy the contiguous global index prefix `0..N-1`, where
  `N = support_raft_layers`; model layers shift up to `N..`.
- The first printed **model** layer is therefore global index
  `support_raft_layers`. The first **physical** layer is always global index
  `0` — under this band the raft *is* the physical first layer.
- Layer indices remain `u32`. No field changed type, and raft-ness must never
...
in `f64` from `support_raft_layers`, `initial_layer_print_height`, and `layer_height`,
...
Mixed-family attribution: each demand is tracked by a `support_demand_id`,
and family attribution is decided at the host support-plan merge point rather
than by geometry. An entry is retained only when
`SupportAnalysisIR::family_assignments[(object_id, region_id)]` equals the
entry's `family_id` and the producing module's manifest claims include the
matching support-family claim. The check is per entry and default-deny: a
trespassing entry is dropped with a diagnostic instead of being arbitrated
...
once per `(global_layer_index, object_id, region_id)` triple.
An entry's owning family is `SupportAnalysisIR::family_assignments[(object_id, region_id)]`, and the producing module must hold the `support-family:<family_id>` claim; otherwise the host drops the entry at the support-plan merge point.
Each entry carries:
...
Entry layer indices remain `i32` for historical reasons. The positive raft band
uses non-negative indices `0..support_raft_layers-1`, followed by model layers.
Negative values are reserved for deterministic off-grid support rows and are
not a raft marker.
...
<!-- VERIFY: LayerCollectionIR.global_layer_index and the positive raft band
     are both `u32`-indexed in crates/slicer-ir/src/slice_ir.rs. SupportPlanEntry
     retains `i32` for historical reasons because deterministic negative
     indices can identify off-grid support rows; those negatives are not raft
     layers and are not representable in LayerCollectionIR. -->
