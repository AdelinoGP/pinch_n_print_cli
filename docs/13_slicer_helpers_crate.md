...
>   implemented in `crates/slicer-model-io/src/writer.rs` (Packet 71,
...
> `cargo doc -p slicer-helpers --no-deps` (and `-p slicer-model-io` for the mesh
> writers/loaders) for exact function and type signatures. This document owns what code
...
| OBJ / 3MF output   | (writer API)             | `write_obj` / `write_3mf` serializers in `crates/slicer-model-io/src/writer.rs` |
...
| `truck-meshalgo` | Yes     | BRep triangulation via `robust_triangulation` (see §STEP Import)   |
...
## Fixture Feature Inventory Helper

<!-- VERIFY: this section previously documented a `measure_stl_feature_inventory`
     function and a `FeatureInventory` struct in this crate, plus a
     `crates/slicer-helpers/tests/feature_inventory_tdd.rs` test. None of those
     symbols or that test file exist anywhere in the tree (grep returns zero
     hits). They appear to have been planned but never implemented. The fixture
     feature-inventory gate itself is real and lives in
     `12_architecture_gate_metrics.md` § "Fixture Feature Inventory
     Verification", which describes the inventory as regenerated by a dispatched
     binary-STL parser without naming a `slicer-helpers` entry point. Restore a
     concrete API description here only once such a helper actually exists. -->

The fixture feature-inventory gate is defined in
`12_architecture_gate_metrics.md` § "Fixture Feature Inventory Verification".
No `slicer-helpers` public function currently implements it under the names this
section once claimed — see the note above.
...
Reference: `08_coordinate_system.md` — integer-coord unit definitions
...
Options:
  ...
  --format    Output format (stl | obj | 3mf). Defaults to inferring from the
              output extension, then from the input extension. All three
              formats are wired through `write_mesh` in
              crates/pnp-cli/src/helpers_cmd.rs.
...
- `truck-meshalgo`: triangulates B-Rep shells into indexed triangle meshes via `robust_triangulation`
...
repair`, `mesh decimate`, and `mesh import` only.)
...
loaders (`crates/slicer-model-io/src/loader.rs`) and the JSON-Lines
...
`crates/pnp-cli/src/helpers_cmd.rs`. They do not initialise the WASM
...
`09_progress_events.md`, which carries `slice_id`, `phase`, and
...
These tasks extend the Phase B sequence in `07_implementation_status.md`.
...
| TASK-059 | Wire `pnp_cli mesh repair`, `pnp_cli mesh decimate`, `pnp_cli mesh import` subcommands (`crates/pnp-cli/src/helpers_cmd.rs`); STL writer; JSONL `--stats` events; integration tests in `crates/pnp-cli/tests/helpers_cli.rs`.                           | D     | done   |
