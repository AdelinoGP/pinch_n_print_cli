...
|---|-------------|---------------------|----------|------------|--------|------------|
| 1 | `239a-anchored-host-seams` | Give the host an anchored-entity input seam, switch all three non-anchored `execute_per_layer*` call sites to the committed anchored variant, and lower `CommittedLayerEvent::Anchored` into off-grid print rows using the canonical `\|dz\| <= EPSILON` merge rule. | TASK-399..TASK-408 | - | generated | - |
