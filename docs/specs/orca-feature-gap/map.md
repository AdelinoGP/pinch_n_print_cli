...
- [113 — Add range validation to `FeedrateConfig`](issues/113-feedrate-config-range-validation.md) — **implemented directly; bounds now enforced for all 26 host speed keys.** The premise needed correcting first: canonical *declares* these bounds but never enforces them (see the GUI-hint Note above), so this is a deliberate divergence, ruled by the user as reject-the-slice. It was warranted — `read_speed` rejects nothing and `resolve_feedrate` applies no floor, so a configured `0` emitted `F0` and a negative emitted a negative feedrate. **14 of the 26 keys were already checked by accident** (a module manifest happened to declare a twin) and the other 12 by nothing at all; `ConfigBoundsIndex::from_modules` now seeds `slicer_ir::feedrate::speed_bounds()` so all 26 go through one seam with one error. Canonical's inclusive mins (`1`, `0` for the sentinel keys, `10` for `wipe_tower_max_purge_speed` — the bound ticket 108 called inexpressible) fit `NumericBounds` as-is, so the "`> 0` is inexpressible" problem dissolved. **Retired the invented `max = 300.0`** from 12 manifest rows. Three keys have no canonical counterpart (`thin_wall_speed`, `bottom_surface_speed`, `prime_tower_speed`). Six percent-form `ratio_over` keys are silently ignored by `read_speed` — handed to ticket 128, not fixed blind. Also found: `docs/15_config_keys_reference.md` was **already stale on HEAD** (`bridge_density` 120 vs 125), absorbed by this ticket's regen.
  ...
  - [115 — Retire `support_sharp_tails`: remove the field, hardcode `true`](issues/115-retire-support-sharp-tails.md) — **the key is retired and the behaviour frozen ON; out of scope as a queue key (dead in canonical, rule 3), queue count unchanged.** Q13's ruling executed with its own verification: canonical's frozen state confirmed against the designated oracle (`g_config_support_sharp_tails = true` in `libslic3r.h`; `PrintConfigDef::handle_legacy`'s obsolete-key `ignore` set lists `support_sharp_tails`; consumers in `SupportMaterial.cpp` / `TreeSupport.cpp`), the `ResolvedConfig` field deleted (declaration home only — it was in **no** module manifest), the doc row + doc-lock arm dropped (59 → 58 host keys), and `detect_support_contacts` gates its object-bottom exception on the new `SHARP_TAIL_OBJECT_BOTTOM_EXCEPTION: bool = true` constant (`crates/slicer-core/src/algos/overhang_annotation.rs`). The geometry change is verified by re-baselined tests in `support_overhang_detection_tdd.rs` (feature-gated `host-algos`): the pointed object-bottom profile now **emits** contacts with wholly default params, and the tiny-spot fixture moved off the object bottom with a new bracket asserting the bottom layer is exempt from the `-0.1 × fw` filter. **Scope correction recorded in the ticket:** the retired key gated the port's object-bottom branch only — canonical's ordinary-layer sharp-tail append (the `g_config_support_sharp_tails` block in `detect_overhangs`' `else if (auto_normal_support)` branch, walking `raw_slices` with the `area_thresh_well_supported` test and feeding `sharp_tails_height`) is **not modelled** here and freezing the constant does not model it. Production reach at defaults is confined to raft-bottomed objects (the committed stream skips global layer 0, so the gate's layer never enters detection without a raft). Audit survivor resolved: `bridge_no_support` is wired (P30, `e7404ff1`), `enforce_support_layers` stays owned by draft packet 265.
  - [116 — Document the `_mm` suffix as the PnP-provenance key marker](issues/116-document-the-mm-suffix-key-convention.md) — **documented, no code or count change.** `CLAUDE.md` § Config Key Naming Convention (mirrored in `AGENTS.md`) now rules `_mm` a PnP-provenance marker for invented dimensional keys with no canonical counterpart (four members, membership-by-rule) and `mm3_per_mm` a unit spelling, not the marker; `docs/03_wit_and_manifest.md` cross-refs it. Future sweeps must verify a canonical counterpart before stripping (104/106 positive precedent, these four negative).
  - [117 — `silent_mode`: needs a per-variant machine-limit model](issues/117-silent-mode-per-variant-machine-limit-model.md) — **closed by direct implementation, no packet** (grilled Q1–Q4, all recommended). The ten `machine_max_*` fields are now `Option<MachineLimitPair>` keeping both stride-2 `(normal, stealth)` entries (scalar/1-list duplicates, first pair wins, hard errors preserved); `silent_mode` (canonical default `false`, host-only, never in the CONFIG_BLOCK) selects the stealth entry in `EstimatorLimits::from_config`. Q1 corrected the ticket: the envelope keeps the normal entry (canonical `print_machine_envelope` reads `values[extruder * stride]` — draft packet 267 already specifies this), only the estimator switches. Flavor-agnostic selection is **DEV-200** (canonical gates on Marlin flavors; this seam carries no flavor). Equal-variant pairs keep the bare-value CONFIG_BLOCK spelling (byte-stable); distinct pairs emit the canonical comma pair. 04/05 rows graduate `silent_mode` to live (Tier B); packet 281's eight scalar fields untouched (DEV-173 stands — extend the pair pattern at activation).
- [122 — Author packet — prime tower body parity](issues/122-author-packet-prime-tower-body-parity.md) — **authored as packet 307** (`docs/spec_packets/307-prime-tower-body-parity/`, `draft`), preflight **PASS**. The body (finish_layer assembly, top-down depth planning, idle layers, tool selection, smooth mode + its suppression clause) lands in the existing `wipe-tower` module behind `PostPass::LayerFinalization`; **five keys in, none re-declared** — the three draft producers are consumed, not folded (254a depth/pitch/brim/framework, 254b interface, 255 wall primitives), landing order 254a → 254b → 255 → 307. No new module, no IR/WIT field, no ADR, no claim. **P22 and P24 are both carried by 307.** Two authoring findings: `timelapse_type`'s canonical spelling is `"0"`/`"1"` (`s_keys_map_TimelapseType`, *"using 0,1 to compatible with old files"*), so the packet carries an explicit ingest adapter (AC-14) rather than a silent Traditional fallback; and `farthest_point_timelapse` is a **second** live canonical key missing from the gap source, recorded in the fog for [123](issues/123-audit-gap-source-key-set-completeness.md). Queue target unchanged (all five keys were already in the 407).


## Not yet specified

- **Canonical's ordinary-layer sharp-tail append is not modelled, and ticket 115's freeze
  does not model it.** Surfaced by ticket 115's scope correction. The retired
  `support_sharp_tails` key gated the port's *object-bottom* exception only.
  Canonical's `g_config_support_sharp_tails` test proper sits in
  `detect_overhangs`' (`SupportMaterial.cpp`) `else if (auto_normal_support)`
  branch: it walks `layerm->raw_slices`, classifies a slice as a sharp tail when
  it does not `overlaps` the eroded lower layer and its area is below
  `area_thresh_well_supported`, appends the result to `diff_polygons`, and
  accumulates `sharp_tails_height` for the extend-higher pass
  (`SupportMaterial.cpp` / `TreeSupport.cpp` — tree support reads
  `config_detect_sharp_tails` and the `is_auto && g_config_support_sharp_tails`
  entry). This port's `detect_support_contacts`
  (`crates/slicer-core/src/algos/overhang_annotation.rs`) has no `raw_slices`
  equivalent and no `sharp_tails_height` accumulator, and the tree-support
  planner passes `is_sharp_tail: false` unconditionally
  (`modules/core-modules/tree-support-planner/src/lib.rs`, `OverhangContext`).
  The fog is whether any FFF queue feature needs it: it changes where ordinary
  thin-tip contacts appear on non-bottom layers and how far tree support
  sharp-tail roofs extend upward. Ticketable when a packet touches
  ordinary-layer contact geometry or tree-support roof placement; not before,
  since no queue key reads it and freezing the constant ON is canonical's own
  state.
...
- **The gap source is missing at least one live canonical key.** Surfaced by ticket
  ...
  legitimately scope it. **Second instance, surfaced by ticket 122's authoring
  (2026-09-11):** `farthest_point_timelapse` (coBool, default false) — declared
  beside `timelapse_type` in `PrintConfigDef::init_fff_params` and read in
  `GCode::process_layer`'s traditional-snapshot arm, inert except on H2C/H2D
  profiles — is likewise absent from the snapshot. This is a *completeness* defect
  in the gap source, distinct from the known *accuracy* defect in its ✅/❌ column
  (ticket 01 measured that one). Belongs to
  [ticket 123](issues/123-audit-gap-source-key-set-completeness.md), which is on
