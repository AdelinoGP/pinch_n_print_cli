...
- **Elephant-foot and polyhole cannot both sit where their packets put them, and canonical's
  order is the one being lost.** Surfaced by ticket 94. Canonical's `PrintObject::slice()` runs
  `slice_volumes()` — XY compensation, then elephant-foot, then `apply_conical_overhang()` —
  then `fix_slicing_errors`, then `_transform_hole_to_polyholes()`. **Polyhole is last.** Packet
  303 (ticket 93) places elephant-foot on `Layer::SlicePostProcess`, which runs after *every*
  `PrePass::*` stage; packet 304 (ticket 94) must place polyhole in the prepass, because a
  cross-layer pass has nowhere else to go in a layer-major executor. The result inverts canonical.
  It is **not** cosmetic: canonical EFC iterates `idx_contour <= simplified.holes.size()`, so it
  resamples and offsets **holes**, and canonical's own grouping carries a lone-first-layer rescue
  whose stated reason is "cause of first layer compensation" — the rescue exists *because* EFC
  ran first. Observable only when both features are enabled (both default off) and only on the
  first `elefant_foot_compensation_layers` layers. Packet 304 records it as a deviation clause and
  neither packet amends the other. **Note this is a second symptom of the same root question**:
  303's own deviation (a) already records that its seam is later than canonical's, "so any future
  consumer of pre-compensation footprints must be authored against this seam" — polyhole is the
  first such consumer, and it arrived one ticket later. The question to settle is whether
  `Layer::SlicePostProcess` is the right home for elephant-foot at all, given canonical also has
  shell classification and support analysis see *compensated* slices, which that seam cannot
  deliver either. Cheap to be wrong in one direction only: if the answer is "move EFC to a prepass
  built-in", packet 304 changes the position of one `run_builtin_stage` call and 303 changes its
  seam; if the answer is "accept the inversion", nothing changes. Both packets are `draft` and
  unimplemented, so this is settleable now at near-zero cost — and it gets more expensive the
  moment either one merges.
