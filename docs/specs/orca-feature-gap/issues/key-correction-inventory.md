...
| `sparse_fill_holder` and siblings | Q3(b) | unmatched holder must fail validation | implemented | `validate_startup_dag_with_configured_holders` checks each configured holder against loaded manifests using `module_id_matches_holder`; it emits `UnmatchedClaimHolder` when no module matches and `ClaimHolderDoesNotDeclareClaim` when a matching module lacks the claim. | no — ticket 110 resolved |
...
| `support_ironing_speed` (support module) | Q11(a) | rename from `ironing_speed`, keep 30.0 | implemented | Module-local normalization input; canonical has one global `ironing_speed`, so this PnP-specific key stays outside `SPEED_KEYS` rather than creating a second host feedrate mechanism. | no — ticket 109 resolved |
...
|---|---|---|
| `precise_outer_wall` | DEFAULT-MISMATCH | No question covered it. Ticket 100 deliberately declined to flip it (the port's precise-outer-wall path changes wall ordering); recorded as DEV-166 (filed as DEV-158, renumbered in the 2026-09-03 merge) and still open. |
