...
| 7 | organic-tree-engine | Port the organic tree engine (canonical `TreeSupport3D.cpp`, ~4200 lines, plus its `TreeModelVolumes` avoidance model) so `support_style=default/organic` on tree types can run the canonical engine and retire the DEV-150 Strong alias. | TASK-441 | #4 | queued | none |
