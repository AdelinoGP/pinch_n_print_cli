...
| `cube_4color_ironing_per_painted_top_color` | `crates/slicer-runtime/tests/executor/cube_4color_ironing_per_painted_top_color_tdd.rs:170` | ~~FIX (paint/region-order churn)~~ **RESOLVED 2026-07-17 — test-parser defect, not a pipeline bug.** Both colors ARE ironed at the top layer: T3's ironing block runs as the layer's first (carry-over-tool) fragment, before any `T` line in that layer. The test's `tools_per_ironing_block` reset `current_tool = None` at `;LAYER_CHANGE`, so the carry-over block collected zero tools and was dropped → union `{0}`. The physical tool persists across layers; parser fixed to keep it. Exposed (not caused) by the region-order change making T3 the top-layer carry-over. |
| `cube_4color_per_layer_outer_walls_fragment_by_color_with_tool_changes` | `crates/slicer-runtime/tests/executor/cube_4color_gcode_output_tdd.rs:945` | FIX — **fn confirmed 2026-07-16** (was "exact fn TBD"). Fails AC-4(a) at layer 78: 3 outer-wall header fragments vs 4 distinct tool indices (`headers={0,1,2} tools={0,1,2,3}`). **Classic**-perimeters path (arachne is dropped by claim-dedup in this test), so NOT Track C and unaffected by the arachne fixes. **RESOLVED 2026-07-17 — real emitter defect, fixed in `slicer-gcode::emit`.** The geometry was complete: T0's quarter-loop was extruded right after `T0`, but the emitter only re-emits `;TYPE:` on role CHANGE, and on alternate layers T0's purge is scheduled elsewhere so no wipe-tower entity separates the carry-over color's outer wall from T0's — the header was coalesced under the previous color's `;TYPE:Outer wall`. Fix: reset the role-dedup (`prev_role = None`) at every ToolChange, so each color's fragment carries its own header (ADR-0013 accounting; viewers segment by TYPE). Byte-neutral for single-tool prints (legacy golden re-verified green). |
