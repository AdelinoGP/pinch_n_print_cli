...
Packet 107 (overhang-pipeline-consumers-and-refactor) delivered the consumer stack for `overhang-pipeline-restructuring.md` Phase 3: `SliceRegionView::overhang_quartile_polygons() -> &[QuartileBand]` (SDK), the WIT `quartile-band` record + accessor, the host populator (`overhang_areas` + `overhang_quartile_polygons`, keyed by `global_layer_index`), and a refactored `overhang-classifier-default` (78 LOC, reads per-vertex `Point3WithWidth.overhang_quartile`, deletes the wall-distance `classify.rs`/`lines_distancer.rs` per ADR-0022). This closes D-10 and D-12; T-024 and T-077 are unblocked (their preconditions are satisfied). Two gaps remain, registered as follow-up tasks:

| ID | Title | Phase | Files | Acceptance |
|---|---|---|---|---|
| T-024-WIRE-VIEW-CONSUMER | Perimeter generation still writes `overhang_quartile = None` (P104 placeholder, `crates/slicer-core/src/perimeter_utils.rs:311`) — wire `classic-perimeters`/`arachne-perimeters` to read `SliceRegionView::overhang_quartile_polygons()` and write per-vertex `Some(quartile)` via point-in-polygon test, completing T-024. | Phase 2 | `modules/core-modules/{classic,arachne}-perimeters/src/lib.rs`, `crates/slicer-core/src/perimeter_utils.rs` | Overhang-ramp fixture: vertices in flagged quartile band carry expected quartile value. The production-coupled tripwire test `overhang_pipeline_e2e_tdd.rs` (currently red/inert because the field is always `None`) goes green, and its ready-made full-propagation assertions are enabled. |
| T-077-GUEST-ADAPTER-FIELDS | The guest-side macro adapter (`crates/slicer-macros` `__slicer_adapt_slice_regions`) does not map `overhang_areas`/`overhang_quartile_polygons`/`surface_group` into guest `SliceRegionView`s — only host-side/native consumers can see them today. Extend the adapter so real WASM Layer modules (e.g. `classic-perimeters` for T-077) can read these fields. | Phase 2 | `crates/slicer-macros/src/lib.rs` | Guest-side TDD: a WASM Layer module reading `overhang_areas()`/`overhang_quartile_polygons()` via the macro-generated adapter observes the same values as the native/host path. |
...
| T-024 | **UNBLOCKED (P107) — preconditions met.** `[precondition: overhang-pipeline-restructuring Phase 3]` was satisfied by P107: `SliceRegionView::overhang_quartile_polygons()` (SDK), the WIT `quartile-band` accessor, and the host populator all landed. Perimeter module reads `overhang_quartile_polygons()` and propagates per-vertex `Point3WithWidth.overhang_quartile` via point-in-polygon test, mirroring T-020's `is_bridge` pattern. Re-registered as follow-up task **T-024-WIRE-VIEW-CONSUMER** (see below) since P107 itself was scoped as consumer-stack-only, not the perimeter-side wiring. | `modules/core-modules/{classic,variable-width}-perimeters/src/lib.rs`, `crates/slicer-core/src/perimeter_utils.rs` | Overhang-ramp fixture: vertices in flagged quartile band carry expected quartile value; vertices outside overhang regions carry `None`. |
