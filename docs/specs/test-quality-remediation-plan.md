...
| core | partial (core-beading-threshold-review, core-strengthen, core-retire, core-paint, core-brittle, core-cross, core-parity) | `core-flow-consolidation`; deleted the inline `tests` module and `#[test]` registrations in `crates/slicer-core/src/flow.rs`; consolidated survivors in `crates/slicer-core/tests/flow_tdd.rs` (`flow_tdd`); `core-bridge-dup-merge`: `fully_supported_candidate_rejected_zero_bridge_area` merged into the identical survivor `solid_underneath_span_produces_no_bridge_area`; surviving test home `bridge_false_site_gating_tdd`; `core-region-mapping-dup-merges`: `region_mapping_enumerate_chains` and `region_mapping_cross_product_entry_count` absorbed by `region_mapping_two_semantics_produces_cross_product_cardinality` in `algo_region_mapping_tdd`; `core-support-dup-merges`: changed support overhang and support geometry cases `coplanar_step_does_not_hide_the_contact`, `overhang_is_detected_once_at_the_step_layer`, `support_geometry_emits_for_2_layer_fixture`, and `build_emit_schedule_two_objects_per_object_semantics`; `core-wall-sequence-dup-merges`: absorbed inline twins `inner_outer_is_canonical_no_reorder`, `outer_inner_reverses`, `inner_outer_inner_sandwich`; migrated `inner_outer_inner_with_two_walls_swaps_outer_and_first_inner`; `core-geometry-dup-review`: deleted inline `segment_path_preserves_requested_endpoints` (folded into the six-case roster survivor `segment_path_never_emits_segments_longer_than_requested_limit_for_known_cases`) and absorbed inline `closest_point_on_segment_midpoint` into `point_to_segment_distance_squared_matches`  ; core-beading-threshold-review: KEEP; all three threshold tests kept, not merged or deleted; contrasting case is additive; core-strengthen: strengthened five named weak-oracle cases: `bridging_angle_is_deterministic`, `flow_correction_stays_positive_for_vertical_input`, `triangle_crossing_two_edges`, `triangle_crossing_one_edge_vertex_on_plane`, `boolean_ops_produce_expected_presence_for_overlapping_squares`; wider-bead oracle remains owned by `core-flow-consolidation`; core-retire: retired `clip_operation_variants_are_distinct` and `slice_closing_radius_zero_is_noop`; re-homed replacement `prepass_slice_closing_radius_gate_applies_only_when_positive`; core-paint: strengthened `driver_v2_empty_mesh_returns_input_slice_ir`, `driver_v2_no_paint_data_short_circuits`, and `driver_v2_empty_region_map_short_circuits` with nonempty-input guards and unchanged-path equality; replaced the no-panic ending of `prune_one_arc_preserves_border_nodes` with exact `deleted == true` plus empty-`arc_indices` pins; strengthened `from_colored_lines_twin_propagation_is_question_mark` with a nonempty diagram and added `from_colored_lines_invalid_input_returns_err_without_panic` pinning `Err(MmuGraphError::CoordinateOverflow(2147483648))`; probes closed with `vertex_color_get_set_round_trip` and `edge_is_primary_callable` retained and `boostvoronoi_supports_line_segment_sites` kept as a waived compile witness; no test renamed and no production code changed; core-brittle: KEEP-review of the three timing guards; all three retained with exact rationale markers (118s Benchy incremental-union regression → compute_xy_footprint_is_fast_for_thousands_of_disjoint_facets; O(layers) sweep → annotate_overhangs_is_fast_for_many_stacked_layers; ~28s per-layer footprint recomputation → prepass_slice_caches_bottom_surface_footprint_across_layers); thresholds from_secs(3), from_secs(1), and cached_elapsed * 3 < uncached_elapsed * 2 unchanged; criterion-bench migration (benches/polygon_ops.rs) evaluated and rejected (different surface, non-failing harness, no gate runs cargo bench); deterministic call-count accounting rejected (would require production instrumentation, out of test-only scope); no test renamed, added, or removed and no production code changed; core-cross: KEEP-review of the SDK/core offset, simplify, and raycast/bounds wrapper families; no test retired, renamed, or re-homed and no production code changed. Reviewed/kept SDK wrapper tests (crates/slicer-sdk/tests/host_wrappers_tdd.rs): raycast_and_normal_route_through_installed_mesh_source, raycast_returns_none_without_source_documented_signal, object_bounds_returns_host_unavailable_without_source, offset_polygons_shrinks_and_grows, simplify_polygon_drops_collinear_vertices, simplify_polygon_short_input_returned_as_is. Reviewed/kept SDK batch wrapper tests (crates/slicer-sdk/src/host_batch.rs): batch_offset_keeps_results_aligned_with_inputs, batch_forms_agree_with_the_singular_forms. New delegate-contract test added: `offset_polygons_with_miter_limit_clamps_sharp_miter_corners`, the sole cover of the Some(miter_limit) delegation arm to `slicer_core::polygon_ops::offset_with_miter_limit`. Reviewed/kept core counterparts: bounds_match_unit_cube_vertex_extrema, positive_z_raycast_from_below_hits_cube_bottom_face_first, raycast_all_hits_returns_sorted_entry_and_exit_intersections (crates/slicer-core/tests/aabb_tree_tdd.rs), offset_round_trip_preserves_hole_nesting, expolygons_simplify_preserves_square (crates/slicer-core/src/polygon_ops.rs `mod tests`). Each retained test carries an earn-their-keep `KEEP-review (core-cross)` marker naming its thin-delegate or distinct-implementation conclusion. SDK offset wrappers are NOT distinct implementations (thin delegates to core `polygon_ops::offset`), so no duplicate-implementation claim is made and no wrapper-vs-core equality pin was added (self-referential oracle); SDK `simplify_polygon` (inline collinear drop) vs core `expolygons_simplify` (RDP) and the SDK MeshSource raycast/bounds route vs core `AabbTree` are genuinely distinct and both sides are retained. ; the parity packet is additive-only: no symbol was retired, renamed, or re-homed by it, and its only change to pre-existing test code is the replacement of the placeholder line `let _ = max_deviation_x; // placeholder for the tighter test below` in `crates/slicer-core/tests/arachne_construction_insert_node_rib_split.rs` (`apply_transitions_split_position_coincides_on_both_sides`) with a real assertion. | `flow_tdd`: canonical and wider-bead spacing oracle; `width_below_layer_height_still_has_positive_spacing`; `spacing_errors_at_the_canonical_threshold`; zero/negative-width, actionable-error, round-trip, monotonicity, and flow-to-width survivors; bridge ratio/factor/degenerate-fallback cases; role-width precedence and fallback matrices; `unsupported_span_retains_bridge_area`; `ungated_candidates_cannot_silently_return`; `existing_empty_lower_layer_retains_bridge_area`; `region_mapping_two_semantics_produces_cross_product_cardinality` exact cardinality and six-chain coverage; `region_mapping_chains_ordered_by_aggregated_region_split_canonical_order` canonical-order witness; surviving support overhang/support geometry coverage: `overhang_is_detected_once_at_the_step_layer`, `emits_for_2_layer_fixture`, `build_emit_schedule_two_objects_per_object_semantics`, and `empty_plan_produces_empty_support`; `wall_sequence_reorder_tdd` (ungated): `inner_outer_canonical_order`, `outer_inner_reversed_order`, `inner_outer_inner_sandwich_order` with loop-type plus per-slot identity assertions; `inner_outer_inner_with_two_walls_swaps_outer_and_first_inner` N==2 sandwich; `empty_walls_is_noop`; `single_wall_unchanged_for_all_modes`; `geometry_helpers_tdd`: six-case segment_path union including the preserved `(2.0, 0.75)` endpoint tuple with exact unit-equality endpoint witnesses and per-chord cap/non-degeneracy asserts; consolidated `point_to_segment_distance_squared_matches`: dual-API wrapper+delegate calls with two independent `25_000_000.0_f64` oracles plus projection and wrapper-delegate `assert_eq!` witnesses  ; core-beading-threshold-review: beading_factory_passes_split_middle_thresholds; beading_factory_threshold_propagates_through_full_stack; beading_factory_threshold_clamp_bounds_are_canonical; retained default-width .99/.99 case plus additive 3000/5000/4000 case asserting split 0.20 and add 0.75; core-strengthen: `bridging_angle_is_deterministic` pins finite 90.0 degrees; `flow_correction_stays_positive_for_vertical_input` pins finite exactly 1.0; `triangle_crossing_two_edges` pins unordered Point2::from_mm(5.0,0.0) and Point2::from_mm(2.5,5.0); `triangle_crossing_one_edge_vertex_on_plane` pins unordered Point2::from_mm(0.0,0.0) and Point2::from_mm(7.5,5.0); `boolean_ops_produce_expected_presence_for_overlapping_squares` pins union area 15_000_000_000 bounds (0,0)-(15,10) mm components=1, intersection area 5_000_000_000 bounds (5,0)-(10,10) components=1, difference area 5_000_000_000 bounds (0,0)-(5,10) components=1, xor area 10_000_000_000 bounds (0,0)-(15,10) components=2, plus `shape_signature` pairwise distinctness; core-retire: re-homed replacement `prepass_slice_closing_radius_gate_applies_only_when_positive` to `crates/slicer-core/tests/algo_prepass_slice_tdd.rs`; retained `slice_closing_radius_fuses_gap_within_two_r`; function-count deltas: `crates/slicer-core/src/polygon_ops.rs` 21->20, `crates/slicer-core/tests/triangle_mesh_slicer_tdd.rs` 12->11, `crates/slicer-core/tests/algo_prepass_slice_tdd.rs` 6->7; core-paint: `driver_v2_empty_mesh_returns_input_slice_ir`, `driver_v2_no_paint_data_short_circuits` and `driver_v2_empty_region_map_short_circuits` pin nonempty inputs plus `polygons`/len/region-count equality; `prune_one_arc_preserves_border_nodes` pins `deleted == true`, both endpoints empty `arc_indices`, and node survival; `from_colored_lines_twin_propagation_is_question_mark` pins the valid-input `Ok` (`is_ok()`) with a nonempty-diagram witness, paired with `from_colored_lines_invalid_input_returns_err_without_panic`, which pins the exact typed `CoordinateOverflow(2147483648)` an `is_err`-swallowing or panicking implementation would fail to produce.; probe oracles retained: `vertex_color_get_set_round_trip` pins a `42`/`0` colour round-trip with original-diagram isolation; `edge_is_primary_callable` pins the `is_primary` edge witness; `boostvoronoi_supports_line_segment_sites` is a waived compile witness; core-brittle survivors: compute_xy_footprint_is_fast_for_thousands_of_disjoint_facets keeps FACET_COUNT: usize = 1200 with a live assert_eq!(footprint.len(), FACET_COUNT) and the from_secs(3) guard; annotate_overhangs_is_fast_for_many_stacked_layers keeps CUBE_COUNT: usize = 1200 with a live assert!(result.is_empty()) and the from_secs(1) guard; prepass_slice_caches_bottom_surface_footprint_across_layers keeps the live cached_elapsed * 3 < uncached_elapsed * 2 ratio guard on 800-facet, 20-layer inputs (self-normalizing 1.5x minimum vs ~3.5x debug / ~2.3x release); core-cross survivors: raycast_and_normal_route_through_installed_mesh_source; raycast_returns_none_without_source_documented_signal; object_bounds_returns_host_unavailable_without_source; offset_polygons_shrinks_and_grows; offset_polygons_with_miter_limit_clamps_sharp_miter_corners; simplify_polygon_drops_collinear_vertices; simplify_polygon_short_input_returned_as_is; batch_offset_keeps_results_aligned_with_inputs; batch_forms_agree_with_the_singular_forms; bounds_match_unit_cube_vertex_extrema; positive_z_raycast_from_below_hits_cube_bottom_face_first; raycast_all_hits_returns_sorted_entry_and_exit_intersections; offset_round_trip_preserves_hole_nesting; expolygons_simplify_preserves_square; each retained test carries a `KEEP-review (core-cross)` marker naming its regression input; the new miter-limit test pins `assert_ne` on the `+1.0` outward clamped-vs-default contour and a strict point-count increase, with the `1.2` limit the only value that clamps below the `1/sin(45deg)` approx 1.414 corner ratio (core default `2.0` does not clamp), and the live anchored `assert_ne!` is the load-bearing assertion. ; core-parity packet (additive-only; ten new `#[test]` functions across nine files, nothing retired): `get_nearest_beding_includes_vertex_at_exact_radius_boundary` (vertex at the exact `1000`-unit radius boundary, `edge_len <= radius_units` inclusion, plus the `999.5` just-outside and `0.0` zero-radius probes; oracle `exact_radius_boundary`); `apply_transitions_diagonal_source_mid_r_and_foot_sentinels` (diagonal source, shared mid node pinned to `mid_r == 750_000.0` units, twin-crossing foot sentinels `distance_to_boundary == 0.0` / `bead_count == None`, oracle `mid_r`); `apply_transitions_split_topology_exact_counts_and_cross_twin_patch` (exact `3`-vertex / `6`-edge split topology, `4` `EXTRA_VD` ribs in `2` pairs plus `2` `NORMAL` fragments, cross-twin links intact, oracle `cross_twin`); `f5_invariant_node_distances_match_rib_geometry_and_boundary` (L-shape `93` valid `EXTRA_VD` rib pairs, `95` total, `2` dangling stubs; per-vertex `distance_to_boundary` pins); `dumbbell_wide_gap_not_dissolved_pins_exact_ring_topology` (`1.5mm` `> 0.4` `max_dist` gap pinned to the measured `21`-junction closed ring, oracle `single_closed_ring`); `dumbbell_narrow_gap_dissolves_to_single_closed_ring` (`0.3` mm gap dissolves to one closed `13`-junction ring); `limited_inserts_single_centre_sentinel_at_cap_boundary` (at-cap odd case `24000.0` / `6`, oracle `centre_sentinel`, single centre sentinel at index `3`); `chain_junctions_land_at_documented_interpolated_positions` (chain reachability at measured `1.25mm` / `8.75mm`, oracle `1.25mm` and `8.75mm`); `arachne_params_absent_keys_fall_back_to_defaults_per_key` (empty `ConfigView`, per-key fallback; default `initial_layer_min_bead_width == 0.34`, oracle `0.34`); `canonical_remove_small_first_keeps_line_simplify_first_drops_it` (canonical order keeps, swapped order drops; oracle `remove_small`). Oracle tokens also include `unwrap_or`. THREE MEASURED DIVERGENCES this packet found and deliberately pinned as measured truth with in-file divergence comments (test-only; no production change was made for any of them): (a) F3 junction reachability: on the `make_f3_target_graph` fixture the emitted chain's three junctions are x = `1.25`, `1.25`, `8.75` mm, i.e. e0's junction is emitted twice and the shared middle vertex v1's junction is never reached, contradicting the file's documented interpolation claim (`t = 0.75`, expected merge near `5` mm); (b) F5 node distances: of `95` `EXTRA_VD` rib pairs, `93` have valid endpoints and `2` are dangling non-reciprocal stubs; all `93` valid pairs match the perpendicular-to-infinite-source-line definition, and `2` pairs carry a `1.0524414244282525`-unit quantization delta versus an integer-coordinate Euclidean endpoint oracle (implementation is self-consistent under the correct definition); (c) dumbbell topology: the `1.5` mm (`> 0.4` mm `max_dist`) wide gap measures a single closed inset-0 ring with `21` junctions, not the fragmented outer wall the canonical not-dissolved claim predicts; the `0.3` mm narrow gap measures one closed ring with `13` junctions.  | cargo test -p slicer-core --features host-algos --test geometry_helpers_tdd (AC-2/AC-4); `cargo test -p slicer-core --features host-algos --test bridge_false_site_gating_tdd` PASS (five named tests); `cargo test -p slicer-core --features host-algos --test flow_tdd -- --nocapture` PASS (`target/packet01-ac-1.log`, `target/packet01-ac-2.log`, `target/packet01-ac-3.log`, `target/packet01-ac-4.log`, `target/packet01-ac-5.log`, `target/packet01-ac-6.log`, `target/packet01-ac-N1.log`, `target/packet01-flow-verification.log`); `cargo build --workspace` PASS (`target/packet01-build.log`); `cargo check --workspace --all-targets` PASS (`target/packet01-check.log`); `cargo clippy --workspace --all-targets -- -D warnings` PASS (`target/packet01-clippy.log`); `cargo xtask check-literals` PASS (`target/packet01-check-literals.log`); `cargo xtask check-test-quality --report crates/slicer-core/src/flow.rs crates/slicer-core/tests/flow_tdd.rs` PASS (`target/packet01-quality-gate.log`); region-mapping `cargo test`, `cargo check --workspace --all-targets`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo xtask check-literals`, and `check-test-quality` validations recorded; support overhang/support geometry `cargo test` PASS (feature-correct 18-test and 3-test targets); `cargo check --workspace --all-targets` PASS; `cargo clippy --workspace --all-targets -- -D warnings` PASS; `cargo xtask check-literals` PASS; `cargo xtask check-test-quality --report` PASS; wall sequence (ungated): `cargo test -p slicer-core --test wall_sequence_reorder_tdd` PASS (6-test target); `cargo check --workspace --all-targets` PASS; `cargo clippy --workspace --all-targets -- -D warnings` PASS; `cargo xtask check-literals` PASS; `cargo xtask check-test-quality --report` PASS  ; core-beading-threshold-review: `cargo test -p slicer-core --features host-algos --test beading_factory`; full beading_factory target 8 passed, 0 failed; `cargo check --workspace --all-targets`; `cargo clippy --workspace --all-targets -- -D warnings`; `cargo xtask check-literals`; `cargo xtask check-test-quality --report`; `cargo test -p slicer-core --features host-algos --test polygon_ops_tdd -- --exact boolean_ops_produce_expected_presence_for_overlapping_squares`; core-retire validation: `cargo test -p slicer-core --features host-algos --test algo_prepass_slice_tdd -- --exact prepass_slice_closing_radius_gate_applies_only_when_positive`; core-paint: `cargo test -p slicer-core --features host-algos --lib prune_one_arc_preserves_border_nodes` PASS; `cargo check --workspace --all-targets` PASS; `cargo clippy --workspace --all-targets -- -D warnings` PASS; `cargo xtask check-literals` PASS; `cargo xtask check-test-quality --report crates/slicer-core/src/algos/paint_segmentation/` PASS; core-brittle validation: `cargo test -p slicer-core --features host-algos --lib compute_xy_footprint_is_fast_for_thousands_of_disjoint_facets` PASS; `cargo test -p slicer-core --features host-algos --lib annotate_overhangs_is_fast_for_many_stacked_layers` PASS; `cargo test -p slicer-core --features host-algos --test algo_prepass_slice_tdd -- --exact prepass_slice_caches_bottom_surface_footprint_across_layers` PASS; core-cross validation: `cargo test -p slicer-sdk --features test --test host_wrappers_tdd -- offset_polygons_with_miter_limit_clamps_sharp_miter_corners` PASS; `cargo test -p slicer-sdk --features test --lib batch_offset_keeps_results_aligned_with_inputs` PASS; `cargo test -p slicer-sdk --features test --lib batch_forms_agree_with_the_singular_forms` PASS; `cargo test -p slicer-sdk --features test --test host_wrappers_tdd -- simplify_polygon` PASS (2 tests); `cargo test -p slicer-sdk --features test --test host_wrappers_tdd -- raycast` PASS (2 tests); `cargo test -p slicer-sdk --features test --test host_wrappers_tdd -- object_bounds` PASS; `cargo test -p slicer-core --features host-algos --test aabb_tree_tdd` PASS (6 tests); `cargo test -p slicer-core --features host-algos --lib offset_round_trip_preserves_hole_nesting` PASS; `cargo test -p slicer-core --features host-algos --lib expolygons_simplify_preserves_square` PASS; bare-run twelve-test roster witness (self dev-dependency satisfies the `test` feature; no cargo rejection is available) and zero-match-filter `0 passed` non-vacuity control; `cargo check --workspace --all-targets` PASS; `cargo clippy --workspace --all-targets -- -D warnings` PASS; `cargo xtask check-literals` PASS; `cargo xtask check-test-quality --report` PASS; `cargo xtask build-guests --check` PASS. core-parity packet validation: `cargo test -p slicer-core --features host-algos --test arachne_postprocess_order` PASS (representative; all ten new tests run individually with the exact filter and pass); `cargo check --workspace --all-targets`; `cargo clippy --workspace --all-targets -- -D warnings`; `cargo xtask check-literals`; `cargo xtask check-test-quality --report` (all five required gate tokens recorded; each new test executed green with `--features host-algos`).  | core wave partial: this packet closed the two §5.1 DUP-CORE geometry items; remaining core-wave items stay open: beading (row #8), strengthen, retire, paint, brittle, cross, parity ; core-beading-threshold-review (superseding the row #8 mention above): beading (row #8) is covered by this packet; core-strengthen remains open; core-strengthen (this packet): the five named weak-oracle cases now carry independent analytic or numeric oracles; remaining core-wave gap: census-target-scope; parity-packet measured divergences (test-only baselines, no production change; tracked open): F3 junction reachability — the emitted 3-junction chain on the make_f3_target_graph fixture is x = 1.25, 1.25, 8.75 mm, so e0's junction is emitted twice and the shared middle vertex's junction is never reached (contradicts the file's documented t = 0.75 interpolation); F5 node distances — 2 of 95 EXTRA_VD rib pairs are dangling non-reciprocal stubs and 2 pairs carry a 1.0524414244282525-unit quantization delta versus an integer-coordinate Euclidean endpoint oracle (the implementation matches the perpendicular-to-infinite-source-line definition); dumbbell topology — the 1.5 mm wide gap (pad bead counts 7 vs 8, differ by exactly 1, so canonical's strict < 0.4 mm dissolution rule does NOT apply) measures a single closed inset-0 ring with 21 junctions instead of the fragmented outer wall |
| ir | open | — | — | — | — |
| model-io | open | — | — | — | — |
| gcode | open | — | — | — | — |
| helpers | open | — | — | — | — |
