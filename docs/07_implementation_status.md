...
- [~] TASK-152 Expand `path-optimization-default` beyond comment-only output into a real optimization stage with deterministic travel ordering, module-side z-hop planning, and explicit coverage for the remaining DEV-023 feature gaps. Continues DEV-023 and supports TASK-120d. **TASK-152a/152d/152e slice closed 2026-04-26 — packet `18_path-optimization-entity-ordering` adds `order_entities_by_nearest_neighbor` in `layer_executor.rs`, integrates it on the pre-PathOptimization host surface, and delivers 6 acceptance tests in `path_ordering_tdd.rs`. Remaining open: z-hop planning, tool-change ordering, cooling policy (152b/c), finalization geometry coordination (152f).**
