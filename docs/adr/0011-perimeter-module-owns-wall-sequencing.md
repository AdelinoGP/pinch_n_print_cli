...
- The `wall_sequence` config key is owned by `classic-perimeters` and (pre-P108) `arachne-perimeters`. The config-key registration migrates from `path-optimization-default` accordingly. **Amendment (2026-06-19):** `variable-width-perimeters` was never shipped; the iterative-inset module is deleted under P108 and real Arachne is introduced fresh under P110+P112. **Amendment (P108):** the fake `arachne-perimeters` module (iterative-inset approximation) was deleted; `classic-perimeters` is the sole perimeter generator until real Arachne lands under P110+P112 and re-registers `wall_sequence` under the same module id.
- Reordering logic — including `InnerOuterInner` sandwich grouping — lives in the shared `slicer_core::perimeter_utils` module (T-010; **shipped as a module under `slicer-core`, not a standalone `slicer-perimeter-utils` crate** — P102 picked the module form to avoid Cargo/WIT churn; references to `slicer-perimeter-utils` elsewhere in this ADR mean `slicer_core::perimeter_utils`), so every perimeter module calls one implementation.
