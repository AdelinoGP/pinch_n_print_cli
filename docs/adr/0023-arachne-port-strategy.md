...
- **Point-cell `source_index()` dedup ambiguity (Packet 113c).** `boostvoronoi`'s point-cell `source_index()` is **not** deterministically "previous segment in ring order" — at a polygon's wraparound seam it resolves to whichever adjacent segment has the lower raw input-array index. Provenance resolution in graph construction must therefore match by **coordinate against ring vertices**, not by trusting a specific `source_index()`, mirroring OrcaSlicer's own point-cell-range resolution.
