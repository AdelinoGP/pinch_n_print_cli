...
Partially superseded (2026-08-07) by ADR-0056: the second rejected alternative below — a host-native-only module, rejected because it "would require a second module-loading code path" — no longer holds, because integrated modules (ADR-0056) dispatch natively while remaining inside the single scheduler/manifest model. The four-layer bridge pattern this ADR records remains the standard for guest access to host-only algorithms.
...
1. **WIT declaration** in `crates/slicer-schema/wit/deps/common.wit`'s `host-services` interface: a `func` taking plain data (polygons, params — no opaque handles) and returning `result<T, string>`. `generate-arachne-walls: func(polygons: list<ex-polygon>, params: arachne-params) -> result<list<extrusion-line>, string>;` (`common.wit`) mirrors `medial-axis: func(input: ex-polygon, min-width: f32, max-width: f32) -> result<list<thick-polyline>, string>;` (`common.wit`) exactly in this respect.
...
4. **Guest module** calls only the SDK wrapper for bridged algorithms (`modules/core-modules/arachne-perimeters/src/lib.rs` calls `slicer_sdk::host::generate_arachne_walls`, never the `host-algos`-gated `slicer-core` stack directly). `arachne-perimeters`'s own `Cargo.toml` takes `slicer-core` only with `default-features = false`, i.e. the ungated polygon math (`flow`, `perimeter_utils`, `polygon_ops`). *(Corrected 2026-08-07: this ADR originally claimed the guest had no `slicer-core` dependency at all, which had drifted from the tree.)*
