...
2. **`raft-default` ships its own pattern code**, accepting per-module duplication. The existing duplication between `rectilinear-infill::scan_expolygon` and `traditional-support::fill_expolygon` would grow to a third copy.
...
DEV-127, dependent on a future WIT-interface pattern-services design).
