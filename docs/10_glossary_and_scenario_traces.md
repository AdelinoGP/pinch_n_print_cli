...
| Default extrusion-role priority | The `u32` value returned by `ExtrusionRole::default_priority()` used by `push_entity_with_priority` when the caller supplies no explicit priority. Lower numbers print earlier. | Full priority table in `docs/02_ir_schemas.md` IR 10. Added packet 40. |
| `entity_id` / Stable entity ID | Per-layer monotonic `u64` identifier on `PrintEntity` and `TravelMove`, populated by `LayerEntityIdGen`. Travel anchors and finalization mutations reference entities by `entity_id`, not by positional index, so sorts and inserts cannot invalidate anchors. Reserved value `0` means uninitialized. | Generator resets at each new `LayerCollectionIR`. Added packet 39. See `docs/02_ir_schemas.md` IR 10. |
| Finalization mutation primitive | One of `push_entity_with_priority`, `modify_entity`, `sort_layer_by`, `insert_synthetic_layer_after` (packet 40). Each mutation argument is a serialisable enum (`EntityMutation`, `SortKey`, `SyntheticLayerData`; packet 41) so the operation crosses the WIT boundary intact. | See `docs/05_module_sdk.md` 'Finalization mutation API'. |
| `LayerCollectionBuilder` | WIT resource (packet 32) by which `Layer::PathOptimization` modules return their entity ordering. Methods: `set_entity_order(items: list<tuple<u32, bool>>)` and `get_ordered_entities() -> list<OrderedEntityView>`. Tuples are `(entity_index, reverse_direction)`. | Host validates uniqueness and bounds. See `docs/03_wit_and_manifest.md` `world-layer.wit`. |
| `LayerEntityIdGen` | Per-layer generator for `entity_id` values. `Cell<u64>`, starts at `1`. Producers call `id_gen.next()` once per constructed `PrintEntity` / `TravelMove`. | Validator `validate_travel_anchors(layer)` short-circuits on the first dangling anchor. Added packet 39. See `docs/02_ir_schemas.md` IR 10. |
| Overhang quartile | `Point3WithWidth.overhang_quartile: Option<u8>` in `1..=4`, populated by the overhang classifier prepass inside `DefaultGCodeEmitter::emit_gcode` for wall-family roles (`OuterWall`, `InnerWall`, `ThinWall`). Drives per-point dispatch into `overhang_1_4_speed` … `overhang_4_4_speed`. | Classifier short-circuits when all four speed keys are 0.0 (byte-identical no-op). Added packet 57. See `docs/02_ir_schemas.md` IR 7. |
| `RetractMode` | Per-`GCodeCommand::Retract` / `Unretract` enum (`Gcode` \| `Firmware`) added packet 34. `Gcode` emits `G1 E-<length> F<speed>`; `Firmware` emits parameterless `G10` / `G11`. M207 / M208 are intentionally never emitted (OrcaSlicer parity). | Selected by the `retraction_mode` config key. See `docs/02_ir_schemas.md` IR 11. |
| Role-keyed precision tolerance | Per-role Douglas-Peucker simplification budget applied at G-code emit. Three host config keys (`gcode_resolution`, `infill_resolution`, `support_resolution`) plus `min_segment_length` cap fine geometry; `perimeter_arc_tolerance` is per-module on perimeter generators; `slice_closing_radius` runs a per-layer Clipper2 inflate-deflate round-trip in `triangle_mesh_slicer`. | Added packet 60. Table in `docs/02_ir_schemas.md` IR 11. |
