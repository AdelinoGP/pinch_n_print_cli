...
- `docs/spec_packets/204-hybrid-pilot-parity/packet.spec.md` and `docs/spec_packets/203-integrated-cli-provenance/packet.spec.md` (both short) — read-only, whole file each, to reconcile FORWARD-DEP shapes. Never modified.
...
- `xtask/src/editions.rs` is a FORWARD-DEP file that will not exist until 204 lands. If it is absent, read `docs/spec_packets/204-hybrid-pilot-parity/packet.spec.md` (AC-7, AC-N1) for the shape and stop — do not read 204's `design.md` or `implementation-plan.md` directly; delegate a `SUMMARY` if more is needed.
