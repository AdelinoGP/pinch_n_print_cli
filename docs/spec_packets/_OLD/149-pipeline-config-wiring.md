...
- The four overhang/top-surface/wall/bridge-flow deviation rows' `Status` fields flip to `Closed — 2026-07-09: packet 149` at packet close.
...
- **Risk:** the 6 new deviation log rows change the table's row count. The `cargo xtask check-deviations` tool (per deviation audit history documentation on non-authoritative views) regenerates `docs/07_implementation_status.md`'s Open Deviation Map. **Mitigation:** the implementer runs `cargo xtask check-deviations` after the deviation log edit; the regenerated file is committed in the same packet.
