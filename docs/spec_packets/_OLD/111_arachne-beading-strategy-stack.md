...
- **`Arachne sentinel-strip decision` rationale.** The strip-pass is a deliberate divergence from OrcaSlicer (which carries zero-width sentinels into the toolpath output and relies on infill modules to skip them). The `DEVIATION_LOG.md` entry MUST explain WHY this codebase strips instead — namely, the WIT-boundary `WallLoop` type's invariant that `bead_widths.iter().all(|&w| w > 0.0)` (avoiding a contract change at the boundary). D-9 in the roadmap already records the decision; `Arachne sentinel-strip decision` records the implementation log entry using the log's `D-<pkt>-<SLUG>` convention.
