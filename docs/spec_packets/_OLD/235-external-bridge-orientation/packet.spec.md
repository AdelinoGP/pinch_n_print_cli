...
- Depends on: `docs/spec_packets/233-internal-bridge-over-infill/` and `docs/spec_packets/234-bridge-false-site-gating/`. Both prerequisite packets are implemented and their net-new symbols were verified present on disk at this packet's preflight (233: `ExtrusionRole::InternalBridgeInfill`, `bridge_over_infill.rs`, `bridge_extrusion_spacing`/`BRIDGE_EXTRA_SPACING`; 234: `gate_bridge_areas_by_unsupported_span` in `crates/slicer-core/src/algos/prepass_slice.rs` and `crates/slicer-core/tests/bridge_false_site_gating_tdd.rs`; 234's optional `grow_anchor_areas` was not needed and does not exist). They must not be re-derived from the status lines of those packets at later read times — verify symbol presence directly.
