...
- Fork-readiness gate: if `D:\wit-bindgen` is not on `feat/assemblyscript-no-async`, has any tracked or untracked changes, or the user has not confirmed the concurrent async-support work is committed, stop and ask the user. Never stash, clean, switch branches, pull, or probe the in-progress tree.
...
- **AC-5. Given** the four candidate probe records, **when** packet verification reads their evidence fields, **then** each records at least one probe round, its **terminal** (last) result is one of the two permitted values, each round carries its own 64-hex component SHA-256, tool/host command and host-output fields are non-empty, and no blocker marker remains. | `python3 -c "import pathlib,re; names=('moonbit','assemblyscript','cpp','go'); docs=[pathlib.Path(f'docs/feasibility-probes/{n}-text-postprocess.md').read_text(encoding='utf-8') for n in names]; res=lambda d: re.findall(r'^RESULT: (\S+)$',d,re.M); sha=lambda d: re.findall(r'^COMPONENT_SHA256: ([0-9a-f]{64})',d,re.M); ok=lambda d: len(res(d))>=1 and res(d)[-1] in ('LOADABLE_AND_CORRECT','NOT_LOADABLE_OR_CORRECT') and len(sha(d))==len(res(d)) and all(re.search(rf'^{f}: .+',d,re.M) for f in ('TOOL_VERSIONS','HOST_COMMAND','HOST_OUTPUT')) and 'BLOCKED:' not in d; bad=[n for n,d in zip(names,docs) if not ok(d)]; assert not bad,bad; print('PASS')"`
- **AC-6. Given** the user has confirmed the concurrent fork work is committed and `D:\wit-bindgen` is clean on `feat/assemblyscript-no-async`, **when** the AssemblyScript evidence is finalized, **then** it records the 40-hex HEAD resolved immediately before generation, `WIT_BINDGEN_STATUS: clean`, UTF-16 component embedding, and world `slicer:postpass-text-postprocess/text-postprocess-module`. | `python3 -c "import pathlib,re; t=pathlib.Path('docs/feasibility-probes/assemblyscript-text-postprocess.md').read_text(); assert re.search(r'^WIT_BINDGEN_HEAD: [0-9a-f]{40}$',t,re.M); req=('WIT_BINDGEN_BRANCH: feat/assemblyscript-no-async','WIT_BINDGEN_STATUS: clean','UTF-16','slicer:postpass-text-postprocess/text-postprocess-module'); assert all(x in t for x in req),[x for x in req if x not in t]; print('PASS')"`
- **AC-7. Given** all four completed probe results, **when** the Dragon Curve language is selected, **then** `docs/14_submodule_programming_languages.md` contains exactly one selection line naming the first candidate whose **terminal** result is `LOADABLE_AND_CORRECT` in the locked order `MoonBit, AssemblyScript, C++, Go`, or `Rust` only if all four terminal results are `NOT_LOADABLE_OR_CORRECT`. | `python3 -c "import pathlib,re; order=(('MoonBit','moonbit'),('AssemblyScript','assemblyscript'),('C++','cpp'),('Go','go')); term=[(label,re.findall(r'^RESULT: (\S+)$',pathlib.Path(f'docs/feasibility-probes/{slug}-text-postprocess.md').read_text(encoding='utf-8'),re.M)[-1]) for label,slug in order]; assert len(term)==4; expected=next((l for l,v in term if v=='LOADABLE_AND_CORRECT'),'Rust'); found=re.findall(r'\*\*Dragon Curve authoring language: ([^*]+)\*\*',pathlib.Path('docs/14_submodule_programming_languages.md').read_text(encoding='utf-8')); assert found==[expected],(expected,found); print('PASS')"`
...
- **AC-N2. Given** a simulated absent `asc` command, **when** the shared prerequisite gate runs, **then** it exits 42, prints `BLOCKED: TOOLCHAIN assemblyscript asc`, prints the AssemblyScript installation plus version-verification instructions, and leaves the AssemblyScript result record byte-unchanged (creating none where none exists). | `set -o pipefail; rec=docs/feasibility-probes/assemblyscript-text-postprocess.md; before=$(git hash-object "$rec" 2>/dev/null || echo absent); log=$(mktemp); docs/feasibility-probes/foreign-language-text-postprocess/check-prerequisites.sh --simulate-missing asc >"$log" 2>&1; rc=$?; after=$(git hash-object "$rec" 2>/dev/null || echo absent); test "$rc" -eq 42 && grep -F 'BLOCKED: TOOLCHAIN assemblyscript asc' "$log" && grep -F 'INSTALL:' "$log" && grep -F 'VERIFY:' "$log" && test "$before" = "$after"; out=$?; rm -f "$log"; exit $out`
...
1. **AssemblyScript fork branch is `feat/assemblyscript-no-async`, not `feat/assemblyscript-backend`.** The read-only
   local fork `D:\wit-bindgen` carried the AssemblyScript generator on `feat/assemblyscript-no-async` at probe time;
   that is the branch recorded in `docs/feasibility-probes/assemblyscript-text-postprocess.md`
   (`WIT_BINDGEN_BRANCH`, `WIT_BINDGEN_STATUS: clean`, 40-hex `WIT_BINDGEN_HEAD`). AC-6, the scope/prerequisite prose,
   `design.md`, `requirements.md`, and `implementation-plan.md` were corrected to the branch actually used. The
   fork-readiness contract itself (clean tree, committed work, HEAD captured immediately before generation, never
   stash/clean/switch/pull) is unchanged and was honoured.

2. **Probe evidence records two rounds per candidate, not one.** MoonBit, C++, and Go were each re-measured after a
   first failing round — MoonBit's fixture packaging fix, C++ and Go re-run under the packet 225a accommodating host.
   Each round carries its own `RESULT:` and `COMPONENT_SHA256:`. AC-5 originally required exactly one of each and
   AC-7 read the *first* `RESULT:` line, which would have selected AssemblyScript; both were corrected to terminal
   (last) result semantics. The shipped selection, `**Dragon Curve authoring language: MoonBit**`, is correct under
   the locked priority order applied to terminal results, and is what packet 227 consumed.

3. **AC-N2 was self-defeating as authored.** It asserted `test ! -e docs/feasibility-probes/assemblyscript-text-postprocess.md`,
   so it could only pass on a tree where the packet had not yet produced its AssemblyScript evidence — it fails by
   construction on any completed run. Rewritten to assert the real intent: a `BLOCKED: TOOLCHAIN` gate leaves the
   result record byte-unchanged (`git hash-object` before/after), creating none where none exists.

4. **AC-9 and the matching Doc Impact bullet contained an unescaped backtick** inside their `python3 -c "..."`
   command (`'wit-bindgen<backtick> rust 0.57.1 (the host)'`), which opened an unterminated command substitution in
   the shell and broke the surrounding markdown code span — the command could never run as written. The assertion
   was narrowed to the backtick-free substring `'rust 0.57.1 (the host)'`, which checks the same MoonBit historical
   attribution.
