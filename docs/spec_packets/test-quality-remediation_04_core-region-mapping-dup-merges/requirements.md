...
- Re-derive the before/after discovery and execution census at implementation time and reconcile the expected 24 → 22 delta; stop if the live baseline differs from the packet's grounded premise.
...
- New tests, renamed survivors (other than deleting the absorbed names), strengthened oracles, parameterization of the 21 sibling tests, or WIT/IR/schema/coordinate/WASM/OrcaSlicer changes.
...
- `docs/spec_packets/_OLD/93_region-mapping-cross-product.md` - provenance of the `AC-3`/`AC-4`/`AC-9` acceptance aliases the survivor's doc comment cites, and of the Cross-product cardinality invariant whose live doc-comment home is `DEFAULT_REGION_MAP_CAP` (`crates/slicer-ir/src/slice_ir.rs`, read-only).
...
None. This packet retires duplicate tests without changing the production behavior established by the implemented P93 region-mapping packet. No OrcaSlicer source lookup, port, or attribution change is required.
