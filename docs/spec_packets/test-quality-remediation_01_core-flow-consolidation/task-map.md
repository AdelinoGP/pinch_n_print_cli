...
| `core/DUP-CORE-strengthen (wider-bead)` | Steps 1 and 2 | `docs/specs/test-quality-remediation-plan.md` §5.1 and §7; `docs/22_test_quality.md` | Step 1: `wider_bead_spacing_is_larger_than_canonical` in `flow_tdd.rs`; Step 2: §7 `core` row | None; no porting | `S` | Preserve and verify the existing independent formula oracle and strict wider-than-canonical assertion; then record partial evidence with final cell `remaining non-flow §5.1 work stays open; next queued gap: core-bridge-dup-merge`. |
