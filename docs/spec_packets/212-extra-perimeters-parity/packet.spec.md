...
- Activation blockers: `TASK-328` is allocated by the approved plan but does not yet exist in `docs/07_implementation_status.md` (re-derived at preflight 2026-08-28: `TASK-328` is absent from `docs/07_implementation_status.md`, whose highest live ID is `TASK-507`; `TASK-328` is reserved for this packet by row #7 of `docs/specs/deviation-remediation-206-212-plan.md` and is claimed by no other plan; the neighbouring `TASK-320`/`TASK-321` are claimed by `docs/specs/struct-literal-churn-gate-plan.md`). Step 4 adds the row; this is not a blocker, it is scoped work.
...
- **AC-4. Given** `arachne-perimeters.toml`, **when** its `[config.schema.extra_perimeters]` block is inspected, **then** the block exists and carries exactly the four value lines `type = "int"`, `default = 0`, `min = 0`, `max = 10` — the same four the classic manifest's block carries (the classic side is asserted too, so the command fails if either drifts). The section-header anchor tolerates a trailing carriage return because `arachne-perimeters.toml` is committed with CRLF line endings while `classic-perimeters.toml` is LF; the value-line regex needs no such tolerance since its trailing whitespace class already covers it. | `test "$(rg -A6 -N '^\[config\.schema\.extra_perimeters\]\r?$' modules/core-modules/arachne-perimeters/arachne-perimeters.toml | rg -c '^\s*(type\s*=\s*"int"|default\s*=\s*0|min\s*=\s*0|max\s*=\s*10)\s*$')" = "4" && test "$(rg -A6 -N '^\[config\.schema\.extra_perimeters\]\r?$' modules/core-modules/classic-perimeters/classic-perimeters.toml | rg -c '^\s*(type\s*=\s*"int"|default\s*=\s*0|min\s*=\s*0|max\s*=\s*10)\s*$')" = "4" && echo PASS`
...
- `docs/15_config_keys_reference.md` - GENERATED tables; never hand-edit. Regenerate via `cargo xtask gen-config-docs` (`cargo xtask check-deviations --check` only VERIFIES these sections and regenerates doc 07's Open Deviation Map). Read only the `arachne-perimeters` / `classic-perimeters` owner rows around `extra_perimeters`; do not read in full.
