...
This packet is the second of three in the bridge-parity sequence (D3: internal-first → false-site gating → external orientation). It closes the false-site/classification gap (F1) by porting canonical's unsupported-span test — bridge area minus anchor areas derived from lower-layer slices/expansion zones — and demotes the existing mesh-validity filter to at most a cheap pre-filter. It builds on top of 233's seam: after both land, false-site suppression and internal-bridge construction coexist.

## In Scope

- A new host-side function `gate_bridge_areas_by_unsupported_span` in `crates/slicer-core/src/algos/prepass_slice.rs` that computes `bridge area − lower-layer anchor areas` (the canonical unsupported-span test), with expansion-zone growth of the anchors mirroring `LayerRegion::process_external_surfaces` (`expansion_step = scaled(0.1)`, up to 5 steps).
- Wiring the gating into the post-slice `PrePass::ShellClassification` host built-in (`commit_shell_classification_builtin`, `crates/slicer-runtime/src/slice_postprocess_prepass.rs`), reading the lower-layer slices from the already-committed `SurfaceClassificationIR.prev_layer_boundaries` (packet 193) or the committed `SliceIR` — no new scheduler data dependency (see Q3 resolution in `design.md`).
...
- Blast-radius fallout: updating `crates/slicer-runtime/tests/unit/bridge_detector_tdd.rs` (which calls `assemble_bridge_areas` directly and asserts non-empty `bridge_areas`) and any golden/parity baseline asserting the current flooded behaviour.
...
- Coverage/anchoring parity (F4) — its primitives (`construct_anchored_polygon`, expansion-zone growth constants) are delivered in 233/235; this packet only consumes the expansion-zone anchor derivation needed for the span test.
...
- Model oracle (AC-5): `resources/overhang.obj` is a two-level model — a solid base (vertices at Z≈17.0) and an overhanging top (vertices at Z≈23.3). The lowest layer (base) has no lower layer, and the second-lowest layer's lower layer is the solid base, so both must carry zero Bridge-type extrusion; the overhang top carries the known unsupported site. The exact Z values are confirmed at implementation's first reslice (Step 2) and recorded in the AC-5 assertion; the structural assertions (first/second layer zero bridge, site exists, no flooding) are the falsifiable contract.
...
- The gating function must be pure (no blackboard access): it takes `&mut SlicedRegion` and `&[ExPolygon]` lower-layer slices, so it is unit-testable in `slicer-core` without a running host.
