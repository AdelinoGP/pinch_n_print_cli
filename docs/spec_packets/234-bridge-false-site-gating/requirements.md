...
- Blast-radius fallout: `crates/slicer-runtime/tests/unit/bridge_detector_tdd.rs` (which calls `assemble_bridge_areas` directly and asserts non-empty `bridge_areas`) — **outcome (2026-08-22): no edit needed**; the two non-empty call sites still pass because the gate runs post-slice. The file's 3 pre-existing failures at HEAD are stash-verified as not this packet's and are out of scope. Any golden/parity baseline asserting the current flooded behaviour was enumerated and found to use self-constructed fixtures (unaffected).
