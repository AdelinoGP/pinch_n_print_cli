...
- Expansion-zone anchor growth mirrors canonical `expansion_step = scaled(0.1)` (0.1 mm) up to `max_nr_expansion_steps = 5`; the `expansion_margin_mm` already on `BridgeRegion` is in mm and must be converted via `mm_to_units()` before polygon offset. The `prev_layer_boundaries` polygons are already in 100 nm units (host-only aggregation).

## Code Change Surface

- Selected approach: keep `assemble_bridge_areas` stamping candidates during `PrePass::Slice` (its mesh-derived footprint intersection is unchanged), and add a pure `gate_bridge_areas_by_unsupported_span(region: &mut SlicedRegion, lower_layer_slices: &[ExPolygon])` that subtracts the grown lower-layer anchor areas from `region.bridge_areas`. Invoke the gate from `commit_shell_classification_builtin` (`crates/slicer-runtime/src/slice_postprocess_prepass.rs`) after `PrePass::OverhangAnnotation` has populated `prev_layer_boundaries`, keyed by the region's global layer index. The mesh-validity filter (`BridgeRegion.is_valid`) stays as a cheap pre-filter inside `assemble_bridge_areas` (already present at HEAD).
- Exact functions, traits, manifests, tests, and fixtures:
  - `crates/slicer-core/src/algos/prepass_slice.rs` — add `gate_bridge_areas_by_unsupported_span`; optionally factor the anchor-growth (expansion-zone) helper `grow_anchor_areas(lower_layer_slices, expansion_step_units, max_steps)`.
  - `crates/slicer-runtime/src/slice_postprocess_prepass.rs` — `commit_shell_classification_builtin` gains a post-annotation pass that, for each region, looks up `prev_layer_boundaries.get(&global_layer_index)` and calls the gate. The map is ALREADY previous-layer contours keyed by the CURRENT global layer index — the existing consumer `crates/slicer-wasm-host/src/marshal/in_.rs` uses `get(&global_layer_index)`, and `docs/04_host_scheduler.md` documents the same contract. Missing/empty value = first layer (no lower layer): canonical demotes `stBottomBridge && lower_layer == nullptr` to `stBottom`, so the gate removes the candidate from `bridge_areas` (it becomes bottom surface via the existing bottom classification).
...
- `crates/slicer-core/src/algos/prepass_slice.rs` - role: primary change surface; expected change: add `gate_bridge_areas_by_unsupported_span` + anchor-growth helper.
...
- `crates/slicer-core/tests/bridge_false_site_gating_tdd.rs` - role: net-new test home; expected change: AC-1/AC-2/AC-N1/AC-N2.
- `crates/slicer-core/Cargo.toml` - role: register the net-new test target; expected change: `[[test]]` entry.
- `crates/slicer-runtime/tests/unit/bridge_detector_tdd.rs` - role: blast-radius fallout; expected change: update `assemble_bridge_areas` assertions.
...
- Question: how `commit_overhang_annotation_builtin` populates `prev_layer_boundaries` — which layers get an entry, and whether a layer with an empty previous-layer contour set is stored as an empty `Vec` or omitted (the gate must treat both as "no lower layer"); scope: `crates/slicer-runtime/src/`; return: `LOCATIONS`; purpose: Step 2 wiring. (The keying is already confirmed: current global layer index, per `crates/slicer-wasm-host/src/marshal/in_.rs`.)
...
- **Pre-filter decision (recorded):** KEEP `BridgeRegion.is_valid` (min-length + anchor-width pass/fail) as a cheap pre-filter. Rationale: already present at HEAD, costs one boolean check per candidate, and rejects facet clusters that can never be valid bridges before the polygon-difference test runs. Measurement plan: after the gate lands, run AC-3/AC-5 with the pre-filter enabled and disabled; if the pre-filter changes zero output (the gate alone rejects every candidate the pre-filter rejects), discard it in a follow-up step and record the result. If it changes output, keep it and record the delta.
...
- Changing F1 flips baseline-dependent tests: `crates/slicer-runtime/tests/unit/bridge_detector_tdd.rs` asserts non-empty `bridge_areas` from `assemble_bridge_areas`; those assertions must be re-scoped to the ungated stamping (or given lower-layer slices).
