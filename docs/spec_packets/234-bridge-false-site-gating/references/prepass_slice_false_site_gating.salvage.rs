...
pub fn assemble_bridge_areas(
    ...
    // `bridge_regions` stores mesh-level XY candidates. Only attach one to a
    // layer after `classify_region_surfaces` has confirmed that this layer's
    // cross-section intersects the bridge facets. Orca's `BridgeDetector`
    // operates on the current surface and lower-layer slices, so a bridge
    // footprint must not become a layer-invariant material region.
    if !region.is_bridge {
        return;
    }
...
fn execute_prepass_slice_single_layer_impl(
    ...
    for active in &layer.active_regions {
        ...
        // Orca assigns bridge flow to the unsupported part of a bottom
        // surface, not to every downward-facing facet that happens to cross
        // the current Z plane. The batch cache has the preceding raw slice;
        // an absent predecessor means this is the object's first layer and
        // therefore has no bridge candidate.
        let unsupported_region = cache.map(|c| {
            c.prev_raw_polygons
                .and_then(|previous| previous.get(&active.object_id))
                .map(|previous| difference(&raw_polygons, previous))
                .unwrap_or_default()
        });
        ...
        if let Some(unsupported) = unsupported_region.as_deref() {
            // Mesh analysis supplies the bridge candidate and orientation;
            // the current-vs-lower slice difference supplies the actual
            // unsupported material. This is the layer-local support check in
            // Orca's `LayerRegion::process_external_surfaces`.
            sliced_region.bridge_areas = intersection(&sliced_region.bridge_areas, unsupported);
            if sliced_region.bridge_areas.is_empty() {
                sliced_region.is_bridge = false;
                sliced_region.bridge_orientation_deg = 0.0;
            }
        }
        ...
        // Orca's fill stage explicitly requires `layer.id() > 0` for bridge
        // material. Keep the same guard at the SliceIR seam so callers that
        // execute a single uncached first layer cannot inherit mesh-level
        // bridge classification.
        if layer.index == 0 {
            sliced_region.is_bridge = false;
            sliced_region.bridge_areas.clear();
            sliced_region.bridge_orientation_deg = 0.0;
        }
