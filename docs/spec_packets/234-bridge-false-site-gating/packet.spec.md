...
- Depends on: `docs/spec_packets/233-internal-bridge-over-infill/` (status draft). Its net-new symbols — `ExtrusionRole::InternalBridgeInfill` (`crates/slicer-ir/src/slice_ir.rs`), WIT `extrusion-role` variant `internal-bridge-infill`, module `bridge_over_infill` + `determine_bridging_angle`/`construct_anchored_polygon` (`crates/slicer-core/src/algos/bridge_over_infill.rs`), `bridge_extrusion_spacing`/`BRIDGE_EXTRA_SPACING` (`crates/slicer-core/src/flow.rs`), config keys `dont_filter_internal_bridges`/`enable_extra_bridge_layer`/`internal_bridge_angle` — are PLANNED exports, referenced as prerequisites, never as existing HEAD code.
...
- **AC-3. Given** `resources/bridge.obj` sliced with the core modules, **when** the emitted G-code is parsed with M83 relative-E semantics (positive E delta on an XY move = extrusion), keyed by Z (never layer index), with `;TYPE:` carried across layer changes, **then** at least one layer carries Bridge-type extrusion (site exists) AND the number of Bridge-type layers is strictly less than the total layer count (no flooding — false sites suppressed). | `cargo run --bin pnp_cli --release -- slice --model resources/bridge.obj --output target/bridge_false_site.gcode --module-dir modules/core-modules && python3 -c "import sys,re; z=None; t=''; bz=set(); az=set()\nfor l in open(sys.argv[1]):\n l=l.strip()\n if l.startswith(';TYPE:'): t=l[6:].strip()\n m=re.search(r'\\bZ(-?\\d+\\.?\\d*)',l)\n if m: z=float(m.group(1)); az.add(z)\n m=re.search(r'\\bE(-?\\d+\\.?\\d*)',l)\n if m and float(m.group(1))>0 and z is not None and t and 'ridge' in t: bz.add(z)\nassert len(bz)>=1,'no bridge site'\nassert len(bz)<len(az),f'flooding {len(bz)}/{len(az)}'\nprint(f'bridge_layers={len(bz)}/{len(az)} z={sorted(bz)}')" target/bridge_false_site.gcode`
...
- **AC-5. Given** `resources/overhang.obj` (solid base at Z≈17, overhanging top at Z≈23.3) sliced with the core modules, **when** the emitted G-code is parsed with M83 relative-E semantics (positive E delta on an XY move = extrusion), keyed by Z (never layer index), with `;TYPE:` carried across layer changes, **then** the lowest layer carries ZERO Bridge-type extrusion (first layer, no lower layer → demoted to bottom), the second-lowest layer carries ZERO Bridge-type extrusion (its lower layer is the solid base — the I1 solid-underneath case), at least one layer carries Bridge-type extrusion (the known unsupported overhang span), and Bridge-type layers are a strict subset of all layers (no flooding). | `cargo run --bin pnp_cli --release -- slice --model resources/overhang.obj --output target/overhang_false_site.gcode --module-dir modules/core-modules && python3 -c "import sys,re; z=None; t=''; bz=set(); az=set()\nfor l in open(sys.argv[1]):\n l=l.strip()\n if l.startswith(';TYPE:'): t=l[6:].strip()\n m=re.search(r'\\bZ(-?\\d+\\.?\\d*)',l)\n if m: z=float(m.group(1)); az.add(z)\n m=re.search(r'\\bE(-?\\d+\\.?\\d*)',l)\n if m and float(m.group(1))>0 and z is not None and t and 'ridge' in t: bz.add(z)\nsz=sorted(az)\nassert len(bz)>=1,'no bridge site'\nassert len(bz)<len(az),f'flooding {len(bz)}/{len(az)}'\nassert sz[0] not in bz,f'first layer {sz[0]} carries bridge'\nassert sz[1] not in bz,f'second layer {sz[1]} carries bridge (solid lower layer)'\nprint(f'bridge_layers={len(bz)}/{len(az)} z={sorted(bz)}')" target/overhang_false_site.gcode`
...
- **AC-N2. Given** the gating is bypassed (the ungated `assemble_bridge_areas` output is fed directly to partition), **when** the pipeline runs, **then** the test FAILS — ungated candidates cannot silently return; the gated and ungated outputs must differ on a solid-underneath span. | `cargo test -p slicer-core --features host-algos --test bridge_false_site_gating_tdd -- ungated_candidates_cannot_silently_return --nocapture`
...
- **`none`** - this packet changes host-side classification behaviour only: it reuses the existing `SurfaceClassificationIR.prev_layer_boundaries` field and `bridge_areas`/`SlicedRegion` fields, adds no IR/WIT/scheduler/claim/manifest/host-service/SDK contract, and introduces no new stage (the gating runs inside the existing `PrePass::ShellClassification`). No schema version bump, no `STAGE_ORDER` change.
...
- `OrcaSlicerDocumented/src/libslic3r/BridgeDetector.cpp` — `BridgeDetector::unsupported_edges` (`diff_pl(to_polylines(bridge_expolygon), grown_lower)`), the concrete "bridge area minus grown lower-layer anchors" geometry.
