...
- OrcaSlicer refs:
  - `OrcaSlicerDocumented/src/libslic3r/PerimeterGenerator.cpp` - delegated `PerimeterGenerator::process_classic and PerimeterGenerator::process_arachne` parity confirmation only.
- Verification:
...
- Precondition: Step 1 and its verification commands pass; no whole-core closure or Packet Queue implementation edit is justified. AC-7 compares the working tree Packet Queue directly with the section in `HEAD`, so it rejects a Queue edit made at any point in this packet.
...
- Blast-radius discipline:
  - No code/schema blast radius. The six-column row parser rejects malformed cells and anchors extraction between `## 7. Ledger` and the next `##` heading so queue text cannot satisfy the criterion; AC-7 additionally compares the Packet Queue with `HEAD`, so a Queue edit fails rather than merely preserving its table shape.
- Expected sub-agent dispatches:
