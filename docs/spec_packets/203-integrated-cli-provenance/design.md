...
- ~~[FWD] Did 201 leave `prepare_prepass_context` integrated-aware or external-only (201's own [FWD] + FACT-dispatch outcome)? Either answer keeps SupportPreview out of scope here; record which one held in the Step 1 commit message.~~ — **CLOSED by 201's implementation (2026-08-10): `prepare_prepass_context` is integrated-aware** — both live-loader call sites in `run.rs` switched to `load_live_modules_for_plan_with_integrated` (prepass site passes `profile: false`). SupportPreview stays out of scope; the disable-seam gap this creates is tracked in `requirements.md` §Out of Scope (KNOWN GAP).
