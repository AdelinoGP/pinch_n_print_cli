...
- Blast-radius discipline (mandatory — this step adds a struct field):
  ...
  - Verified against the tree at authoring (2026-08-07) — **13 construction sites across 11 files**; re-derive both numbers before editing. **Measured 2026-08-10: 14 sites across 12 files, of which 11 already end in `..Default::default()` or an FRU base and need no edit** (the parallel 194–199 struct-literal plan converted them). The sites that need the new field are exactly four: the full literal in `crates/pnp-cli/src/main.rs:514` (a primary edit above), the full literals in `tests/unit/profile_flag_tdd.rs:41` and `tests/unit/cancel_flag_tdd.rs:32`, and the `impl Default for SliceRunOptions` body in `crates/slicer-runtime/src/run.rs` — a `Self { … }` full literal that the `rg -n 'SliceRunOptions \{' crates/` re-derive grep does **not** match (it matches `SliceRunOptions {` only, not `Self {`); add that site by hand. The 11 FRU sites are: `tests/visual_debug_agent_overhead_tdd.rs`, `tests/unit/slice_run_options_default_tdd.rs`, `tests/e2e/mm_real_fixture_gcode_tdd.rs`, `tests/e2e/run_slice_api_tdd.rs`, `tests/executor/cube_4color_sparse_infill_per_painted_region_tdd.rs`, `tests/executor/cube_4color_phase5_tdd.rs`, `tests/executor/cube_4color_ironing_per_painted_top_color_tdd.rs`, `tests/executor/cube_4color_gcode_output_tdd.rs` (3 sites), `tests/executor/cube_4color_arachne.rs`.
...
- Test-authoring constraints (all mandatory, from `design.md` §Architecture Constraints):
  ...
  - AC-N2 asserts the shadow message **verbatim**: `external module com.core.classic-perimeters shadows integrated module com.core.classic-perimeters`. That string is 201's contract (ADR-0056's consequence bullet verbatim, no inner quoting) — assert it, never restate or reformat it in product code.
