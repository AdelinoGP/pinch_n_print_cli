...
- Depends on: packet 01, `docs/spec_packets/config-scope-resolution_01_config-schema-registry/`, currently `active`; this remains a forward dependency until its exact registry API lands.
...
- **`none`** - implementation changes only test/development surfaces and does not change an IR, WIT, scheduler, claim, manifest, host-service, SDK, or CLI contract.
