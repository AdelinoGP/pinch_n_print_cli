...
This packet groups one new backlog row. Its numeric `TASK-` id is a mutable ledger fact and is deliberately not frozen here or in `packet.spec.md`. Step 10 re-derived it with `rg -o 'TASK-[0-9]+' docs/07_implementation_status.md | sort -uV | tail -1` (highest at that moment: `TASK-530`), took the next free number, and registered **`TASK-560`** in `docs/07_implementation_status.md`; the id now appears in `packet.spec.md`, `requirements.md`, and this file.
