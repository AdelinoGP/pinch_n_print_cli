...
- Files allowed to read, with ranges when over 300 lines: `tmp/alloc-bench/run_bench.ps1` parameter block (lines 1-30 only); `docs/specs/perf-vs-orca/evidence/perf-next/EXPERIMENT.md` and `MEASUREMENTS.md` delegated summaries; `docs/22_test_quality.md` heading style only (for doc numbering/format).
