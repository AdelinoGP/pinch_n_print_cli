...
Make every guest `ConfigView` (load-time binding and per-region layer dispatch) a registry-complete projection of resolved config, make `CONFIG_BLOCK` emission registry-driven over the full registry, remove all 89 load-bearing guest config-literal fallbacks, and enforce warn-then-drop handling for genuinely undeclared keys only after every host-consumed key is registered and the no-drop oracle passes.
...
- the exact 89-site guest cleanup, including the manifest declarations it needs;
- the no-drop end-to-end oracle;
- the packet-03 warn-to-drop transition.

It does not add eligibility policy, layer ranges, modifier typing, aliases, or Phase-C automatic-value expansion.
