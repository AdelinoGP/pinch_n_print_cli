...
- Provenance-aware duplicate-id diagnostic: when the dedup loser is integrated and the winner external, the warning message becomes `external module X shadows integrated module X` (ADR-0056's consequence bullet verbatim, no inner quoting); all other pairings keep the existing generic text.
