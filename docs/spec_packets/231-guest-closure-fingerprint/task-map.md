...
| --- | --- | --- | --- | --- | --- | --- |
| `TASK-342` | `Step 1` | `docs/specs/guest-freshness-artifact-verification-plan.md` (C2, C8, R5-6) | `xtask/src/build_guests.rs` (`#[cfg(test)] mod tests`) | none | `S` | Ten red tests bind the closure API's shape before it exists; the compile failure naming `guest_closure_input_paths` is the proof they are not vacuous |
