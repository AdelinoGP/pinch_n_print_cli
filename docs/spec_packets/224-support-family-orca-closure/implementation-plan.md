...
## Execution Rules
- **ACTIVATION GATE FIRST (added 2026-09-03).** Do not run Step 0 until every packet listed in `packet.spec.md` §Activation Gate is `implemented`, re-derived with the `grep '^status:'` loop recorded there. At authoring time `240a-support-raft-substrate`, `240b-support-raft-module`, and `241-support-agg-rasterizer` were still `draft`. Executing before the gate passes produces evidence that is stale on arrival.
- **All Step status lines below are historical.** They describe a prior execution pass against a tree that has since moved. Their commit SHAs are marked † and **do not resolve** — see `design.md` §Orphaned commit references. Treat a status line as a record of what was once done, never as proof that it still holds; re-verify each step's exit condition against HEAD when the packet is re-run.
- **The 2026-08-20 Orca inspection checklist is superseded and MUST NOT be reused.** AC-2, AC-3 half (b) and AC-6 half (b) require a checklist **regenerated at execution time against HEAD** (see the AC-2 amendment in `packet.spec.md`).
...
- Every canonical feature gap discovered mid-flight is **registered and routed** in `docs/specs/support-parity-gap-register.md`, never implemented here. **Destinations corrected 2026-09-03:** `docs/spec_packets/stubs/` does not exist and 225/226/227 are unrelated packets; the real owners are `238a-support-pattern-config-keys`, `238c-support-renderer-flow-interfaces`, `239a`/`239b`/`239c`/`239d`, `240a-support-raft-substrate` + `240b-support-raft-module`, and `241-support-agg-rasterizer`. Re-derive each owner from the register's destination column. The register is **shared with `242-support-family-orca-closure`** — re-read before writing.
...
- Files explicitly out of bounds: `crates/slicer-schema/wit/**`; interface **pattern** generation (`238a-support-pattern-config-keys`); `support_bottom_z_distance` (`238a-support-pattern-config-keys`).
...
- Postcondition (**strengthened 2026-09-03; gated by AC-7**): for each of the four modules, every manifest-declared key is either read by that module or listed as a dead key with its owning destination packet in `docs/specs/support-parity-gap-register.md`. Additionally: (i) the declared-but-unread set is **re-derived** with the command in `requirements.md` §In Scope, "Config-key reconciliation", not quoted from any document; (ii) the **inverted G-16 and G-22 rows are corrected** — both assert a key is *undeclared* when the manifest declares it; (iii) the **`tree-support` `line_width` / `support_line_width` mismatch** is fixed or explicitly recorded with an owner (the manifest declares `line_width`; `modules/core-modules/tree-support/src/lib.rs` reads `support_line_width`, which is undeclared there, so it always takes its `1.125 * nozzle_diameter` default and the declared key is dead); (iv) the exit report lists an **empty** unreconciled set.
- **Shared-file caution:** `docs/specs/support-parity-gap-register.md` is also written by `242-support-family-orca-closure`. Re-read it immediately before writing; do not assume exclusive ownership.
...
~~**Status 2026-08-18 — DONE (`4d1848eb`†).** Config keys reconciled across the four support modules.~~ **RE-OPENED 2026-09-03: the reconciliation is not met.** Five declared-but-unread keys exist and only one (`support_base_pattern_spacing`, G-03) is recorded; two register rows (G-16, G-22) assert the *opposite* of what the manifests say; and the `tree-support` `line_width` / `support_line_width` mismatch is an unrecorded live defect. See the strengthened postcondition above and AC-7.
...
**Added 2026-09-03 — three closure tests assert less than their AC requires. Fixing them is part of this step.** All three live in `crates/slicer-runtime/tests/integration/support_family_closure.rs`.

1. **`invalid_geometry_fails` never asserts the drop (AC-N1).** It discards the retained plan — `let (_, diagnostics) = try_aggregate_support_plan_irs_with_diagnostics(...)` — and asserts only on the diagnostic, so the AC's central requirement (the body is **dropped**) is untested. Its inputs are also pre-labelled: one entry arrives with `decline_reason: Some(Blocked)` set by the test, and `invalid_body` carries `roles: Vec::new()`, so aggregation can reject on emptiness or echo the caller's label without judging anything. **Required:** bind the returned plan; assert the invalid body is **absent** from it; assert its demand is present and carries an unmet marker naming the body and a reason; and supply inputs with a **non-empty** `roles` list and **no** pre-set `decline_reason`, so aggregation must judge the geometry itself.
2. **The decline-reason assertion is unreachable (AC-3 half (a)).** `pnp_support_evidence` hard-errors on the **first** declined entry, so `assert_attribution_and_decline_reasons` can never observe a decline and "a decline reason on every unmet demand" cannot execute. Separately, `object_id` and `region_id` appear only inside error-message formatting and are never asserted. **Required:** make the declined-entry path reachable — **collect** declines and evaluate them rather than hard-erroring on the first — and **assert** non-empty object/region identity rather than printing it.
3. **The forbidden-reference scanner implements half its contract (AC-6 half (a)).** `collect_test_files` **excludes its own file**, so a violation added to `support_family_closure.rs` — the likeliest place for one — is invisible; and it scans only for the three Orca G-code **path literals**, with **no scan for Orca-derived numeric constants**, which AC-6(a) explicitly demands. **Required:** include the scanner's own file in the sweep, and require every hardcoded canonical-provenance constant in the closure tests to either cite its canonical source (**file + function, never a line number**) at the constant or be replaced by a derived value. **Known instance to dispose of:** `interface_band_counts_match_canonical_structure` hardcodes `count != 3` on nothing but a doc-comment claim of canonical provenance — cite `Slic3r::generate_interface_layers` (`SupportCommon.cpp`) at the constant, or derive the band count from `support_interface_top_layers` / `support_interface_bottom_layers`.
...
- Postcondition (**amended 2026-09-03**): two distinct request fixtures (`tmp/visual-debug-support-family-tree.json`, `tmp/visual-debug-support-family-normal.json`) replace the single request rendered twice; each family and its Orca counterpart are rendered at the same physical heights; and a **NEW, separately dated** `## Orca Inspection Checklist (regenerated <YYYY-MM-DD>)` section is written into `design.md` **below** the 2026-08-20 record, with per-axis inspected verdicts (termination, coverage, collision freedom, interface placement/count, independent heights), each naming its layer and tap.
- **The 2026-08-20 checklist is a superseded historical record.** Do not edit it in place, do not refresh it selectively, and do not cite it for AC-2, AC-3(b) or AC-6(b). At least one of its verdicts is already known wrong (traditional interface count, closed by 238c).
- **The regenerated checklist must stand alone.** The four render bundles (`target/vd-support-family-*`, `target/vd-orca-*-compare`), the four `tmp/*.json` requests, and the two `tmp/SupportTest_*_Orca.gcode` references are untracked/gitignored and cannot be re-opened later. Inline the request contents (or their decisive fields), the exact layer indices with their physical Z, the Orca profile used, and a description of what was seen precise enough to audit each verdict without the bundles.
...
- Postcondition: `packet.spec.md` AC-2/AC-3/AC-6 amended with the amendment recorded verbatim in place (the style used for AC-N2), pointing at the two per-family commands and the inspection-checklist gate, AC-2's fixture path corrected from `tmp/SupportTest.stl` to the authoritative `crates/slicer-runtime/tests/fixtures/support-family/SupportTest.stl`, and the stale claim about `missing_fixture_is_blocking` corrected to match Step 5's actual deletion; `docs/specs/support-parity-gap-register.md` exists listing every routed gap with its owning packet; every routed gap names a **real, existing** owning packet (**corrected 2026-09-03**: `241-support-agg-rasterizer` for the AGG rasterizer / `support_area_algorithm`; `239a`/`239b`/`239c`/`239d` for independent support-layer Z; `238a-support-pattern-config-keys` for base/interface patterns + `support_expansion` + `support_bottom_z_distance` + dead config keys; `240a-support-raft-substrate` + `240b-support-raft-module` for raft; `238c-support-renderer-flow-interfaces` for renderer/flow/interfaces and `needs_support` eligibility follow-through — `docs/spec_packets/stubs/` never existed and no stub may be created there); `docs/07_implementation_status.md` carries the `TASK-335` row and the follow-on rows.
...
**Status 2026-08-20 — DONE.** `needs_support` eligibility gap filed (G-17, decision 2) and the vacuous `enforcer_overrides_needs_support_false` deleted from `modules/core-modules/tree-support/tests/enforcer_blocker_tdd.rs` (tree-support 26/26). Gap register updated: G-01 marked implemented, G-17/G-18 added, destinations renamed at the time to five unnumbered stubs under `docs/spec_packets/stubs/` — **that decision did not survive: the directory does not exist and the register's destination column now names real packets (238a/238c/239a-d/240a/240b/241). Re-derive owners from the register, not from this line.** Stale status lines corrected in `packet.spec.md` and `implementation-plan.md` (Steps 2/3a/3b/6). `docs/07_implementation_status.md` carries the TASK-335 state note and the five stub rows.
...
- Postcondition (**amended 2026-09-03**): the workspace gates pass, and the golden obligation is **discharged by supersession** rather than by regeneration. The benchy golden was **deleted, not renamed**: `resources/golden/` no longer exists, and commit `ac9466c6` (reachable from HEAD) replaced the self-captured tripwire with algorithmic invariants — which better serves this packet's own rule that a self-capture is never parity evidence. **The regeneration and provenance-header obligations are therefore dropped.** What survives is the **rename**, applied to the surviving test file: `modules/core-modules/tree-support-planner/tests/orca_parity_tdd.rs` still carries the `orca_parity` name while asserting algorithmic invariants, and must be renamed to a name that does not claim parity.
- Re-verify `resources/golden/`'s absence before acting; if a golden has reappeared, the original obligation applies to it.
- Files allowed to read: the golden fixture and its owning test.
- Files allowed to edit (3): `modules/core-modules/tree-support-planner/tests/orca_parity_tdd.rs` (rename off `orca_parity`); the benchy golden fixture **if one exists** (it does not at authoring time); `docs/spec_packets/224-support-family-orca-closure/design.md` (closure note).
...
- Exit condition: all three gates pass and no surviving artefact claims Orca parity by name — specifically, no test file or fixture in the tree-support-planner suite is still called `orca_parity` while asserting self-captured or algorithmic invariants.
...
- **The Activation Gate in `packet.spec.md` passed before Step 0 ran**, re-derived at the time of the run.
...
- Every routed gap (base/interface patterns, `support_expansion`, `support_bottom_z_distance`, raft, independent support-layer Z, the AGG rasterizer, the dead raft/`support_base_pattern` keys, `needs_support` eligibility) appears in `docs/specs/support-parity-gap-register.md` against a **real, existing** owning packet — 238a / 238c / 239a-d / 240a / 240b / 241 as applicable, **never** a path under `docs/spec_packets/stubs/`. The roof/floor layer-count gap (G-18) is **closed by 238c** and is not carried forward as open.
- `packet.spec.md` AC-2/AC-3/AC-6/AC-N1 amended in place with the amendment recorded verbatim; **AC-7 (config-key reconciliation) satisfied with an empty unreconciled set**; every remaining AC command returns PASS as written.
- Golden obligation discharged: no surviving artefact claims Orca parity by name (see Step 8; the self-captured golden was deleted rather than regenerated).
...
- Inspect matched-height PNGs and manifest provenance side by side with the Orca renders — not just file existence — and record them in the **regenerated** checklist. The 2026-08-20 checklist may not be cited.
