...
- Add WIT `order-lock: option<u64>` to `record extrusion-path3d`
  ...
  macro read these files directly; the WIT files carry no versioned package path, so no WIT version
  tax.
...
- Add the SDK local-tag allocator type (`OrderLockAllocator`) and the host remap function
  (`remap_order_locks_to_global`), wired at the output boundaries (Infill/InfillPostProcess commit,
  finalization merge).
...
- Update `docs/02_ir_schemas.md` §"IR 10 — LayerCollectionIR".
...
- `docs/02_ir_schemas.md` - ~1650 lines; direct range reads only: §"IR 10 — LayerCollectionIR"
...
- Positive: `AC-1` (schema 1.4.0), `AC-2` (carrier end-to-end), `AC-3` (all-`None` neutrality),
  `AC-4` (proposal enforcement), `AC-5` (remap), `AC-6` (allocator), `AC-7` (remap wired at the
  output boundary).
- Negative: `AC-N1` (InfillPostProcess preserves locked block), `AC-N2` (finalization rejects
  geometry change), `AC-N3` (tool rotation never splits a locked block).
...
- Step 3 (enforcement + remap wiring) depends on Step 1 (the field must exist to validate) and Step 2 (the remap
