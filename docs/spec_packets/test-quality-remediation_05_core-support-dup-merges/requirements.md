...
- The §7 ledger edit occurs only after both merge steps pass and may touch only the `core` row; the no-queue-edit boundary is an explicit diff-reviewed scope condition, while Packet Queue state remains orchestrator-owned.

## Context Discipline Notes

- Read only the named windows of `crates/slicer-core/tests/support_overhang_detection_tdd.rs` and `crates/slicer-core/src/algos/support_geometry.rs`; inventories and survivor mappings above avoid exploratory whole-crate reads.
