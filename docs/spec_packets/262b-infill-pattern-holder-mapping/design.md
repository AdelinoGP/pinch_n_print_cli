...
- **DIV-8 — the narrow split is module-side and uses only canonical's geometric-core branch.** Two divergences in one place, both forced by the same architectural fact. (a) Canonical splits `stInternalSolid` **surfaces** inside `group_fills` and gives the narrow half its own `SurfaceFill` with `pattern = ipConcentricInternal`; this port's claim seam is per region, so a per-polygon pattern override cannot be a second holder, and the split lives inside whichever module holds `claim:top-fill`, branching on the role it is about to emit. *Amended 2026-09-04 (ticket 35 follow-up):* the original authoring read "this port has no internal-solid surface domain at all — `SlicedRegion::internal_solid_fill` is a marker subset of `top_solid_fill`, and the `InternalSolidInfill` role is decided per region by shell-index stamps". That is no longer true of the tree: the fill-stage partition now carves a real fillable `internal_solid_fill` bucket (five-way precedence, emitted as `InternalSolidInfill` by the `claim:top-fill` holders), so Step 7b's splitter operates on the partitioned bucket polygons rather than shell-index-selected top/bottom polygons. The seam decision itself survives — the claim seam is still per region, so the split is still module-side. (b) Canonical's `split_solid_surface` takes a **line-based** branch for the rectilinear/monotonic pattern family (rotate by the aligning angle, then reconnect across the scan direction with a `LinesDistancer`) and a geometric-core branch otherwise; this port applies the geometric-core branch to every pattern. Rationale: the core branch is the one that defines *what counts as narrow*, and it is pattern-independent; the line-based reconnection changes which marginal slivers survive, not whether narrow areas are rerouted. Recorded so a later packet can add the line-based branch without re-deciding the seam.
- **DIV-9 — `detect_narrow_internal_solid_infill` adopts canonical's `true` default, and this changes output.** Unlike DIV-1, where the port deliberately keeps its own pattern default, this key ships at canonical's own default. Consequence stated plainly: a region with a narrow internal-solid area emits concentric loops where it previously emitted scan lines, so parity baselines covering such geometry move in this packet. AC-N5 bounds the change — no narrow area, and depth-0 exposed surfaces, stay byte-identical. Rationale: the key exists to stop scan-line fill degenerating in slivers; shipping it off by default would leave the defect in place for every default print while claiming the key is live.
...
- `crates/slicer-sdk/src/narrow_solid.rs` (**new**) + its `mod` line in `crates/slicer-sdk/src/lib.rs`, and `crates/slicer-sdk/tests/narrow_solid_tdd.rs` (**new**) — the splitter and the concentric-loop generator (AC-13, AC-14).
- `modules/core-modules/rectilinear-infill/{rectilinear-infill.toml,src/lib.rs}` and `modules/core-modules/rectilinear-infill/tests/narrow_internal_solid_tdd.rs` (**new**) — the `detect_narrow_internal_solid_infill` table and the narrow branch on the `InternalSolidInfill` role only. **This is a carve-out from the read-only rule below**; it is the one edit this packet makes inside a module 262a owns, and it lands after 262a per the existing merge-order dependency.
- `modules/core-modules/monotonic-infill/{monotonic-infill.toml,src/lib.rs,tests/monotonic_infill_tdd.rs}` — the same table and branch on the module this packet creates.
- Three new guest `.wasm` artifacts, plus a rebuilt `rectilinear-infill` guest.
...
- `crates/slicer-sdk/src/host.rs` — `medial_axis`, `clip_polygons`, `offset_polygons` (the `narrow_solid` helpers build on the offset/clip primitives).
- `modules/core-modules/rectilinear-infill/src/lib.rs` §`solid_fill_role` — the per-region `top_shell_index` / `bottom_shell_index` ≥ 1 rule that decides when a solid path is `InternalSolidInfill`. This is the port's whole definition of "internal solid": there is no separate internal-solid polygon domain, which is why the narrow split is a module-side branch on the emitted role rather than a host-side surface split as in canonical `group_fills`.
...
- `modules/core-modules/{gyroid-infill,lightning-infill}/**` — read-only; their behaviour must not change (262a owns gyroid). `modules/core-modules/rectilinear-infill/**` is read-only **except** for the three files named in §Code Change Surface (manifest table, narrow branch, new test binary); every other file in that module, and every behaviour other than the `InternalSolidInfill` role's narrow split, is out of bounds. `modules/core-modules/infill-linker/**` is read-only **except** for the `GapFill` passthrough named in §Code Change Surface; `claim:infill-link` stays solely the linker's (ADR-0025).
