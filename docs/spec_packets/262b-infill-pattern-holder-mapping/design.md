...
- **DIV-8 — the narrow split is module-side and uses only canonical's geometric-core branch.** Two divergences in one place, both forced by the same architectural fact. (a) Canonical splits `stInternalSolid` **surfaces** inside `group_fills` and gives the narrow half its own `SurfaceFill` with `pattern = ipConcentricInternal`; this port has no internal-solid surface domain at all — `SlicedRegion::internal_solid_fill` is a marker subset of `top_solid_fill`, and the `InternalSolidInfill` role is decided per region by `top_shell_index` / `bottom_shell_index` ≥ 1 — and its claim seam is per region, so a per-polygon pattern override cannot be a second holder. The split therefore lives inside whichever module holds `claim:top-fill`, branching on the role it is about to emit. (b) Canonical's `split_solid_surface` takes a **line-based** branch for the rectilinear/monotonic pattern family (rotate by the aligning angle, then reconnect across the scan direction with a `LinesDistancer`) and a geometric-core branch otherwise; this port applies the geometric-core branch to every pattern. Rationale: the core branch is the one that defines *what counts as narrow*, and it is pattern-independent; the line-based reconnection changes which marginal slivers survive, not whether narrow areas are rerouted. Recorded so a later packet can add the line-based branch without re-deciding the seam.
