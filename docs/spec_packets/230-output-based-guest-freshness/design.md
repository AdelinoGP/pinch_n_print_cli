...
- `pub struct CheckContext { pub shared: FreshnessSnapshot, pub canonical: WorldModel, pub wasm_tools_version: String }` — built once per `check_command` invocation so canonical WIT is parsed once, not 42 times.
- `pub fn check_command(ws_root: &Path) -> CheckOutcome` — signature change.
