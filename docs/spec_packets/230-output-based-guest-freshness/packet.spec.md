...
- **AC-16. Given** the migration is complete, **when** the implementer times `cargo xtask build-guests --check` on this machine before and after the change, **then** both wall-clock figures and the exact timing command are recorded in this packet's `requirements.md` under the heading "Measured Freshness Timing", with no unmeasured figure quoted anywhere in the packet. | `rg -q '^## Measured Freshness Timing' docs/spec_packets/230-output-based-guest-freshness/requirements.md && rg -q 'measured 20[0-9][0-9]-[0-9][0-9]-[0-9][0-9]' docs/spec_packets/230-output-based-guest-freshness/requirements.md && echo PASS || echo FAIL`
...
The normative freshness-contract prose in `docs/03_wit_and_manifest.md`, `docs/05_module_sdk.md`, `CLAUDE.md` and the `wasm-staleness` snippet is deliberately **not** edited here; packet 232 owns that surface in one coherent pass, and splitting it across two packets would leave the docs internally inconsistent between them.
