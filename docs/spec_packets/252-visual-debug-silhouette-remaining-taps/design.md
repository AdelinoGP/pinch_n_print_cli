...
- Exact surface, per file:
  ...
  - Tests: extend `crates/slicer-runtime/tests/visual_debug_silhouette_tdd.rs` (AC-1..6, AC-N3..N5, plus the non-positive-height fail-closed guard from the packet-252 review), `crates/pnp-cli/tests/visual_debug_validation_tdd.rs` (AC-8, AC-N1 — retire the two arms of `silhouette_unsupported_taps_rejected_with_reasons`, keep its remaining arms passing), `crates/pnp-cli/tests/visual_debug_silhouette_bundle_tdd.rs` (AC-7, AC-N2 — the render-level tool test drives the real pipeline per `implementation-plan.md` Step 4).
