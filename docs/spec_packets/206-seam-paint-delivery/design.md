...
- **Selected approach.** Four coordinated changes:
  ...
  1. **Writer (`crates/slicer-core/src/algos/paint_segmentation/seam_annotations.rs`, new).** Two functions:
     ...
     - `pub(crate) fn stamp_seam_paint_annotations(mesh: &slicer_ir::MeshIR, layers: &mut [slicer_ir::SliceIR])` — builds, per seam semantic name (`"seam_enforcer"`, `"seam_blocker"`), one `slicer_ir::IndexedTriangleSet` from the painted facets and stroke triangles, exactly mirroring the `push_tri` accumulation in `execute_paint_segmentation`'s facet/stroke arms; then, per layer, projects each painted triangle onto XY and, for every layer whose z-band (`layer.z` ± `effective_layer_height/2`) overlaps the triangle's z-range, marks a region contour edge as painted when the same edge-midpoint containment test `build_modifier_segment_annotations` uses — `modifier_volumes::any_expolygon_contains_point` on the integer midpoint of `points[k]`/`points[(k+1) % n]` — reports containment inside the triangle's XY projection; then for every layer, every region, and every region polygon, emits one `Option<PaintValue>` per contour point → `Some(PaintValue::Flag(true))` inside, `None` outside. (`crate::slice_mesh_ex` was not viable on the AC-1 fixture: it slices at exact z-planes and chains intersection lines into closed ExPolygons, which the fixture's sparse surface triangles — 2709, spanning z 0..25 — never form.) Merges into (never replaces) the region's existing `segment_annotations` map, so modifier-volume annotations survive. Merges into (never replaces) the region's existing `segment_annotations` map, so modifier-volume annotations survive.
