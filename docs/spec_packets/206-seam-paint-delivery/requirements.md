...
- Wiring the discriminator into the planner's production path (`region_candidates` / `choose_region_candidate`, `modules/core-modules/seam-planner-default/src/lib.rs`). Without it the exact-match fix is inert: `src/visibility.rs` carries `#[allow(dead_code)]` and had zero production callers, and since the default `seam_mode = "aligned"` routes `seam-placer` through the planner's `resolved_seam`, seam paint could not move the seam on default settings. Pinned by AC-10 against the production entry point, not against a `#[path]`-included copy of the module.
...
- Positive: `AC-1` through `AC-11`. Refinements not restated in their Given/When/Then text:
  - `AC-11` is the footprint criterion, and it must run on a synthetic mesh. The AC-1 fixture's paint covers the object's full XY and Z extent (measured: 2709 triangles over exactly the mesh bounds), so it can prove delivery but not precision — on it, stamping every contour vertex is the correct answer.
  - `AC-10` is the anti-dead-code criterion. AC-4/AC-5/AC-N1 verify the discriminator's *logic* through test files that `#[path]`-include `src/visibility.rs`, so they pass identically whether or not production ever calls it. AC-10 drives `run_aligned_planning_entries` and asserts a *change* against an unpainted baseline arm, so it can only pass on a wired tree.
...
| `mkdir -p target && cargo test -p seam-planner-default --test seam_region_aware_planning_tdd 2>&1 \| tee target/test-output.log \| grep -E '^test result'` | AC-10 production-path wiring, plus the blocker-exclusion and fully-blocked-region guards | FACT pass/fail |
