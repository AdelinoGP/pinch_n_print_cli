...
- **AC-5. Given** `support_type` is `tree*` or `hybrid*`, **when** family selection runs, **then** the matching `support-family:tree` planner and renderer are selected atomically and the manifests retain `support-planner`/`support-generator` role claims. | `rg -q 'support-family:tree' modules/core-modules/tree-support/tree-support.toml modules/core-modules/tree-support-planner/tree-support-planner.toml 2>/dev/null || rg -q 'support-family:tree' modules/core-modules/tree-support`
