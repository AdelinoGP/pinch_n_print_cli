...
- `crates/slicer-ir/src/slice_ir.rs` — one `#[serde(default)]` field on `ObjectMesh`: `model_rotation_z_rad: f32`. No schema-version constant moves; `CURRENT_MESH_IR_SCHEMA_VERSION` is untouched.
- `crates/slicer-model-io/src/loader.rs` — the `ObjectMesh` constructor (the private helper that today writes `transform: identity_transform()`) takes the composed build-item transform and records `atan2(m[1], m[0])` into the new field. `resolve_object` already computes that matrix as `effective_transform` and bakes it into the vertices; this threads it out rather than recomputing it. The STL path passes the identity and therefore records `0.0`.
- `crates/slicer-ir/src/resolved_config.rs` — two cli-bound fields: `align_infill_direction_to_model: bool = false` and the derived `model_rotation_deg: f32 = 0.0`.
- `crates/slicer-runtime/src/run.rs` — a new host helper invoked after each of the two `resolve_per_object_configs` call sites, writing the gated `model_rotation_deg` into every object's `ResolvedConfig`.
- `crates/slicer-model-io/tests/model_rotation_z_tdd.rs` — **new** binary, AC-11.
- `crates/slicer-runtime/tests/integration/model_rotation_alignment_tdd.rs` — **new** arm in the `integration` aggregate, AC-12.

**Blast radius owned by this packet — `ObjectMesh`:** adding a field to `ObjectMesh` (`crates/slicer-ir/src/slice_ir.rs`) obliges the step that adds it to fix **every** struct literal of `ObjectMesh` across the workspace in the same step, production and test alike. `ObjectMesh` is a watched type under the struct-literal churn gate (`docs/21_data_defaults_and_fixtures.md`): test literals need a `..` rest or an `// exhaustive: <reason>` waiver, production literals stay exhaustive. Re-derive the literal sites from the tree at implementation time (`rg -n 'ObjectMesh \{'`) — never from a count frozen in this document. Do not let a follow-up `cargo check` discover them.
...
- `crates/slicer-ir/src/{slice_ir.rs,resolved_config.rs}` — the two field additions only
- `crates/slicer-model-io/src/loader.rs` + `crates/slicer-model-io/tests/model_rotation_z_tdd.rs`
- `crates/slicer-runtime/src/run.rs` + `crates/slicer-runtime/tests/integration/model_rotation_alignment_tdd.rs`
...
- `crates/slicer-schema/wit/**` and every IR **schema-version constant** (`CURRENT_MESH_IR_SCHEMA_VERSION`, `CURRENT_SLICE_IR_SCHEMA_VERSION`, …) — no WIT change and no version bump is permitted; a worker who believes one is needed stops and raises a `[BLOCK]`. The two `ResolvedConfig` fields and the one `ObjectMesh` field named in §Code Change Surface are the whole of the permitted IR edit; anything beyond them is out of bounds.
- Re-applying the recorded rotation to any geometry. `model_rotation_z_rad` is a record of a transform already baked into the vertices; a step that multiplies a mesh, a polygon, or a bounding box by it has misread the design and must stop.
...
- `SNIPPETS` (≤ 2, ≤ 30 lines) — canonical's `align_infill_direction_to_model` blocks in `Fill.cpp::group_fills` and `LayerRegion.cpp::process_external_surfaces`: the exact `atan2` spelling, which matrix entries it reads, and where the offset is added relative to `calculate_infill_rotation_angle` (once, Step 7).
