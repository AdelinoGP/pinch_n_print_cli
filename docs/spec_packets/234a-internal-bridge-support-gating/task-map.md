...
| N-A (backlog: ISSUE-82 filter half) | Step 2 | `docs/04_host_scheduler.md` ShellClassification section; verified scheduler facts in design.md | `crates/slicer-ir/src/slice_ir.rs` (one host-only field), `crates/slicer-core/src/algos/prepass_slice.rs` (one literal), `crates/slicer-runtime/src/layer_executor.rs` (arm reduction), `crates/slicer-runtime/src/slice_postprocess_prepass.rs` (relocated pass) | — (placement mirrors 234's gate precedent) | `M` | Option-A revision: prepass writes `internal_bridge_lines` + extends `bridge_areas`; arm reduced to pure emitter. Ordering locked: after shell passes and 234's gate. |
