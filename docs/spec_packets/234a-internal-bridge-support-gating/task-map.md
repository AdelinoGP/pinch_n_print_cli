...
| N-A (backlog: ISSUE-82 filter half) | Step 2 | `docs/04_host_scheduler.md` ShellClassification section | `crates/slicer-runtime/src/layer_executor.rs` (arm removal), `crates/slicer-runtime/src/slice_postprocess_prepass.rs` (relocated pass), net-new unit fixture | — (placement mirrors 234's gate precedent) | `S` | Ordering locked: support gating strictly after 234's gate. |
| N-A (backlog: ISSUE-82 filter half) | Step 3 | `docs/15_config_keys_reference.md` (Doc Impact edit) | `resources/calicat.stl` import, net-new e2e test through e2e aggregator | — (bar frozen from authoring-session measurements) | `M` | AC-5 bar frozen 2026-08-24: ≤6 layers, ≤5000 mm, external Z≈3.2 ∈ [85°,95°]. |
