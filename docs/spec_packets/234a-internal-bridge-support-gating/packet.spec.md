...
- **AC-N1. Given** a synthetic two-layer fixture whose upper internal-solid surface sits ENTIRELY above lower-layer dense material (fully supported), **when** the relocated prepass pass processes the pair, **then** `InfillRegion.internal_bridge_infill` remains empty and `sparse_infill_area` is not modified — proving construction cannot fire without genuine unsupported span (root cause of the 148-layer flood). | `cargo test -p slicer-runtime --test unit -- internal_bridge_support_gating_tdd::fully_supported_pair_emits_no_internal_bridges --nocapture`
