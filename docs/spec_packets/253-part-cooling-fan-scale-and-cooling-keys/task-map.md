...
| N-A (queue packet P01) | Step 2 | `05-asset-packet-list.md` P01 row | `part-cooling.toml`, `machine-gcode-emit.toml`, `tests/cooling_config_schema_tdd.rs` | `PrintConfig.cpp::PrintConfigDef` | S | 15 keys in part-cooling + 6 in machine-gcode-emit (4 header/footer + folded `support_air_filtration` + supporting `chamber_temperature`); no co-declaration; guest freshness gate |
...
| N-A (queue packet P01) | Step 8 | — | `machine-gcode-emit/src/lib.rs`, new `tests/exhaust_and_chamber_emission_tdd.rs` | `GCode.cpp::_do_export`, `GCodeWriter.cpp::set_exhaust_fan`, `set_chamber_temperature`, `GCode.cpp::custom_gcode_sets_temperature` | S | AC-1b, AC-8 (+ folded `support_air_filtration` arm), AC-8b, AC-8c, AC-N1b; independent of Steps 3-7 |
