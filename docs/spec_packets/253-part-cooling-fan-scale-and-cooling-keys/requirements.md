...
### Folded key from wayfinder ticket 68 (P61) — `support_air_filtration`

Folded in from wayfinder ticket 68 (packet group P61, 2026-09-08) because it is an operator on exactly the header/footer emission this packet builds: canonical `GCode::_do_export` gates **both** exhaust-fan emissions behind the printer-level `support_air_filtration` bool (`PrintConfig.cpp` `PrintConfigDef`, `coBool`, default `true`; read sites `GCode.cpp::_do_export` print-start and print-end blocks, each `if (m_config.support_air_filtration.value)` wrapping the per-filament `activate_air_filtration` / `activate_air_filtration_during_print` / `activate_air_filtration_on_completion` reduction and the `GCodeWriter::set_exhaust_fan` write of `M106 P3 S<(int)(speed / 100.0 * 255)>`). Without it, this packet's AC-8 emits `M106 P3` lines its own canonical source guards with a key the packet never declares — wiring the gate anywhere else would mean writing the header/footer emission twice, and the ordering is load-bearing: it wraps both emissions, not one. The key is a Tier B master enable on the emission this packet adds (In Scope item 7 gains one gate clause; AC-8 gains the `false`-silences-both case), declared **only** in `machine-gcode-emit` alongside the four header/footer keys it gates. Packet counts stay 19 P01 keys + this 1 folded P61 key (queue count unchanged — P61 closes by fold, not by packet).

Canonical declaration: `PrintConfig.cpp` `PrintConfigDef` (`coBool`, default `true`). Canonical consumers (file + function): `GCode.cpp::_do_export` (both exhaust-fan blocks). Ported behaviour: `false` suppresses both `M106 P3` lines at any `activate_air_filtration` value; `true` (the default) leaves AC-8's behaviour exactly as specified.
