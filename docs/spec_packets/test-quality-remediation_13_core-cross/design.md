...
- Test-only change: no production signature, branch, threshold, or output changes; thirteen existing tests are preserved with marker-comment hardening only, twenty-six tests across the four touched files are untouched (five in `host_wrappers_tdd.rs`: the three logging tests `log_capture_records_all_levels_in_order` / `log_level_str_matches_host_mapping` / `take_log_messages_uninstalls_sink`, plus `clip_polygons_union_produces_nonempty_result_for_real_input` and `now_us_is_monotonic_within_a_thread`; three in `aabb_tree_tdd.rs`; eighteen in `polygon_ops.rs`'s `mod tests`; none in `host_batch.rs` since both its tests are marked), and exactly one new test is added (`offset_polygons_with_miter_limit_clamps_sharp_miter_corners`). AC-N1 pins the complete 12-name and 6-name rosters of `host_wrappers_tdd.rs` and `aabb_tree_tdd.rs`; the untouched `polygon_ops.rs` tests rest on AC-6's comment-only diff discipline.
...
- Exact functions, traits, manifests, tests, and fixtures:
  - `crates/slicer-sdk/tests/host_wrappers_tdd.rs` — six markers on `object_bounds_returns_host_unavailable_without_source`, `raycast_and_normal_route_through_installed_mesh_source`, `raycast_returns_none_without_source_documented_signal`, `offset_polygons_shrinks_and_grows`, `simplify_polygon_drops_collinear_vertices`, `simplify_polygon_short_input_returned_as_is`; one new test `offset_polygons_with_miter_limit_clamps_sharp_miter_corners` inserted between `offset_polygons_shrinks_and_grows` and the simplify section, calling `host::offset_polygons_with_miter_limit(&a, 1.0, OffsetJoinType::Miter, 0.0, 1.2)` on `square(0, 100_000)` and asserting both arms non-empty, `assert_ne!(grown, clamped, ...)` against the default-limit `host::offset_polygons(&a, 1.0, OffsetJoinType::Miter, 0.0)` result, and a strict point-count increase on the clamped contour. The delta is `+1.0` (outward) because Clipper2 clamps a miter only when the corner's miter ratio exceeds the limit, which requires a convex outward corner: measured, `+1.0` yields 4 default-limit points vs 8 clamped points, while `-1.0` yields 4 vs 4 for every limit in `0.5..=3.0`.
...
- Rejected alternatives and reasons:
  ...
  - Asserting an exact clamped-vertex count in the new test (rejected: the exact vertex split is a Clipper2 implementation detail that the packet did not need to pin. The count WAS measured at implementation time — `+1.0`/limit `1.2` yields 4 default-limit vs 8 clamped points (`target/probe-miter2.log`) — but the retained assertions stay non-empty, contour inequality, and strict count increase, so the test does not break if Clipper2 changes how many vertices a clipped miter emits).
...
- `crates/slicer-sdk/tests/host_wrappers_tdd.rs` - lines `1-263` only - purpose: the twelve-test roster (eleven pre-existing plus the new miter-limit test), the `square()` helper, and the seven marked tests' bodies. Line ranges: logging 18-64, mesh queries 65-150, geometry parity 151-251, time 252-263.
...
- The new test's strict point-count assertion depends on Clipper2's clipped-miter emitting two vertices per clamped corner; the load-bearing pinned assertions are non-empty + `assert_ne!` + strictly-more. **Measured** (2026-09-15 probe, `target/probe-miter2.log`): `+1.0` / limit `1.2` / arc `0.0` gives 4 default-limit points vs **8** clamped points (`assert_ne!` true, strict increase true); limit `1.5+` gives 4 vs 4 (no clamp, as expected, since `1/sin 45° ≈ 1.4142 < 1.5`); `Round` join at `+1.0` gives 56 vs 56 (clamp correctly does not apply). The count is asserted as a strict increase, never an exact value.
