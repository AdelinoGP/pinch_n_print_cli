...
- Verification:
  ...
  - `bash -c 'cargo test -p machine-gcode-emit --test machine_gcode_emit_tdd -- next_extruder_in_filament_start_gcode_is_fatal --exact 2>&1 | rg "^test result:" | rg -q "^test result: ok\. [1-9]" && echo PASS || echo "FAIL: next_extruder_in_filament_start_gcode_is_fatal did not run or did not pass"'` — **verbatim copy of `packet.spec.md` AC-N2; if either changes, change both.**
...
- Verification:
  ...
  - `bash -c 'cargo test -p machine-gcode-emit --test machine_gcode_emit_tdd -- max_layer_z_in_role_gcode_is_fatal --exact 2>&1 | rg "^test result:" | rg -q "^test result: ok\. [1-9]" && echo PASS || echo "FAIL: max_layer_z_in_role_gcode_is_fatal did not run or did not pass"'` — **verbatim copy of `packet.spec.md` AC-N1; if either changes, change both.**
...
- Files allowed to read, with ranges when over 300 lines:
  ...
  - `crates/slicer-runtime/tests/integration/machine_start_end_gcode_emission_tdd.rs` — **long; ranged reads only.** `slice_with_raw` / `try_slice_with_raw` and `count_occurrences`. Do not load the whole file.
- Files allowed to edit (at most 3):
...
### Step 7: File both residual rows, close `DEV-085`, register `TASK-307`

- Task IDs: `TASK-307`
- Objective: file one `DEV-###` row for the five unreachable points with their measured evidence, a second for the `coStrings` per-filament gap and the unmodelled `change_filament_gcode` flush/travel variable group, then flip `DEV-085` to `Closed` citing all three task IDs, and hand-add the `TASK-307` backlog row outside the generated block.
- Precondition: Steps 1-6 complete. **`DEV-085` may only be closed after both residual rows exist**, so the log never contains a closed row whose remainder is untracked.
- Postcondition: both residual rows exist; `DEV-085` reads `Closed` and cites `TASK-305` / `TASK-306` / `TASK-307`; `TASK-307` resolves in `docs/07_implementation_status.md`.
- Files allowed to read, with ranges when over 300 lines:
...
- Verification:
  ...
  - `bash -c 'python3 -c "import io; p=chr(124); L=io.open(r\"docs/DEVIATION_LOG.md\",encoding=\"utf-8\").read().splitlines(); need=(\"coStrings\",\"get_at\",\"flush_length_1\",\"x_after_toolchange\",\"erNone\",\"append_tcr\",\"get_extruder_id\",\"manual_filament_change\"); rows=[l for l in L if l.startswith(p+chr(32)+\"DEV-\") and not l.startswith(p+chr(32)+\"DEV-085\")]; hit=[l for l in rows if all(t in l for t in need)]; best=max(rows,key=lambda l:sum(t in l for t in need),default=\"\"); print(\"PASS\" if hit else \"FAIL: no single new DEV row carries all tokens; best row misses \"+str([t for t in need if t not in best]))"'` — **verbatim copy of `packet.spec.md` AC-16; if either changes, change both.** An earlier draft left the superseded round-1 probe here, so a worker gating on this step used a weaker check than the AC it claims to satisfy.
...
- Exit condition: AC-15, AC-16, AC-17 and AC-18 print PASS.
...
- Exit condition: both gates exit 0, `build-guests --check` reports no `STALE:`, and all twenty-three numbered AC commands (AC-1..AC-20, AC-N1..AC-N3) print PASS. Note Step 0's baseline ref must still exist at `target/pkt-188-baseline-ref.txt`; AC-12's no-touch guard reads it.
