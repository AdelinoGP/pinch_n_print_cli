...
| `TASK-307` | `Step 7` | `docs/DEVIATION_LOG.md`, `docs/07_implementation_status.md` | none (doc only) | `PrintConfig.cpp`, `GCode.cpp` — delegate; supply all three rows' evidence | `S` | **Three** residual rows and the deleted aggregate label's continued absence, then `TASK-307` registered outside the generated block. |
...
| --- | --- | --- |
| retired `custom-G-code injection deviation` label | **Remains absent.** This packet files the final two residual rows after packet 187's row; the surviving ledger carries each remainder explicitly. | Step 7 — AC-17 |
