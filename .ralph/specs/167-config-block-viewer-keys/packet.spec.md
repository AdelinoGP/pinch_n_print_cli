...
This packet touches only the CONFIG_BLOCK serialization in `crates/slicer-gcode/src/serialize.rs`, its integration test coverage in `crates/slicer-runtime/tests/integration/gcode_header_thumbnail_config_blocks_tdd.rs`, and one new normative doc subsection in `docs/02_ir_schemas.md`. The verbatim raw_config passthrough (`serialize_config_block` real-key dump) and the `emit_config_kv` dedup contract are unchanged. Full scope lists live in `requirements.md`.
