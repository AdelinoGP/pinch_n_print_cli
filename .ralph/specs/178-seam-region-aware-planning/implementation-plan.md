...
- Work one atomic step at a time; map every step to `TASK-294` (re-derived against the live `docs/07_implementation_status.md` during collision reconciliation; the previously quoted `TASK-284` row is the closed `claim:raft-fill` row of packet 124; the original `TASK-281` row is closed under packet 117).
