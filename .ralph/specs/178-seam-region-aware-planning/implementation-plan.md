...
- Work one atomic step at a time; map every step to `TASK-281`.
...
- `docs/07_implementation_status.md` receives the `TASK-281` crosswalk through a worker dispatch.
- `D-168-SEAM-PREPASS-SOURCE` is superseded or narrowed with evidence; packet 2's remaining algorithm reductions remain open.
- `packet.spec.md` is ready for `status: implemented` only after packet 2 can consume its exports.
