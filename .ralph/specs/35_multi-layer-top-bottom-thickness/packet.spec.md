...
- In scope:
  ...
  - looking up `top_solid_layers: u32` and `bottom_solid_layers: u32` per `(global_layer_index, object_id, region_id)` from `RegionMapIR.entries[*].config`
  ...
  - default values `top_solid_layers = 3`, `bottom_solid_layers = 3` when keys are absent (matches Orca defaults; confirm via OrcaSlicer reference delegation)
...
- Unblocks:
  ...
  - packet `38_top-surface-ironing` — needs precise topmost-layer detection (this packet provides it via the `top_solid_layers` window)
- Activation blockers:
...
- **Given** a region with resolved config `top_solid_layers = 3` and a `TopSurface` facet whose `z_min` lies inside the 3-layer window above `layer.z` (i.e. `[z_n, z_{n+3})`), **when** `classify_region_surfaces` runs against the multi-layer window, **then** every layer in `[n, n+3)` flags `is_top_surface=true` for that region. | `cargo test -p slicer-host --test multi_layer_thickness_tdd top_solid_layers_three_flags_three_layers -- --exact --nocapture`
- **Given** a region with resolved config `bottom_solid_layers = 3` and a `BottomSurface` facet whose `z_max` lies inside the 3-layer window below `layer.z`, **when** the helper runs, **then** every layer in `(n-3, n]` flags `is_bottom_surface=true`. | `cargo test -p slicer-host --test multi_layer_thickness_tdd bottom_solid_layers_three_flags_three_layers -- --exact --nocapture`
- **Given** an object with only 2 active layers and a region with resolved config `top_solid_layers = 5`, **when** the helper runs on layer 0, **then** both layers flag `is_top_surface=true` (window naturally truncates to the available layers; no panic, no out-of-bounds). | `cargo test -p slicer-host --test multi_layer_thickness_tdd window_truncates_at_object_extent -- --exact --nocapture`
- **Given** a region whose resolved config omits `top_solid_layers`, **when** the helper runs, **then** the window defaults to `3` layers and the resulting flags match the `top_solid_layers = 3` case from the first AC. | `cargo test -p slicer-host --test multi_layer_thickness_tdd missing_config_uses_default_three -- --exact --nocapture`
- **Given** `execute_layer_slice` invoked with `region_map: Some(&region_map_ir)` whose `entries[*].config` carries `top_solid_layers = 2`, **when** the call returns for a layer 1 step below a `TopSurface` facet at layer 2, **then** `SliceIR.regions[0].is_top_surface == true`. | `cargo test -p slicer-host --test multi_layer_thickness_tdd execute_layer_slice_honors_region_map_top_solid_layers -- --exact --nocapture`
- **Given** the unmodified Benchy STL run end-to-end with `top_solid_layers = 4` and `bottom_solid_layers = 4`, **when** the slicer produces G-code, **then** the count of `;TYPE:Top surface` blocks AND the count of `;TYPE:Bottom surface` blocks each is at least `4`. | `cargo test -p slicer-host --test benchy_end_to_end_tdd benchy_multi_layer_top_bottom_evidence -- --exact --nocapture`

## Negative Test Cases

- **Given** `region_map: None` (test fixtures that pre-seed without `RegionMapIR`), **when** `execute_layer_slice` runs, **then** the helper falls back to default `top_solid_layers = 3`, `bottom_solid_layers = 3`, NOT to `1` (which would be a regression vs Orca defaults). | `cargo test -p slicer-host --test multi_layer_thickness_tdd none_region_map_uses_orca_defaults -- --exact --nocapture`
- **Given** `top_solid_layers = 0` (user explicitly disables top surfaces), **when** the helper runs, **then** `is_top_surface` is `false` for every layer regardless of facet position. | `cargo test -p slicer-host --test multi_layer_thickness_tdd zero_top_solid_layers_disables_flag -- --exact --nocapture`
...
- `docs/03_wit_and_manifest.md` — config-key declaration rules; the implementer must verify whether `top_solid_layers` / `bottom_solid_layers` are already in the central config schema or need to be added. Delegate FACT.

## OrcaSlicer Reference Obligations

- `OrcaSlicerDocumented/src/libslic3r/PrintObject.cpp` — `discover_horizontal_shells()` propagates top/bottom solid-fill regions across `top_solid_layers` / `bottom_solid_layers`. Parity touchstone for the window semantics. Delegate FACT confirming the propagation direction (top: window EXTENDS DOWNWARD from a top-facing facet; bottom: EXTENDS UPWARD from a bottom-facing facet).
