...
- Extend `classify_region_surfaces` signature to accept `top_solid_layers: u32, bottom_solid_layers: u32` (or compute them inline from `region_map`).
...
- Default values: `top_solid_layers = 3`, `bottom_solid_layers = 3` (Orca defaults). Confirm via OrcaSlicer FACT delegation in Step 0.
...
- New Benchy E2E test asserting count of top/bottom blocks ≥ `top_solid_layers` / `bottom_solid_layers`.
...
- Config schema documentation update in `docs/02_ir_schemas.md` if `top_solid_layers` / `bottom_solid_layers` are not already declared.
...
- `docs/03_wit_and_manifest.md` — config-key declaration rules. Delegate FACT confirming whether `top_solid_layers` / `bottom_solid_layers` are already declared.
...
- Positive cases (see `packet.spec.md` §Acceptance Criteria):
  ...
  - Benchy E2E count of top/bottom blocks scales with `top_solid_layers` / `bottom_solid_layers`.
- Negative cases:
  - `region_map: None` falls back to Orca defaults, NOT to `1`.
  - `top_solid_layers = 0` disables the flag.
- Measurable outcomes:
