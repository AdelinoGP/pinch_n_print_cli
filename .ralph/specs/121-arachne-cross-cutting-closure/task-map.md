# Task Map: 121-arachne-cross-cutting-closure
...
| M2 — P116a/P116b/P117/P118/P119/P120 (no TASK-###) | A1/A2/B/C/D/E | draft → implemented | F depends on ALL of A1–E strictly; F cannot close until they are `status: implemented`. |
...
| --- | --- | --- |
| `D-116A-JUNCTION-BANDS` | Closed (A1) | F adds a one-line addendum noting the chain is closed. No in-place edits. |
| `D-116B-CONNECTJUNCTIONS-EMISSION` | Closed (A2) | F adds a one-line addendum noting the chain is closed. |
| `D-117-TRANSITION-ENDS` | Closed (B) | F adds a one-line addendum noting the chain is closed. |
| `D-118-ANGLE-FUDGE-NONCENTRAL` | Closed (C) | F adds a one-line addendum noting the chain is closed. |
| `D-119-LOCAL-MAXIMA-EPILOGUE` | Closed (D) | F adds a one-line addendum noting the chain is closed. |
| `D-120-POSTPROCESS-ORDER` | Closed (E) | F adds a one-line addendum noting the chain is closed. |
| `D-121-CHAIN-CLOSURE` (NEW) | — | F creates this entry documenting the chain closure (all N1–N13 fixes in place, e2e closure gate green, `cargo xtask test --workspace --summary` PASS). |
