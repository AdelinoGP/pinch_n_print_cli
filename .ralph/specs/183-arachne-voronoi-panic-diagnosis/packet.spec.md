...
- **AC-2. Given** the `perimeter_parity` workload that produced 13 swallowed panics during the D-160 session, **when** it is run after the guard lands, **then** the suite's pass/fail status is **unchanged from the Step-0/Step-1 baseline**, and `FINDINGS.md` records the baseline raw-panic count, the post-guard caught-builder-panic count, and an explicit same/changed statement about the suite status. Either count may legitimately be `0`. | `bash -c 'F=.ralph/specs/183-arachne-voronoi-panic-diagnosis/FINDINGS.md; mkdir -p target && cargo xtask build-guests --check && cargo test -p slicer-runtime --test integration -- perimeter_parity 2>&1 | tee target/183-parity.log | rg "^test result"; rg -q "^## Baseline" "$F" && rg -q "^## Caught panic count" "$F" && rg -q "^## Suite status vs baseline" "$F" && echo PASS || echo "FAIL: FINDINGS.md is missing the Baseline / Caught panic count / Suite status vs baseline sections"'`
