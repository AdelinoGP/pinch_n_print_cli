...
- **AC-3. Given** a 3-wall region in `aligned` mode with a host-injected `resolved_seam` 0.3 mm off the nearest wall vertex and a non-empty `seam_candidates` list (the aligned consumption branch — `select_seam_candidate` is preferred when `seam_candidates` is non-empty, and the residual aligned path through `aligned_seam_target` + `project_onto_wall_segment` only fires when `seam_candidates` is empty), **when** `run_wall_postprocess` runs, **then** the output region contains exactly 3 wall loops and the two non-target loops are point-for-point identical to their inputs. The target loop's `path.points` may have grown by one vertex (continuous projection inserts a point at the projected parameter `t ∈ (0, 1)`); the sibling-invariant claim only applies to the two non-target loops. | `cargo test -p seam-placer --test seam_sibling_walls_tdd -- aligned_snap_preserves_siblings 2>&1 | tail -5`
