...
- The historical `D-109-SEAM-FATAL-CORRECTED` reference (and its companion `D-108-SEAM-CONSUMED`) appears in `docs/07_implementation_status.md` (the P108/P109 closure paragraphs) and in `docs/05_module_sdk.md` (the seam-candidate-generation convention section) but is not registered in the canonical `docs/DEVIATION_LOG.md`. The historical claim is that P109 corrected P108's "fatal on empty seam-candidates" carve-out to graceful wall preservation; the audit's wall-preservation invariant is the codified form of that correction. Cite the historical references by packet (P108/P109) and by the in-module comment that documents the graceful-fallback path, not by the unregistered deviation ID.
