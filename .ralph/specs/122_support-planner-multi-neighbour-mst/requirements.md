...
- Grouped task IDs:
  - `TASK-263` — Multi-neighbour MST propagation (C4 from `docs/specs/support-modules-orca-port.md`)
...
- Replace the `nearest_neighbour` + `nearest_distance` lookup blocks in the propagation pass with a per-node aggregate computation.
...
- Apply the existing `max_move_xy` cap to the displacement; apply the existing `clamp_to_avoidance` post-cap.
- Add the wedge harness invariant `merge_geometry_symmetric_for_n_branches`.
- Add `modules/core-modules/support-planner/tests/multi_neighbour_mst_tdd.rs` with AC-2, AC-3, AC-N1 unit tests.
- Regenerate the wedge goldens.
...
- Cross-packet impact: goldens re-anchored.
...
| `cargo test -p support-planner --test multi_neighbour_mst_tdd 2>&1 \| tee target/test-output.log` | AC-2, AC-3, AC-N1. | FACT pass/fail; SNIPPETS ≤ 20 lines on failure |
...
- The `max_move_xy` cap and `clamp_to_avoidance` snap continue to apply AFTER the multi-neighbour aggregate is computed. The new aggregate replaces the *direction* of the move, not the cap or the avoidance enforcement.
...
- Large files in the read-only path that MUST be ranged or delegated:
  - `modules/core-modules/support-planner/src/lib.rs` — range-read propagation block (lines 586-660 area).
