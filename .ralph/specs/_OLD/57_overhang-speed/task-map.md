...
| `TASK-182` | Step 4 — `resolve_feedrate` extension + `classify_layers` invocation in `emit_gcode` | `docs/02_ir_schemas.md` | `crates/slicer-host/src/gcode_emit.rs:154,272,388,443` (signature change, classifier call inside `emit_gcode`, per-point threading, z-hop) — covers both pipeline arms via `execute_postpass`; no edits to `pipeline.rs` | none new | `M` | Per-point loop at `gcode_emit.rs:362-393` is structurally unchanged; only the new arg threads at `:388`. All seven AC tests go GREEN at the end of this step. |
| `TASK-182` | Step 5 — Regression sweep + clippy gate | none | none (pure dispatch) | none | `S` | `gcode_feedrate_emission_tdd`, `gcode_emit_tdd` (carries `;TYPE:` label invariants), clippy, `cargo check --workspace`. |
