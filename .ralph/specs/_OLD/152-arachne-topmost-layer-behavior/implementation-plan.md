...
- Objective: add `is-bottom-layer` / `is-topmost-layer` to `common.wit`
  `arachne-params`; mirror in `ArachneParams` (+ `Default = false`); add the two
  fields at ALL THREE conversion sites — `slicer-sdk/src/host.rs:551` (native
  path), `slicer-sdk/src/host.rs:690` (WIT path), and
  `slicer-wasm-host/src/host.rs:1773-1794` (host-side WIT→core mapping). There
  is NO `ArachneParams` adapter in `slicer-macros` — do not edit that crate.
  Rebuild guests.
...
- Files read: `crates/slicer-schema/wit/deps/common.wit:22-50`,
  ...
  `crates/slicer-sdk/src/host.rs:545-700`,
  `crates/slicer-wasm-host/src/host.rs:1760-1800`.
- Files edit (≤3 per touch): touch 1 — `common.wit`, `pipeline.rs`; touch 2 —
  `crates/slicer-sdk/src/host.rs`, `crates/slicer-wasm-host/src/host.rs`.
- Out-of-bounds: generated `*/wit-guest/` bindgen; `crates/slicer-macros`.
- Dispatches: "Confirm the only `ArachneParams` conversion sites are
  `slicer-sdk/src/host.rs:551,:690` and
  `slicer-wasm-host/src/host.rs:1773-1794` (rg both crates plus
  wit_host.rs/dispatch.rs, which have none today); LOCATIONS."; "Run `cargo
  build --tests`; FACT."; "Run `cargo xtask build-guests`; FACT success +
  failing guests."
- Context cost: `M`.
...
- Objective: `remove_small_lines` keys the lenient `min_width/2` threshold on
  `is_bottom || is_topmost` (the threshold's `min_width` is the per-line
  junction-width minimum bound at `:63-67` — keep that derivation; the
  `_min_width` param stays discarded). `run_arachne_pipeline` passes the flags
  from `params`. Adapt the G10 gap test's CALL to supply the topmost flag `true`
  (assertion `!surviving.is_empty()` unchanged — see requirements §Step
  Completion). Audit `is_initial_layer` consumers before subsuming it. Author a
  NEW `#[cfg(test)] mod tests` in remove_small.rs housing `non_top_layer_strict`
  (no test module exists in the file today — without it the AC-N1 `--lib`
  filter matches nothing and false-passes).
...
- Dispatches: "Find all readers of `ArachneParams.is_initial_layer`; LOCATIONS.";
  ...
  "Run the G10 gap test + `cargo test -p slicer-core --lib --
  arachne::remove_small::tests::non_top_layer_strict --exact`; FACT (must
  report ≥1 test run)."
- Context cost: `M`.
...
- Files edit (≤3): `modules/core-modules/arachne-perimeters/src/lib.rs`,
  `modules/core-modules/arachne-perimeters/tests/only_one_wall_top_tdd.rs`
  (NEW — houses `only_one_wall_top_disabled` now, `only_one_wall_top_second_pass`
  in Step 4; standalone `*_tdd.rs` following the module's 10 existing native
  test files ⇒ auto-registered).
- Out-of-bounds: the host prepass (read-only — confirm population only).
- Dispatches: "Run the G3 gap test + `cargo test -p arachne-perimeters --test
  only_one_wall_top_tdd -- only_one_wall_top_disabled --exact`; FACT (must
  report ≥1 test run)."; "Run `cargo xtask build-guests --check`; FACT."
- Context cost: `M`.
...
- Files edit (≤3): `modules/core-modules/arachne-perimeters/src/lib.rs`,
  `modules/core-modules/arachne-perimeters/tests/only_one_wall_top_tdd.rs`
  (append `only_one_wall_top_second_pass` to the file Step 3 created — the
  native `run_perimeters` harness, NOT `--lib`).
...
- Dispatches: "Summarize `PerimeterGenerator.cpp:2160-2246` second-pass
  algorithm; SUMMARY ≤200 words."; "Run `cargo test -p arachne-perimeters --test
  only_one_wall_top_tdd -- only_one_wall_top_second_pass --exact`; FACT or
  SNIPPETS."
- Context cost: `M`.
...
- Gaps: bookkeeping.
- Objective: document the two WIT fields (docs/03), annotate the EXISTING
  `only_one_wall_top` (`docs/15:59`) and `min_width_top_surface` (`docs/15:58`)
  rows as behavioral as of P152 (both keys already appear there — the Doc Impact
  grep tests the P152 annotation, not the key name), mark G3/G10 closed in the
  docs/18 rows (`:178`, `:185`), and narrow
  `D-104d-MIN-WIDTH-TOP-SURFACE-NONE` in the DEVIATION_LOG: arachne half landed
  (P152); classic-perimeters remainder split into
  `D-152-CLASSIC-MIN-WIDTH-TOP-SURFACE-REMAINDER`. Do NOT mark D-104d wholly
  closed.
...
- 15 `arachne_parity.rs` locks green, incl. the
...
- Doc Impact greps hit; `D-104d-MIN-WIDTH-TOP-SURFACE-NONE` narrowed (arachne
  half landed, classic remainder split into
  `D-152-CLASSIC-MIN-WIDTH-TOP-SURFACE-REMAINDER`). With this packet, gaps
  G1–G10 are all closed (G11 excluded by decision).
...
- Record the G10 test-call adaptation, the topmost-area-source decision
  (including the amending-ADR flag if it diverges from Orca — ADR-0035's
  recording bar), and the `is_initial_layer` disposition explicitly before
  `status: implemented`.
