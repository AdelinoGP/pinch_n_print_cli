...
- The `cube_4color_arachne/` fixture directory's `expected_perimeter_ir.json` is captured by running the arachne wall output on `cube_4color.3mf` once and committing the result. This is a one-time self-captured golden per `self-captured Arachne baselines`, not regenerated.
...
- **`interim MMU topology` stays open across this packet:** the original 113a draft claimed the deviation would close. It does not. The deviation's "follow-up" column should be updated to point to P113b's quad/rib pass + `connectJunctions` as the target, not to AC-4's unit test.
