...
- Verification:
  ...
  - `bash -c 'git diff crates/slicer-runtime/tests/fixtures/golden/precision_legacy_20mmbox.gcode | rg "^[-+][^-+]"'` — FACT: **every changed line must be an `; outer_wall_line_width` or `; inner_wall_line_width` comment, and nothing else.** Do **not** assert a fixed `git diff --numstat` of 2 insertions / 2 deletions: draft packet `184-classic-perimeter-flow-parity` Step 5 re-blesses this same golden for wall-coordinate drift, and if it lands first the numstat baseline is no longer `master`'s. The line-shape check above is order-independent; the numstat is not.
- **Cross-packet collision on this exact file.** `184-classic-perimeter-flow-parity` Step 5 re-blesses `precision_legacy_20mmbox.gcode` for a different reason (its `line_width_to_spacing` inset change moves the wall coordinates). Both packets are `draft`; **neither may assume it lands first, and whichever lands second re-blesses and re-verifies rather than assuming** the first's green result still holds. Packet 184 declares the same rule in its `packet.spec.md` §Prerequisites, `requirements.md` §Cross-packet impact, and `implementation-plan.md` Step 5. Before re-blessing, establish the order with `bash -c 'rg -q line_width_to_spacing modules/core-modules/classic-perimeters/src/lib.rs && echo "184 LANDED FIRST" || echo "182 LANDS FIRST"'` and record the answer in this step's completion note. If 184 landed first, the wall coordinates have already moved; do not treat them as fallout of this packet's header change.
## Step 3 completion note (2026-07-28)
- Landing order probe: `rg -q line_width_to_spacing modules/core-modules/classic-perimeters/src/lib.rs` returned non-zero, so the probe concluded `182-LANDS-FIRST`. Recorded at the time of re-bless, before `git diff --numstat` showed `2\t2`. The diff line-shape check (`git diff ... | rg "^[-+][^-+]"`) showed only the four expected wall-width lines (two removed, two added) — no other drift absorbed.
- Exit condition: AC-3 passes, the golden's changed lines are exactly the two wall-width header comments, and the landing order relative to packet 184 is recorded.
...
- Flip the `G-code header width-default gap` row in `docs/DEVIATION_LOG.md` to `Closed — <date> (packet 182): header defaults corrected to the governing 0.4/0.4 fallback; dangling config_schema.rs citation removed; precision_legacy_20mmbox golden re-blessed`, via a worker dispatch editing only that row.
...
- Separately, hand-add the `TASK-295` backlog row to `docs/07_implementation_status.md` (the task rows sit well outside the generated block, near the other `TASK-###` entries). This is the one legitimate hand-edit to that file. **Re-derive the id situation at implementation time; never trust a "highest is TASK-NNN" figure written into a packet** — ids land continuously from parallel packets. Check both ledgers before writing: `rg -o 'TASK-[0-9]{3}' docs/07_implementation_status.md | sort -u | tail -5` **and** `rg -o 'TASK-[0-9]{3}' .ralph/specs/ --no-filename | sort -u | tail -5`. If `TASK-295` has been taken since this packet was authored, take the next free id and update every `TASK-295` reference in this packet's five files.
