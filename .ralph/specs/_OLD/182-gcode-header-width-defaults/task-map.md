...
Backlog anchor: deviation `G-code header width-default gap` in `docs/DEVIATION_LOG.md` (also listed in the generated open-deviations block of `docs/07_implementation_status.md`, which is regenerated via `cargo xtask check-deviations` — never hand-edited).
