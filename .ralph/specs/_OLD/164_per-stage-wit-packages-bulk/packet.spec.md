...
if not os.path.exists(pt): bad.append('missing flat deps/prepass-types.wit')
    ...
    for t in ['mesh-object-view','paint-value-view','paint-stroke-view','paint-layer-view','layer-plan-view','layer-plan-view-entry']:
        if t not in s: bad.append(f'prepass-types missing {t}')
sg=open(f'{B}/prepass-support-geometry/prepass-support-geometry.wit',encoding='utf-8').read()
if re.search(r'(?m)^\s*record layer-plan-view',sg): bad.append('support-geometry redeclares layer-plan-view; it must use the shared slicer:prepass-types copy (ADR-0002)')
...
- **AC-7. Given** post-163 the manifest key `wit-world` is required by `crates/slicer-scheduler/src/manifest.rs` (`required_string(&root, manifest_path, "module.wit-world")` then `validate_wit_world`) and `slicer_schema::SUPPORTED_WIT_WORLDS` backs the allowlist, **when** the key retires, **then** zero `modules/core-modules/*/*.toml` files contain a `wit-world` line; `validate_wit_world` and the manifest struct's `wit_world` field and `wit_world()` accessor are deleted from `crates/slicer-scheduler/src/manifest.rs`; `SUPPORTED_WIT_WORLDS` is deleted from `crates/slicer-schema/src/lib.rs`; `crates/pnp-cli/src/module_new.rs` no longer scaffolds a `wit-world` line (its scaffold comment names the stage's qualified export via `slicer_schema::qualified_export_for_stage_id` instead, and `wit_world_for_stage` plus its `wit_world_mapping` test are deleted); and the scheduler manifest-ingestion suite passes with its `wit_world_*` tests removed or rewritten per AC-N3. | `mkdir -p target && python3 -c "import glob,re; n=sum(1 for f in glob.glob('modules/core-modules/*/*.toml') if re.search(r'(?m)^\s*wit-world\s*=',open(f,encoding='utf-8').read())); m=open('crates/slicer-scheduler/src/manifest.rs',encoding='utf-8').read(); s=open('crates/slicer-schema/src/lib.rs',encoding='utf-8').read(); p=open('crates/pnp-cli/src/module_new.rs',encoding='utf-8').read(); bad=([f'manifests still declaring wit-world: {n}'] if n!=0 else [])+(['validate_wit_world survives'] if 'validate_wit_world' in m else [])+(['SUPPORTED_WIT_WORLDS survives'] if 'SUPPORTED_WIT_WORLDS' in s else [])+(['module_new still scaffolds wit-world'] if 'wit-world' in p else []); print('PASS' if not bad else 'FAIL '+'; '.join(bad))" && (cargo test -p slicer-scheduler --test scheduler_integration -- manifest_ingestion 2>&1 | tee target/test-output.log | rg '^test result: ok\. [1-9][0-9]* passed' || echo 'FAIL: tests failed or none ran')`
