...
- Add `crates/slicer-runtime/tests/common/dispatch_fixture.rs` with `pub struct DispatchFixture`; entry point `pub fn for_stage(stage_id: &str) -> DispatchFixtureBuilder` (a free function in the module — both migrated tests call it as `crate::common::dispatch_fixture::for_stage(...)`); builder methods `.with_slice(SliceIR)`, `.with_perimeter(PerimeterIR)`, `.with_config(ConfigView)`, `.with_wat(&str)`, `.no_wasm()`; terminal `.build() -> DispatchFixture`. The default builder path compiles a real WAT test guest and uses an empty `ConfigView::from_map(HashMap::new())`.
- The `DispatchFixture` value owns `dispatcher: WasmRuntimeDispatcher`, `blackboard: Blackboard`, `arena: LayerArena`, and the `TestModuleBundle` produced by the builder. It exposes per-runner methods `run_layer(&mut self, layer: &GlobalLayer) -> Result<(), slicer_ir::LayerStageError>` (takes `&mut self` because the underlying `run_layer_and_commit_with_bundle` mutates the arena), `run_prepass(&self) -> Result<slicer_core::PrepassStageOutput, slicer_ir::PrepassRunnerError>`, `run_finalization(&self, layers: &mut Vec<LayerCollectionIR>) -> Result<slicer_ir::FinalizationOutput, slicer_ir::FinalizationError>` (takes `&mut Vec` to match the `FinalizationStageRunner::run_stage` trait signature), `run_postpass(&self, gcode: &mut GCodeIR) -> Result<slicer_ir::PostpassOutput, slicer_ir::PostpassError>` (uses `PostpassStageRunner::run_gcode_postprocess` and takes `&mut GCodeIR` to match its signature). Mutation of arena state happens inside `run_layer` via the existing `run_layer_and_commit_with_bundle` path.
