...
- `crates/slicer-host/src/region_mapping.rs::execute_region_mapping` — **structurally refactored** (wrapped by new `execute_region_mapping_with_cap`); no modifier-volume parameter and no Z-band stamping added. Modifier overlap detection moved to the paint annotation pipeline (see below).
- `crates/slicer-host/src/slice_postprocess.rs::execute_slice_postprocess_paint_annotation` — extended with `modifier_projections: Vec<ExPolygon>` field on `SlicePostProcessPaintAnnotationRequest`. When `FuzzySkin` is a required semantic, after the primary paint annotation pass, a second pass checks each contour point against the per-layer modifier projections via `slicer_core::ex_polygon_contains_point`. Points inside modifier projections get `PaintValue::Flag(true)` for `FuzzySkin`, overriding any default `Flag(false)` from paint region fallback. This piggybacks on the existing paint-annotation → boundary_paint → feature_flags → fuzzy-skin pipeline — zero WIT, module, or dispatch changes.
- `crates/slicer-host/src/layer_executor.rs::run_paint_annotation` — before calling `execute_slice_postprocess_paint_annotation`, iterates `blackboard.mesh().objects[].modifier_volumes` and calls `slicer_core::slice_mesh_ex(&mv.mesh, &[layer.z])` for each `modifier_part` subtype volume. The resulting per-layer ExPolygon set is passed as `modifier_projections` to the paint annotation request.
- `crates/slicer-core/src/paint_region.rs` — `ex_polygon_contains_point` made public and exported from `slicer_core`.
- `crates/slicer-host/src/pipeline.rs` — **no changes.** Modifier projections are computed and consumed within the existing `Layer::SlicePostProcess` paint annotation step; the Blackboard is the data bus.
- `modules/core-modules/fuzzy-skin/fuzzy-skin.toml`'s `[config.schema]` block is verified to declare `apply_to_all`. If absent, register it additively (entry name: `apply_to_all`, type: `bool`, default: `false`).
- `modules/core-modules/fuzzy-skin/src/lib.rs` — config key normalization applied (DEV-053): `"apply-to-all"` → `"apply_to_all"` and `"point-distance"` → `"point_distance"`. Required because the stamped key `"apply_to_all"` (snake_case per CLAUDE.md convention) would not match the module's previous kebab-case lookup.
...
- `crates/slicer-host/tests/core_module_ir_access_contract_tdd.rs` — manifest schema contract. Must stay green after the `fuzzy-skin/fuzzy-skin.toml` edit.
...
| Paint drop on non-`NormalPart` | Drop with `log::warn!` per dropped part. The warning contains the substring `"paint data on non-normal part dropped"` and the part id. | DEV-052 contract. |
...
| `modifier_part` fuzzy region wiring | **Paint-annotation stamp** via `execute_slice_postprocess_paint_annotation` in `slice_postprocess.rs`. Before calling the annotator, `layer_executor.rs::run_paint_annotation` iterates `modifier_volumes` and calls `slice_mesh_ex` to project each `modifier_part` mesh at the current layer Z. The per-layer `ExPolygon` set is passed as `modifier_projections`. Inside the annotator, `ex_polygon_contains_point` tests each contour point; points inside a modifier projection receive `PaintValue::Flag(true)` for `FuzzySkin`. `execute_region_mapping` is unchanged for modifier logic. | Changed from Option 1 (region-mapping direct stamp) during implementation: `ActiveRegion` carries config only at RegionMapping time — no polygon geometry is available there — so the Z-band geometric test must run post-slicing where `SlicedRegion.polygons` exist. The paint annotation step in `slice_postprocess.rs` is the correct integration point. |
...
| Fuzzy-skin manifest gate | Step 3 FACT dispatch checks the `[config.schema]` block. If `apply_to_all` is missing, additive edit. If present, no-op. | Activation Q4 from the original packet. |
...
| PaintSegmentation piggyback for `modifier_part` fuzzy_skin | Originally rejected in favour of Option 1 (region-mapping direct stamp). During implementation Option 1 was also rejected (polygon geometry unavailable at RegionMapping time). The final approach uses the paint annotation pipeline — `layer_executor.rs` + `slice_postprocess.rs` — which is distinct from the PaintSegmentation prepass stage and does not require WIT or module changes. |
...
| Add `WallFeatureFlags.fuzzy_skin` propagation through perimeter generation | The `fuzzy-skin` module's existing `apply_to_all` config flag covers the entire region. No per-vertex flag path needed. |
| Run modifier-overlap stamp BEFORE Packet 51's paint-semantic overlay | Both producers stamp the same `fuzzy_skin.apply_to_all` key only on overlap, so ordering doesn't matter for the bool — but stamping after preserves Packet 51's invariants more clearly. |
...
2. `crates/slicer-host/src/layer_executor.rs` — `run_paint_annotation` modifier projection loop. ~50 added lines.
3. `crates/slicer-host/src/slice_postprocess.rs` — `execute_slice_postprocess_paint_annotation` modifier overlap check inside paint annotation pass. ~40 added lines.
4. `crates/slicer-core/src/paint_region.rs` — `ex_polygon_contains_point` made public and exported. ~1 added line.
5. `modules/core-modules/fuzzy-skin/fuzzy-skin.toml` — additive edit only if `apply_to_all` is missing.
6. `modules/core-modules/fuzzy-skin/src/lib.rs` — config key normalization (kebab→snake); DEV-053.
7. Two new test files under `crates/slicer-host/tests/`.
8. Documentation files (`docs/02_ir_schemas.md`, `docs/07_implementation_status.md`, `docs/DEVIATION_LOG.md`, `docs/14_deviation_audit_history.md`).
...
| `modules/core-modules/fuzzy-skin/src/lib.rs` | 1-120 | `apply_to_all` consumer branch (read-only verification). |
| `modules/core-modules/fuzzy-skin/fuzzy-skin.toml` | full (small) | `[config.schema]` block. |
