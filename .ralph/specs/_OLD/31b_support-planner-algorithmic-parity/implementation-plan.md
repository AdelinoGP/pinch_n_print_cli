...
- Task IDs: `TASK-163`
- Objective: Capture `resources/golden/benchy_tree_support_orca_branch_count.txt` (single integer: Pinch 'n Print baseline branch count) and `resources/golden/benchy_tree_support_orca_endpoints.txt` (newline-delimited `x,y,z` of Pinch 'n Print baseline branch endpoints) from a clean run of `support-planner` on the synthetic single-object overhang fixture. Headers in both files explicitly note these are Pinch 'n Print self-captures, not OrcaSlicer reference data. Implement `benchy_orca_parity_within_tolerance` (test name retained for stability): asserts branch count within ±10% of the captured baseline AND endpoint Hausdorff distance ≤ 0.5mm via `directed_hausdorff` helper in the test file. Either failure fails the test. Provide a `SUPPORT_PLANNER_REGEN_GOLDEN=1` environment-variable escape hatch in the test for intentional re-capture.
