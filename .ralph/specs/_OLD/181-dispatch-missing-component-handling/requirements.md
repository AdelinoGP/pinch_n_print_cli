...
- **Migrate the one behavioral blast radius: tests where an absent component reaches `WasmRuntimeDispatcher`.** `dispatch_fixture.rs`'s `.no_wasm()` builder (consumers: `tests/contract/dispatch_protocol_tdd.rs`, `tests/contract/infill_postprocess_contract_tdd.rs`); the `run_layer_and_commit` helper in `tests/common/mod.rs` (one external caller: `tests/executor/live_seam_path_tdd.rs`); and the inline `CompiledModuleLive::new(..., None, ...)` sites in `tests/unit/dag_validation_tdd.rs`, `tests/executor/live_seam_path_tdd.rs`, `tests/contract/postpass_gcode_empty_list_tdd.rs`, and `tests/contract/postpass_gcode_boundary_tdd.rs`. This class is measured complete. Stated precisely, so Step 1 does not "discover" a contradiction: no *other* test-side `CompiledModuleLive::new` passes a literal `None`. Several sites pass a *variable* `Option` — `bundle.component.clone()` or `self.component.clone()` (`tests/common/mod.rs`, `tests/executor/finalization_mutation_roundtrip_tdd.rs` ×2, `tests/executor/finalization_live_tdd.rs` ×2, `tests/executor/finalization_world_deep_copy_tdd.rs`; re-derive with `rg -n 'CompiledModuleLive::new' crates/*/tests -A6`). Those are `None` only when the bundle came from `make_no_wasm_bundle` (`tests/common/dispatch_fixture.rs`), which Step 6 owns; through every other constructor they are `Some(...)`. The remaining sites pass a literal `Some(...)`.
- De-vacuum `postpass_gcode_boundary_carries_all_payload_variants_into_guest`.
- Update `docs/04_host_scheduler.md`, amend ADR-0020 with a `missing-component ADR amendment` deviation row, flip/correct the missing-component dispatch defect row, and register `TASK-297`.
...
- The per-stage versioned-interface split owned by draft packets `162`-`165`. This packet removes their missing-component dispatch defect carve-out without touching their surface.
- DEV-026 and custom-G-code injection deviation, which the same draft packets also name as out-of-scope. Each has its own queue entry.
- Changing `is_placeholder_wasm` itself, or the `placeholder_wasm` accessor on `LoadedModule` (`crates/slicer-scheduler/src/manifest.rs`). They become inert for dispatch purposes once the loader branch is fatal. **The deferral is not left unowned:** no successor packet exists in `.ralph/specs/` for it, so this packet **files a tracking deviation row** in `docs/DEVIATION_LOG.md` recording the dead-but-live symbols (`is_placeholder_wasm`, `LoadedModule::placeholder_wasm`, and `manifest.rs`'s now-false affordance doc comment), owner `slicer-scheduler maintainers`, severity Low, status Open. **Re-derive the id at implementation time — never quote one from this packet:** `rg -o '^\| DEV-[0-9]{3}' docs/DEVIATION_LOG.md | sort -u | tail -1`, then take the next free number. Use the log's existing eight-column format (`| ID | Date | Affected Section | Risk | Rationale | Mitigation Owner | Target Close | Status |`).
- **Removing the six `.unwrap_or_else(|| (WasmInstancePool::placeholder(), None))` fallbacks in `crates/slicer-runtime/src/{prepass,layer_executor,postpass,layer_finalization}.rs`.** An earlier draft of this packet did remove them, on the theory that a `wasm_handles` lookup miss should be an internal error. Measurement refuted it: **18 test files** pass an empty or partial `wasm_handles` alongside a module-bearing stage list, and 13 of them do so *deliberately*, injecting the real pool/component through a bespoke runner — a pattern ADR-0007 sanctions ("`WasmInstancePool::placeholder()` … exists as the explicit fallback for in-process test pipelines that don't need real dispatch"). Removing the fallbacks would have forced ~53 call-site migrations across the `unit` and `executor` buckets for **no missing-component dispatch defect benefit**: the deviation is about dispatch laundering, and once the five arms are fatal, a genuine production handle miss is caught there anyway. The fallbacks stay; the completion gate asserts the count is still 6.
...
- `docs/DEVIATION_LOG.md` — the missing-component dispatch defect row only; large file, never read in full.
...
- Cross-packet impact: unblocks the missing-component dispatch defect carve-out in draft packets 163 and 164; no code overlap with them.
