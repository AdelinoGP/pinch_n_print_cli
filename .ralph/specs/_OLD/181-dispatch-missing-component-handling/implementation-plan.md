...
- Task IDs: `TASK-297`
- Objective: Change all five `Err(e) if e.phase == DispatchPhase::MissingComponent` arms in `crates/slicer-wasm-host/src/dispatch.rs` to return the phase-appropriate fatal module error (`PrepassRunnerError`, `LayerStageError::FatalModule`, `FinalizationError`, `PostpassError::FatalModule`) naming the module id. **Also correct the now-false doc comment on `CompiledModuleLive::wasm_component` (`crates/slicer-wasm-host/src/binding.rs`)**, which currently reads "`None` for placeholder or test modules" — after this step `None` is never legal at dispatch, and that comment is the exact sentence packet 163 cites as its justification for the carve-out (see Packet Completion Gate). Doc-comment text only; the field type and `CompiledModuleLive::new`'s signature are unchanged.
...
- Files allowed to read: `crates/slicer-wasm-host/src/dispatch.rs` (long; never in full) — ±40-line windows via `rg 'DispatchPhase::MissingComponent'`; `crates/slicer-wasm-host/src/binding.rs` — the `wasm_component` field doc comment only.
- Files allowed to edit (at most 3):
  - `crates/slicer-wasm-host/src/dispatch.rs`
  - `crates/slicer-wasm-host/src/binding.rs` (doc comment only — no signature or type change)
...
- Verification:
  ...
  - `bash -c '! rg -q "None. for placeholder or test modules" crates/slicer-wasm-host/src/binding.rs && echo PASS || echo "FAIL: stale wasm_component doc comment"'` — FACT PASS/FAIL. (The `.` stands in for a backtick so the pattern survives a double-quoted `bash -c` string; the phrase currently matches, so the check is falsifiable.)
...
- Amend `docs/adr/0020-layer-stage-commit-as-per-stage-enum.md` by **appending an `## Amendment — <date> (packet 181)` section** that quotes §Decision item 1's retired clause verbatim and states the replacement (`None` is the empty-commit case only; a missing component is `Err(LayerStageError::FatalModule)`). Do **not** delete or rewrite the clause in place. The post-purge ledger no longer carries the historical `missing-component ADR amendment` label, so verify the surviving ADR section and fatal dispatch regression instead: `bash -c "rg -q '^## Amendment' docs/adr/0020-layer-stage-commit-as-per-stage-enum.md && rg -q 'packet 181' docs/adr/0020-layer-stage-commit-as-per-stage-enum.md && rg -q 'is the empty/' docs/adr/0020-layer-stage-commit-as-per-stage-enum.md && ! rg -q '^\| missing-component ADR amendment ' docs/DEVIATION_LOG.md && rg -q 'missing_component_is_fatal_for_all_five_stages' crates/slicer-runtime/tests/contract/dispatch_missing_component_tdd.rs && echo PASS || echo FAIL"`.
- File the deferred-placeholder-cleanup tracking row in `docs/DEVIATION_LOG.md` (Low / Open / owner `slicer-scheduler maintainers`) covering `is_placeholder_wasm`, `LoadedModule::placeholder_wasm`, and `manifest.rs`'s now-false skip-affordance doc comment — no successor packet exists to own them. Re-derive the id first: `rg -o '^\| DEV-[0-9]{3}' docs/DEVIATION_LOG.md | sort -u | tail -1`, then take the next free number; never carry an id forward from this packet.
- Preserve the packet-181 fatal-on-missing-component behavior; the historical missing-component dispatch row is retired from the post-purge ledger. The surviving proof is `dispatch_missing_component_tdd::missing_component_is_fatal_for_all_five_stages` plus the ADR-0020 amendment.
...
- Reconcile draft packets `163`/`164`, whose missing-component dispatch defect carve-outs go stale once this lands. Re-derive the exact wording at reconciliation time (`rg -n 'MissingComponent|missing-component dispatch defect' .ralph/specs/163*/ .ralph/specs/164*/`) rather than trusting a quotation frozen here; as authored it is 164's `implementation-plan.md` ("byte-untouched"), 164's `design.md` ("must not be widened or narrowed here" / "do not touch"), and 163's `requirements.md` §Out of Scope. Record the note; do not edit their files.
- **Also rebut 163's stated justification** — the one live objection to this packet. It has two halves, both in `163_per-stage-wit-packages-pilot/requirements.md` (§"Why this packet exists" and §Out of Scope) and restated in its `design.md`; rebut both:
  1. *"`binding.rs:29` documents `CompiledModuleLive::wasm_component` as \"`None` for placeholder or test modules\""* — that doc comment is the field's own documentation on `CompiledModuleLive::wasm_component` (`crates/slicer-wasm-host/src/binding.rs`). It describes the pre-packet contract only, and this packet **falsifies** it: after Step 5 there is no legal absent-component path, so the comment must be updated in the same step (it is on Step 5's edit list) rather than cited as a reason not to change the arms.
  2. *"the four conversion sites are load-bearing for host-builtin stages that legitimately have no component"* — false on two counts. The count is five, not four (163 predates the `LayerStageRunner::run_stage` arm being noticed). And host builtins run through `run_builtin_stage` (`crates/slicer-runtime/src/prepass.rs`) under `host:*` stage ids and never construct a `CompiledModuleLive`; `build_live_execution_plan` (`crates/slicer-wasm-host/src/execution_plan_live.rs`) inserts only discovered `LiveModuleBinding`s.
  Record the rebuttal with those symbol references so the objection is not silently re-litigated.
