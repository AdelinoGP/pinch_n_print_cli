...
- Exact functions, traits, manifests, tests, and fixtures:
  ...
  - The `CompiledModuleLive::wasm_component` field doc comment (`crates/slicer-wasm-host/src/binding.rs`) — "`None` for placeholder or test modules" becomes false once the arms are fatal. Doc comment only; no type or signature change.
...
- Rejected alternatives and reasons:
  - *Preserve the skip behind an explicit placeholder marker.* Requires carrying the fact from the loader to six sites in another crate — either widening `wasm_handles` (~21 signatures) or adding a field to `CompiledModuleStatic`. All that machinery would protect a path with **no current users** (no module in the tree is an ≤8-byte stub — re-derive with `find modules/core-modules -name '*.wasm' -printf '%s %p\n' | sort -n | head -1`; every built artifact is four to five orders of magnitude above the threshold) and whose trigger is a file-size heuristic. Retiring it is cheaper and safer.
...
- `crates/slicer-wasm-host/src/binding.rs` — the `wasm_component` field doc comment only.
...
- `docs/adr/0015-...md`, `docs/adr/0020-...md` (§Decision item 1), `docs/04_host_scheduler.md` (`### Error Handling Policy`), `docs/DEVIATION_LOG.md` (missing-component dispatch defect row only).
