...
| Unknown subtype handling | Map to `PartSubtype::NormalPart` + `log::warn!` containing the unrecognized string and the substring `"downgrading to normal_part"`. | DEV-050 contract. |
| Missing sidecar handling | Return `HashMap::new()`; no warning. | DEV-051 contract; absence is the default for fixtures without sidecars. |
| Malformed sidecar handling | Return `HashMap::new()` + `log::warn!` containing `"treating all parts as normal_part"`. | DEV-051 contract. Loader must not fail. |
