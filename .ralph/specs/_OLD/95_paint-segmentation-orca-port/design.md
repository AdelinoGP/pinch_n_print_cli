...
- Exact functions, traits, manifests, tests, or fixtures expected to change: **see `task-map.md`** for the per-sub-step crosswalk to the roadmap's table. Highlights:
  ...
  - **`crates/slicer-runtime/src/prepass.rs`** — new insertion point for `host:paint_segmentation` between `shell_classification` and `support_geometry`; delete the old position of the existing `paint_segmentation_producer` invocation (line resolved by Step 1 dispatch; P94r retired the `PrePass::MeshSegmentation` block but did not move other stages).
...
- `[FWD]` — The exact line number of the existing `paint_segmentation_producer` invocation in `prepass.rs` may have drifted between roadmap-write and now. Sub-step 15 dispatch confirms. P94r's retirement removed the `PrePass::MeshSegmentation` driver block but did not shift any other stage.
