...
- Precondition: Steps 1, 2, 3a, 3b and 4 green. The retired infill-linker anchor label is absent; the surviving infill-linker neighbour-bookkeeping residual row carries the neighbour-bookkeeping residual and the code still says "no shortest-first ordering". `docs/07_implementation_status.md` has zero `TASK-311` hits. `crates/slicer-ir/src/resolved_config.rs` still says `OrcaSlicer: infill_anchor_max` on `infill_resolution`. ADR-0025 still carries `### Not yet ported from canonical`.
...
- Record remaining packet-local risk: the absent neighbour-trimming residual (DEV-110) and the solid-bucket behaviour move.
