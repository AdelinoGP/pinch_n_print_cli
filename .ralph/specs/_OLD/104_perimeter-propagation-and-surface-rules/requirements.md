...
- Both `lib.rs` files in `classic-perimeters` and `arachne-perimeters`: call `build_wall_flags` for inner walls in addition to outer (with `is_outer=false`); iterate per-vertex `is_bridge` via `point_in_any_polygon(&pt, region.bridge_areas())`; read `only_one_wall_top` and `only_one_wall_first_layer` from `_config`. **`Point3WithWidth` construction sites (T-024/T-025):** classic has NO inline `Point3WithWidth` literal — it emits per-vertex vertices through the shared `expolygon_to_path3d` helper (`crates/slicer-core/src/perimeter_utils.rs:125`), which already sets `flow_factor: 1.0, overhang_quartile: None` (lines 138-139). Arachne ALSO uses that helper AND has a second, inline `Point3WithWidth { … overhang_quartile: None, flow_factor: 1.0 }` for its variable-width path at `arachne-perimeters/src/lib.rs:428`. The sibling-roadmap O-T031 doc-comment is added to the `overhang_quartile: None` line at BOTH the helper (perimeter_utils.rs:139, covers classic + arachne helper path) and arachne's inline site (lib.rs:428). `Point3WithWidth` has `flow_factor: f32` and `overhang_quartile: Option<u8>` — NOT `is_bridge`/`tool_index` (those live on `WallFeatureFlags`).
...
<!-- Sub-top `only_one_wall_top` reduction was implemented this session via a `split_top_surfaces` carve (top_solid_fill-scoped); no deviation is registered for it. -->
...
- Refinements not captured in Given/When/Then:
  - `AC-3`'s WIT accessor name MUST be `overhang-areas`, not `overhang_regions`, because the data shape is `Vec<ExPolygon>` (already-projected XY footprints), not raw `OverhangRegion` structs. Naming convention follows `bridge-areas`. **Forward-dep (net-new field, NOT pre-existing):** the field `OverhangRegion.xy_footprint: Vec<ExPolygon>` does **not** exist in the current tree — `OverhangRegion` today is `{ id, facet_indices, max_angle_deg, needs_support }` (`crates/slicer-ir/src/slice_ir.rs:586`). It is added by **P106 (`106_overhang-pipeline-prepass-foundation`, O-T010)**, mirroring `BridgeRegion.xy_footprint` (the field that *does* exist at `slice_ir.rs:581`). Until P106 lands, this packet's `overhang_areas()` host populator returns `Vec::new()` (it does **not** reference the not-yet-existent field); the `xy_footprint`-intersection body is wired by P106/consumer packets once the field exists. AC-3-EMPTY pins the empty return as a regression bed.
...
- Likely temptation read: `crates/slicer-core/src/algos/mesh_analysis.rs` to see how `OverhangRegion.xy_footprint` is computed. **Skip** — that's sibling roadmap O-T010/P106 territory. The `OverhangRegion.xy_footprint` field is **net-new (added by P106)** and is absent from the current tree; this packet's `overhang_areas()` accessor therefore returns `Vec::new()` and does NOT read the field. When P106 lands the field, the intersection body is wired then — no further change to this packet's accessor signature is needed.
