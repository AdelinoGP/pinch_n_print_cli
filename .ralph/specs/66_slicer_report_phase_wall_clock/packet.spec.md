...
Fix the slicer report Phase Totals table so PerLayer displays actual phase wall-clock elapsed time alongside aggregate thread time, rather than conflating the two under a single "wall-clock, sum" label.

## Scope Boundaries

This packet corrects the slicer report HTML renderer and underlying collector so the Phase Totals table shows two distinct columns: actual phase wall-clock (`Wall (ms)`) and aggregate across threads (`Worker total (ms)`). The collector gains per-phase start/end timestamp recording; `SliceMeta` gains a `PhaseWallTimes` struct; the renderer displays both values. The fix is host-only — no WASM, IR schema, WIT boundary, scheduler rules, or OrcaSlicer parity.
