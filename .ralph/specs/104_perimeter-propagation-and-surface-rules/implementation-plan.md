...
- Files explicitly out-of-bounds for this step:
  ...
  - `crates/slicer-runtime/tests/contract/main.rs` — Step 2 registers the contract test entries.
...
- Objective: in `crates/slicer-core/src/perimeter_utils.rs`, rename `build_outer_wall_flags` → `build_wall_flags` and add `is_outer: bool` parameter; run same Material/FuzzySkin/boundary-type extraction on inner walls under `if !is_outer`; add `pub fn point_in_any_polygon(pt: &Point2, polys: &[ExPolygon]) -> bool` helper; write `inner_wall_material_boundary_tdd.rs` and `per_vertex_is_bridge_propagation_tdd.rs`; register both in `crates/slicer-runtime/tests/contract/main.rs`.
...
- Objective: in both `lib.rs` files, call `build_wall_flags(.., is_outer=false)` for inner walls; iterate per-vertex `is_bridge` via `point_in_any_polygon(&pt, region.bridge_areas())`; explicitly set `Point3WithWidth { …, overhang_quartile: None, flow_factor: 1.0, … }` with a doc-comment block citing sibling roadmap O-T031. Also add the contract test `inner_wall_boundary_type_tdd.rs` (AC-2b) and register it in `main.rs`.
...
- Task IDs:
  ...
  - Deviation for MED-2: `docs/DEVIATION_LOG.md` (`D-104-ONLY-ONE-WALL-TOP-SUBTOP`).
...
- Verification:
  ...
  - `rg -q 'D-104-ONLY-ONE-WALL-TOP-SUBTOP' docs/DEVIATION_LOG.md` — exit 0.
...
- AC-1, AC-2, AC-2b, AC-3, AC-4, AC-5, AC-6, AC-N1, AC-N2 verification commands all return PASS via worker dispatch.
...
- Note both sibling-roadmap silent dependencies in the closure log:
  ...
  - Sub-top `only_one_wall_top` reduction deferred — see `D-104-ONLY-ONE-WALL-TOP-SUBTOP`.
