...
### Step 1: Add `overhang_areas` + `surface_group` accessors to `SliceRegionView`
...
- Files explicitly out-of-bounds for this step:
  ...
  - Any `slicer-core` file (the source data fills these accessors from PrePass-committed IRs; the host reads them but does not edit `mesh_analysis.rs`).
...
- Exit condition: AC-3 verification grep passes; build green; guests not STALE.

### Step 2: Extend `build_wall_flags` for inner walls + bridge per-vertex propagation
...
- Objective: in both `lib.rs` files, call `build_wall_flags(.., is_outer=false)` for inner walls; iterate per-vertex `is_bridge` via `point_in_any_polygon(&pt, region.bridge_areas())`; explicitly set `overhang_quartile = None` and `flow_factor = 1.0` with a doc-comment block citing sibling roadmap O-T031.
- Precondition: Step 2 exit condition met; `cargo check --workspace --all-targets` clean.
- Postcondition: AC-1 + AC-2 + AC-6 verification commands pass (AC-1 and AC-2 now exercise the modules end-to-end, not just the helper).
- Files allowed to read (with line-range hints when > 300 lines):
  - Both perimeter modules' `lib.rs` — range-read `run_perimeters` body and the inner-wall emission loop.
- Files allowed to edit (≤ 3):
...
- Expected sub-agent dispatches:
  ...
  - "Run `cargo test -p slicer-runtime --test contract per_vertex_is_bridge_propagation_tdd`; return FACT pass/fail."
- Context cost: `M` (two-module rewrite)
- Authoritative docs:
...
- Verification:
  ...
  - `rg -q 'overhang_quartile.*None.*sibling roadmap' modules/core-modules/classic-perimeters/src/lib.rs` — exit 0.
  - `rg -q 'overhang_quartile.*None.*sibling roadmap' modules/core-modules/arachne-perimeters/src/lib.rs` — exit 0.
- Exit condition: AC-1, AC-2, AC-6 partial (only doc-comment greps; deviation log handled in Step 5) green.
...
- Objective: register the two config keys in both manifests; in both `lib.rs` files, read both keys via `_config.get_bool` per-call; clamp `wall_count = 1` when the corresponding gate fires; write `only_one_wall_top_tdd` (covers AC-4 + AC-N2) and `only_one_wall_first_layer_tdd` (covers AC-5).
...
- Files allowed to read (with line-range hints when > 300 lines):
  - Both perimeter modules' `lib.rs` — range-read `run_perimeters` head + wall-emission loop.
  - Both `.toml` manifests — full.
- Files allowed to edit (≤ 3, per-substep):
  - Sub-step 4a (manifests): `modules/core-modules/classic-perimeters/classic-perimeters.toml`, `modules/core-modules/arachne-perimeters/arachne-perimeters.toml`.
  - Sub-step 4b (consumers): `modules/core-modules/classic-perimeters/src/lib.rs`, `modules/core-modules/arachne-perimeters/src/lib.rs`.
  - Sub-step 4c (tests): `crates/slicer-runtime/tests/contract/only_one_wall_top_tdd.rs` (NEW), `crates/slicer-runtime/tests/contract/only_one_wall_first_layer_tdd.rs` (NEW).
- Files explicitly out-of-bounds for this step:
...
- Context cost: `M` (two manifests + two-module consumer wiring + two new tests)
- Authoritative docs:
  - `docs/15_config_keys_reference.md` — range-read §"Walls" to align manifest key format.
- OrcaSlicer refs:
...
- Task IDs:
  ...
  - Deviation for `T-024`: `docs/DEVIATION_LOG.md`.
- Objective: land the three doc edits + register the deferral deviation.
...
- Files allowed to read (with line-range hints when > 300 lines):
  ...
  - `docs/15_config_keys_reference.md` — range-read §"Walls".
  - `docs/DEVIATION_LOG.md` — range-read the most recent N entries to align format.
- Files allowed to edit (≤ 3):
...
- Verification:
  ...
  - `rg -q 'D-.*-OVERHANG-QUARTILE-NONE' docs/DEVIATION_LOG.md` — exit 0.
- Exit condition: all Doc Impact Statement greps pass; AC-6 fully green (doc-comment greps from Step 3 + deviation grep from this step).

## Per-Step Budget Roll-Up

| Step | Context Cost | Notes |
| --- | --- | --- |
| Step 1 | M | Three crates (SDK + WIT + host) — view-accessor plumbing. |
| Step 2 | M | Helper extension + two new tests; no module edits yet. |
| Step 3 | M | Two-module rewrite for per-vertex propagation. |
| Step 4 | M | Manifests + two-module gate logic + two new tests. |
| Step 5 | S | Three doc edits. |

Aggregate context cost: `M`. No single step is `L`. Per-step file edit count never exceeds 3 (Step 4 splits into sub-steps a/b/c).
