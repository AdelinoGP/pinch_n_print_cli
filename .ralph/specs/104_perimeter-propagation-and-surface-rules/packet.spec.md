...
- Forward dependencies (both packets are `status: draft` — do NOT treat as satisfied):
  - Packet `106_overhang-pipeline-prepass-foundation` — produces `OverhangRegion.xy_footprint` population via O-T010 (sibling roadmap). Until P106 lands, `overhang_areas()` returns an empty slice; this packet documents the forward-dep in design.md and the closure log, not as a blocker for P104 activation.
...
`only_one_wall_top` fires as a blanket gate ONLY for `top_shell_index() == Some(0)` (topmost layer — OrcaSlicer parity). For sub-top layers (`Some(1)`, `Some(2)`, etc.), the `split_top_surfaces`-equivalent `top_solid_fill`-scoped single-wall reduction is deferred. Deviation `D-104-ONLY-ONE-WALL-TOP-SUBTOP` is registered in `docs/DEVIATION_LOG.md` by this packet.
...
- `docs/DEVIATION_LOG.md` — CREATE `D-104-OVERHANG-QUARTILE-NONE` entry and `D-104-ONLY-ONE-WALL-TOP-SUBTOP` entry — `rg -q 'D-104-OVERHANG-QUARTILE-NONE' docs/DEVIATION_LOG.md && rg -q 'D-104-ONLY-ONE-WALL-TOP-SUBTOP' docs/DEVIATION_LOG.md`
