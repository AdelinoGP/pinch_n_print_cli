...
- ADR-0043 amendment is explicit: its `Decision` item 2 says Arachne wall-width keys are “plain mm floats, default 0.4, range [0.1, 2.0]”. This packet changes that contract for canonical parity, adds `D-185-ADR-0043-AMENDED` to `docs/DEVIATION_LOG.md`, and appends an amendment record to the ADR file.
- ADR-0014 amendment is explicit: its freshness list says “`slicer-core` and `slicer-helpers` are explicitly NOT tracked”. The current guest freshness contract tracks `slicer-core` as a universal guest dependency, so this packet adds `D-185-ADR-0014-AMENDED` to `docs/DEVIATION_LOG.md` and appends an amendment record to the ADR file.
...
- Rejected alternatives and reasons:
  ...
  - Editing `ResolvedConfig::to_config_map` — rejected; `DEV-100` verified it is not the barrier.
...
- `modules/core-modules/classic-perimeters/src/lib.rs` + `classic-perimeters.toml` - role: resolver consumer, D-105-FLOW-NOT-WIRED residual formula, overlap + one-wall-top; expected change: largest module diff
...
- `docs/DEVIATION_LOG.md` - role: deviation ledger; expected change: add `D-185-ADR-0043-AMENDED`, `D-185-ADR-0014-AMENDED`, and the next free DEV row for deferred flow-ratio controls, plus status updates for D-105-FLOW-NOT-WIRED, D-164-WALL-WIDTH-KEYS-NOT-FLOAT-OR-PERCENT, and DEV-100
- `docs/adr/0043-derive-arachne-bead-widths-from-wall-flows.md` - role: append-only amendment record for `D-185-ADR-0043-AMENDED`; expected change: preserve the original Decision item 2 and append the approved type/default amendment
- `docs/adr/0014-xtask-guest-discovery-via-validated-filesystem-walk.md` - role: append-only amendment record for `D-185-ADR-0014-AMENDED`; expected change: preserve the original freshness rule and append the current `slicer-core` tracking decision
...
- `docs/DEVIATION_LOG.md` - rows `D-105-FLOW-NOT-WIRED` (:24), `D-164-WALL-WIDTH-KEYS-NOT-FLOAT-OR-PERCENT` (:90), `DEV-100` (:95), `D-152-TOP-AREA-SOURCE` (:126) only - purpose: residual wording to close/amend
...
- `D-185-ADR-0043-AMENDED` quotes the contested ADR clause and records why canonical `float_or_percent`/auto-`0` behavior is intentional; no silent ADR drift is permitted.
- `D-185-ADR-0014-AMENDED` quotes the stale `slicer-core` freshness exclusion and records why the current universal-guest dependency check is required; no stale-guest deflection is permitted.
...
- Classic one-wall-top and the D-105-FLOW-NOT-WIRED infill-boundary formula will drift classic goldens; re-bless only after matrix tests pin the intended semantics.
