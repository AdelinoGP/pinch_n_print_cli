...
- Postcondition: both constants are `0.25` / `0.000625` in **both** `pipeline.rs` and `host.rs`; the two provenance comments no longer claim `meshfix_maximum_resolution = 0.05mm` / `meshfix_maximum_deviation = 0.005mm` and instead cite canonical `PrintConfigDef::init_fff_params` and `WallToolPaths::simplifyToolPaths`; the `maximum_extrusion_area_deviation` comment is **not touched by this step** — it was corrected out of band together with its value and the non-canonical formula it gated (`DEV-094`), so this step fixes **two** provenance comments, not three, and finding that third comment already accurate is the expected state rather than a missing precondition. **`Step 5a` files ONE new `DEV-###` row (the shared D-164 absent-key residual), not two**; do not file it from this step, and do not carry an ID forward from here — each is re-derived at the moment of writing. Because `arachne_params_from_config`'s two `unwrap_or(defaults.…)` fallbacks read `ArachneParams::default()` rather than local literals (verified against the tree), this step alone is what makes AC-8's assertion true; 2b adds the test that states it.
- Files allowed to read, with ranges when over 300 lines:
...
- Task IDs: `TASK-304`
- Objective: bring `docs/15_config_keys_reference.md` back in sync with the changed manifest schema, flip D-163 / D-164 / D-168 to `Closed` with D-168's provenance corrected, file **one** new `DEV-###` row — for the two D-164 residuals (the shared row this packet owns on behalf of packet 184 as well; see `packet.spec.md` §Doc Impact) — with its ID re-derived at the moment of writing, and file `D-185-ADR-0043-AMENDED` for the ADR amendment 5b performs. **There is no second row:** the `maximum_extrusion_area_deviation` divergence landed out of band as `DEV-094`, which already records the formula port, the derived `2e-6 mm²` threshold and the large-printer residual.
