...
| `TASK-146c` | Steps 4-5 | ADR-0002; packet 181's closed fatal-on-miss invariant | `crates/slicer-wasm-host/src/{host.rs,dispatch.rs}` | none | M+M | 12 `bindgen!` mods; per-stage instantiate; fatal-on-miss reasons; preserve `missing_component_is_fatal_for_all_five_stages` while moving the five arms |
