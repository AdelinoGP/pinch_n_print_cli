...
- Question: the deviation row owned by `164_per-stage-wit-packages-bulk` — its DEV id and current status line; scope: `docs/DEVIATION_LOG.md`; return: `FACT`; purpose: AC-8 closure (re-derive, never assume `per-stage WIT intermediate`).
