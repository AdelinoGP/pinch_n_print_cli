# Design: 116a-arachne-beading-propagation-and-junction-bands
...
- Packet-specific constraint: **A1 must not remove the π hack (`pipeline.rs:334`) or the 0.1× filter-dist fudge (`pipeline.rs:272-277`).** Those are Packet C's (`118`) scope, strictly after A2. A1's rewrite is gated on the centrality scheme the π hack sustains.
