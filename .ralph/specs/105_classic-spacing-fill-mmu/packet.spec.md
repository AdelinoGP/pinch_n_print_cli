...
- **AC-5. Given** the two additive IR additions composing the 4.3.0 bump — `LoopType::GapFill` + `ExtrusionRole::GapFill` (T-062b, gap-fill emission variants) AND the resurrected `bisector_edge_skip_mask` field on `SlicedRegion` (T-P96-C0, MMU per-color outer-wall fragmentation) — **when** `crates/slicer-ir/src/slice_ir.rs` is inspected post-packet, **then** `SlicedRegion` carries `pub bisector_edge_skip_mask: Vec<bool>` (flat per-edge mask indexed against `polygons`, per ADR-0013), the host's `paint_segmentation` populator fills it deterministically using the tie-break rule from `docs/specs/orca-mmu-perimeter-investigation.md` (NEW, authored in Step 1; default: lower color-ID owns the bisector edge), and `CURRENT_SLICE_IR_SCHEMA_VERSION` bumps from the live `4.3.0` to `4.4.0` (computed at activation from `CURRENT_SLICE_IR_SCHEMA_VERSION`). | `rg -q 'pub bisector_edge_skip_mask: Vec<bool>' crates/slicer-ir/src/slice_ir.rs && rg -q 'LoopType::GapFill' crates/slicer-ir/src/slice_ir.rs && rg -q 'ExtrusionRole::GapFill' crates/slicer-ir/src/slice_ir.rs && cargo test -p slicer-core --test paint_segmentation_bisector_mask_tdd -- --nocapture 2>&1 | tee target/test-output.log`
- **AC-6. Given** a 4-color cube painted-region fixture where adjacent color cells share bisector edges, **when** each perimeter module traces the outer wall per-cell, **then** edges where `bisector_edge_skip_mask[flat_idx] == true` are skipped (not traced) and edges with `false` are traced exactly once (flat index computed via `edge_offset_for_polygon(region, poly_idx) + edge_j`); the union of all per-color outer wall fragments covers the external perimeter exactly (no gap, no double-trace within ±0.01 mm); and the `external_contour` consumption path is removed from both modules (no remaining call to `region.external_contour()`). | `cargo test -p slicer-runtime --test integration mmu_bisector_dedup_tdd -- --nocapture 2>&1 | tee target/test-output.log && ! rg -q '\.external_contour\(\)' modules/core-modules/classic-perimeters/src/lib.rs modules/core-modules/arachne-perimeters/src/lib.rs`
...
- **AC-N3. Given** a single-color region (no paint, no bisector neighbors), **when** the host populates `bisector_edge_skip_mask`, **then** every element of the flat `Vec<bool>` is `false` (no skipped edges) and both perimeter modules trace every outer-wall edge once (count of distinct outer-wall extrusion sequences = 1, matching the unpainted baseline). | `cargo test -p slicer-runtime --test integration mmu_bisector_dedup_tdd single_color_unaffected_case -- --nocapture 2>&1 | tee target/test-output.log`

## Verification

- `cargo check --workspace --all-targets`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test -p slicer-runtime --test integration outer_inner_width_and_spacing_tdd thin_wall_emission_tdd gap_fill_emission_tdd mmu_bisector_dedup_tdd && cargo test -p slicer-core --test wall_sequence_reorder_tdd && cargo test -p slicer-core --test paint_segmentation_bisector_mask_tdd`
...
- `docs/02_ir_schemas.md` §"SlicedRegion" — document `bisector_edge_skip_mask` and 4.4.0 schema bump — `rg -q 'bisector_edge_skip_mask' docs/02_ir_schemas.md && rg -q '4\.4\.0.*bisector_edge_skip_mask|4\.4\.0.*GapFill' docs/02_ir_schemas.md`
