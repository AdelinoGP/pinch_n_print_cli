# Requirements: 118-arachne-angle-fudge-and-noncentral-regions
...
Packet 116a (A1) left the π-cap workaround (`pipeline.rs:334`,
...
This packet supersedes `D-116A-JUNCTION-BANDS` for the centrality-parameter
...
- **Deviation-log entry**: `D-118-ANGLE-FUDGE-NONCENTRAL` (new ID, addendum
  on `D-116A-JUNCTION-BANDS` noting C removes the π hack A1 left in place,
...
- **N1/N7 (junction geometry + BeadingPropagation)** — A1 (`116a`). C does not
  change junction generation.
- **N2/N4 (`perimeter_index` + `is_odd`)** — A2 (`116b`).
- **N3/N8 (transition ends + extra ribs)** — B (`117`). C does not touch
  `apply_transitions` or the `BeadingStrategy` trait extension.
- **N9–N13** — Packets D (`119`), E (`120`), F (`121`).
...
- `docs/DEVIATION_LOG.md` `D-116A-JUNCTION-BANDS` entry — read full;
...
| `rg -q 'D-118-ANGLE-FUDGE-NONCENTRAL' docs/DEVIATION_LOG.md` | Deviation log entry present | FACT pass/fail |
...
- **C must NOT extend the `BeadingStrategy` trait.** B (`117`) owns the trait
...
- **Deviation-log correction uses the supersession pattern** — new
  `D-118-ANGLE-FUDGE-NONCENTRAL` + addendum on `D-116A-JUNCTION-BANDS`. No
