# Implementation Plan: 118-arachne-angle-fudge-and-noncentral-regions
...
- Precondition: A2 (`116b`) is `status: implemented` — the canonical junction scheme is in place; the π hack is no longer load-bearing.
...
- Authoritative docs:
  ...
  - `docs/DEVIATION_LOG.md` `D-116A-JUNCTION-BANDS` entry — addendum target (Step 2 writes the addendum; Step 1 confirms the target exists).
- OrcaSlicer refs:
...
- Objective: Port `filterNoncentralRegions` (`SkeletalTrapezoidation.cpp:811-862`) as `filter_noncentral_regions` in `centrality.rs` (promote non-central gaps between same/±1-bead-count central regions within 0.4 mm = 4000 units back to central; copy bead counts across); call it unconditionally after `assign_bead_counts` in `pipeline.rs`. Write the dumbbell test (`arachne_filter_noncentral_regions.rs`, NEW). Re-baseline `centrality_*.json` fixtures. Add the `D-118-ANGLE-FUDGE-NONCENTRAL` deviation-log entry + `D-116A-JUNCTION-BANDS` addendum.
- Precondition: Step 1 is green (π hack + 0.1× fudge gone; configured angle threaded; N1 stays green).
- Postcondition: AC-2 (dumbbell single central region) passes. N1, N2, N3, N4 stay GREEN. `centrality` regression green (fixtures re-baselined). `D-118-ANGLE-FUDGE-NONCENTRAL` present.
- Files allowed to read (with line-range hints when > 300 lines):
...
- Files allowed to edit (≤ 3):
  ...
  - `docs/DEVIATION_LOG.md` (addendum only — new `D-118-ANGLE-FUDGE-NONCENTRAL` + one-line addendum on `D-116A-JUNCTION-BANDS`; no in-place edits)
...
- Verification:
  ...
  - `rg -q 'D-118-ANGLE-FUDGE-NONCENTRAL' docs/DEVIATION_LOG.md` — FACT pass.
- Exit condition: AC-2 passes; N1/N2/N3/N4 stay green; `centrality` regression green (fixtures re-baselined); `D-118-ANGLE-FUDGE-NONCENTRAL` present; `cargo check -p slicer-core --all-targets` + `cargo clippy -p slicer-core --all-targets -- -D warnings` pass; e2e closure delta recorded (record-only).
...
- `D-118-ANGLE-FUDGE-NONCENTRAL` present in `docs/DEVIATION_LOG.md` with addendum on `D-116A-JUNCTION-BANDS`.
