...
- Depends on: `181-dispatch-missing-component-handling` (TASK-297) — **must be IMPLEMENTED**. AC-12 consumes packet 181's real-component `postpass_gcode_boundary` repair and fatal-on-missing-component dispatch invariant; this packet does not re-open or re-implement DEV-087.
...
- `docs/DEVIATION_LOG.md` — **`DEV-087` is referenced as closed, never created or re-filed.** Packet 181 repaired the `postpass_gcode_boundary_tdd.rs` live-component/resource round-trip and made all five `MissingComponent` dispatch arms fatal. Verification grep: `rg -q '^\| DEV-087 ' docs/DEVIATION_LOG.md && rg -q 'postpass_gcode_boundary_tdd' docs/DEVIATION_LOG.md`
