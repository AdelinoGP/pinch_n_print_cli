...
- Depends on: `181-dispatch-missing-component-handling` (TASK-297) — **must be IMPLEMENTED**. AC-12 consumes packet 181's real-component `postpass_gcode_boundary` repair and fatal-on-missing-component dispatch invariant; this packet does not re-open or re-implement missing-component dispatch defect.
...
- `docs/DEVIATION_LOG.md` — the historical `per-stage WIT intermediate` and `missing-component dispatch defect` labels have no surviving rows after the purge. The current contract evidence remains the versioned-package plan and the five-stage fatal dispatch regression. Verification: `! rg -q '^\| per-stage WIT intermediate |^\| missing-component dispatch defect ' docs/DEVIATION_LOG.md && rg -q 'versioned WIT package' docs/specs/adr-0045-per-stage-wit-packages-plan.md && rg -q 'missing_component_is_fatal_for_all_five_stages' crates/slicer-runtime/tests/contract/dispatch_missing_component_tdd.rs`
