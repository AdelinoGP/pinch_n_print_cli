...
This packet groups a single task ID (`TASK-306`). The crosswalk is carried anyway because `TASK-306` is **newly allocated** and has zero hits in `docs/07_implementation_status.md` at authoring time — registering it there is packet work (AC-14, Step 5), and this table is the mapping that registration must reproduce. Its backlog authority is not `docs/07` but `docs/specs/deviation-backlog-remediation-plan.md` §Packet Queue **row 8b** (`.ralph/specs/187-custom-gcode-injection-registry` · `custom-G-code injection deviation` (layer-scoped points) · tranche `T3` · **depends on #8a**), added by that plan's "Queue amendment (2026-07-25b)" note, which decomposed row 8 into 8a/8b/8c because the plan rated it aggregate `L`.
...
| `TASK-306` | `Step 5` | `docs/DEVIATION_LOG.md`, `docs/07_implementation_status.md` | none (doc only) | `GCode.cpp` — `GCode::process_layer`, `GCode::generate_timelapse_gcode` | `S` | One surviving packet-187 residual row carries all four accepted parity residuals and cites `TASK-306`; the deleted aggregate custom-G-code label is not recreated; `TASK-306` is registered outside the generated block. |
...
| --- | --- | --- |
| retired `custom-G-code injection deviation` label | **Remains absent.** This packet's surviving residual row cites `TASK-306`; packet 188 carries the remaining residuals. | Step 5 — the packet-187 residual probe |
...
`140_lightning-module-rewrite` is the only `active` packet — **verified with an anchored match**, `rg -l '^status: active' .ralph/specs/*/packet.spec.md`. (An unanchored search for the word "active" matches prose in several draft packets and produces false positives; packet 190's frontmatter read `draft` at the time.) This packet is `status: implemented` in this commit; its closure does not unblock the backlog's main path. Packet 188 still carries the toolchange-, role- and unreachable-site remainder; packet 140 must clear before the custom-G-code trilogy is fully activated. `cargo xtask check-deviations` and the per-step gates continue to gate downstream work; the status flip does not exempt the post-purge residual ledger from those gates.
