...
- Exit condition: AC-1, AC-3, AC-4, AC-5, AC-6, AC-8, AC-N1, AC-N2 (under the renamed `malformed_layer_marker_warns_and_uses_prior_z_per_adr_0051` test, the AC-N2 probe's `--exact` filter) and AC-N3 all print PASS.
...
### Step 5: Reconcile the retired aggregate label, file the residual row, register `TASK-306`

- Task IDs: `TASK-306`
- Objective: preserve the post-purge absence of the deleted aggregate label; file one residual row carrying **all four** accepted parity residuals — AC-13 items (a)+(a2) the `max_layer_z` dead-write and the *no*-divergence finding at the other two sites, (b) the unported BBL timelapse path, (c) the six unmodelled `layer_change_gcode` variables, (d) the `change_layer` interleaving difference; hand-add the `TASK-306` backlog row outside the generated block and regenerate that block.
- Precondition: Steps 1-4 complete.
- Postcondition: one surviving packet-187 residual row cites `TASK-306` and is still `Open`; it names `is_BBL_Printer`, `generate_timelapse_gcode`, `timelapse_inline_photo` and `max_layer_z`; `TASK-306` resolves in `docs/07_implementation_status.md`.
- Files allowed to read, with ranges when over 300 lines:
  - `docs/DEVIATION_LOG.md` — **long; delegate or range-read.** The surviving packet-187 residual row and the two most recent rows for format, plus a re-derivation of the highest `DEV-###`.
...
- Verification:
  ...
  - `bash -c 'python3 -c "import io; L=io.open(r\"docs/DEVIATION_LOG.md\",encoding=\"utf-8\").read().splitlines(); r=[l for l in L if l.startswith(chr(124)+chr(32)+\"D-285-ADR-0051-AMENDED\")]; print(\"PASS\" if r and \"D-285-ADR-0051-AMENDED\" in r[0] and \"ADR-0051\" in r[0] and \"packet 187\" in r[0] else (\"FAIL: no D-285-ADR-0051-AMENDED row\" if not r else \"FAIL: D-285 row does not name the ADR or packet\"))"'` — **the new amendment row probe.** Re-derive the highest `D-` at the moment of writing (`rg -o '^\| D-[0-9]{3}' docs/DEVIATION_LOG.md | sort -u | tail -1`); the row is the *next* free number after the highest, and a sibling packet may have filed one in parallel. Token set is fixed: the row must name the slot (`D-285-ADR-0051-AMENDED`), the ADR (`ADR-0051`), and the packet (`packet 187`).
  - `bash -c 'python3 -c "import io; s=io.open(r\"docs/adr/0051-gcode-marker-contract-ownership.md\",encoding=\"utf-8\").read(); has_section=\"## Amendment — 2026-08-01 (packet 187)\" in s; quotes_78 = \"A consumer must fail loudly on an unrecognised stream, never guess.\" in s; quotes_80 = \"a fatal module error naming the\" in s; print(\"PASS\" if has_section and quotes_78 and quotes_80 else \"FAIL: amendment section missing, or the original lines 78/80 are not quoted verbatim\")"'` — **the ADR amendment probe.** Confirms the section heading exists, that the original obligation #3 heading line is quoted verbatim, and that the original "fatal module error" prose is quoted verbatim. The replacement text is verified by reading the section in the review pass; a strict-token check on the replacement would be brittle.
- Exit condition: AC-13, AC-14, the packet-187 residual probe, the `D-285-ADR-0051-AMENDED` row probe, and the ADR amendment probe all print PASS.
...
- The deleted aggregate custom-G-code row stays absent — packet 188 carries the toolchange-, role- and unreachable-site remainder in surviving residual rows. Do not recreate it.
