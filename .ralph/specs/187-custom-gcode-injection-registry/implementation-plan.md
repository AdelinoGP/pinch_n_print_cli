...
- Task IDs: `TASK-306`
- Objective: add `InjectionSite`, `InjectionPoint`, `INJECTION_POINTS` (five entries), `LayerContext` and `ERR_MALFORMED_LAYER_MARKER`; rewrite `run_gcode_postprocess` to resolve every template through the table against a **per-site** lookup; splice `before_layer_change_gcode`, `time_lapse_gcode` and `layer_change_gcode` immediately after each layer's `;HEIGHT:` marker, in that order.
...
- Verification:
  ...
  - `bash -c 'rg -q "ERR_MALFORMED_LAYER_MARKER" modules/core-modules/machine-gcode-emit/src/lib.rs && echo PASS || echo "FAIL: ERR_MALFORMED_LAYER_MARKER does not exist - the layer-marker contract is unenforced"'`
...
- Record remaining packet-local risk: the module now depends on host-emitted comment text (`;LAYER_CHANGE` / `;Z:` / `;HEIGHT:`); confirm `ERR_MALFORMED_LAYER_MARKER` is exercised by a green AC-N2 and that the dependency is stated in the module's doc comment.
