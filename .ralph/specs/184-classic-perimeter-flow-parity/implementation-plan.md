...
- Task IDs: `TASK-303`
- Objective: regenerate the `docs/15_config_keys_reference.md` generated block without hand-editing its markers; correct the narrative for the retyped wall-width and top-surface keys; close the flow-spacing integration gap and classic top-surface remainder while partially closing the wall-width config-type gap; cross-reference the single shared `parse_percent_default` residual owned by packet 185; hand-add `TASK-303` outside the generated status block; and run `cargo xtask check-deviations` to regenerate that block.
- Precondition: Steps 1-7 complete; all code and test ACs PASS.
- Postcondition: `cargo xtask gen-config-docs --check` and `cargo xtask check-deviations --check` both exit `0`; the post-purge ledger retains `D-152-TOP-AREA-SOURCE`, `DEV-101`, and `DEV-102` evidence without recreating the retired shared residual label; `TASK-303` is in `docs/07_implementation_status.md`.
- Files allowed to read, with ranges when over 300 lines:
...
- Verification:
  ...
  - `bash -c '! rg -q "parse_percent_default" docs/DEVIATION_LOG.md && rg -q "^\| DEV-101 " docs/DEVIATION_LOG.md && echo PASS || echo "FAIL: the retired parser row reappeared or the surviving width residual is missing"'` — FACT PASS/FAIL: the post-purge ledger is checked against current semantic evidence rather than a deleted row.
  - `bash -c 'rg -q "float_or_percent|auto-`0`" docs/adr/0043-derive-arachne-bead-widths-from-wall-flows.md && echo PASS || echo "FAIL: ADR-0043 wall-width amendment evidence is missing"'` — FACT PASS/FAIL: the current wall-width contract is recorded in the ADR.
...
- Exit condition: AC-12 PASS, both xtask `--check` gates exit `0`, the surviving `D-152`/`DEV-101`/`DEV-102` evidence is present, the retired shared residual row is not recreated, the branch taken is recorded in the completion note, and `TASK-303` is present.
...
- The post-purge ledger carries the surviving `D-152-TOP-AREA-SOURCE`, `DEV-101`, and `DEV-102` records. The historical `parse_percent_default` shared row and the old packet-local labels are not recreated; the wall-width contract is carried by ADR-0043's amendment. **No new ADR is authored for the wall-width divergence.**
