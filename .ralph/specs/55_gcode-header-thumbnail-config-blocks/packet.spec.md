...
- **Given** a `slicer-cli` invocation that produces `out.gcode` with `--thumbnail fixtures/test_thumb.png`, **when** the file is scanned, **then** `HEADER_BLOCK_START`, `THUMBNAIL_BLOCK_START`, `THUMBNAIL_BLOCK_END`, `CONFIG_BLOCK_START`, and `CONFIG_BLOCK_END` all appear exactly once. | `cargo test -p slicer-host --test gcode_header_thumbnail_config_blocks_tdd -- sentinels_present_with_thumbnail --nocapture`
...
- **Given** a `--thumbnail fixtures/test_thumb.png` invocation, **when** the bytes between `THUMBNAIL_BLOCK_START` and `THUMBNAIL_BLOCK_END` are stripped of the `; ` line prefix, concatenated, and Base64-decoded, **then** the result begins with the PNG magic `89 50 4E 47 0D 0A 1A 0A` and equals `fixtures/test_thumb.png` byte-for-byte. | `cargo test -p slicer-host --test gcode_header_thumbnail_config_blocks_tdd -- thumbnail_roundtrip_matches_input --nocapture`
- **Given** a `--thumbnail fixtures/test_thumb.png` invocation, **when** the Base64 payload is inspected, **then** each line between the sentinels begins with `; ` and the Base64 portion of each line is ≤ 76 characters (OrcaSlicer wraps at 76, leaving room for the `; ` prefix to stay under 80 columns). | `cargo test -p slicer-host --test gcode_header_thumbnail_config_blocks_tdd -- thumbnail_base64_chunking_orca_parity --nocapture`
...
- **Given** a `--thumbnail fixtures/not_a_png.bin` invocation where the file is non-empty but lacks the PNG magic header, **when** the slicer is run, **then** it exits non-zero with a `thumbnail_path: invalid PNG magic` diagnostic and produces no `.gcode` output. | `cargo test -p slicer-host --test gcode_header_thumbnail_config_blocks_tdd -- rejects_non_png_thumbnail --nocapture`
