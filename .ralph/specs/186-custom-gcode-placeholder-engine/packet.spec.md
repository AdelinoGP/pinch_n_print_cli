...
- `docs/DEVIATION_LOG.md` — long; the `DEV-085` row and the residual row only, plus a re-derivation of the highest `DEV-###`. Delegate; never load whole. Both rows are single very long lines.
...
- `docs/DEVIATION_LOG.md`, the existing `DEV-085` row — stays `Open` (187 and 188 carry the rest). Two measured errors were corrected in place: it said "2 of OrcaSlicer's 15" where `PrintConfigDef::init_fff_params` registers **13 `coString` plus 3 `coStrings`** — 16, not 15, and three are per-filament vectors rather than scalars; and it said "All twelve appear only as inventory rows in `docs/ORCA_CONFIG_REFERENCE.md`" where `filament_change_extrusion_role_gcode` and `process_change_extrusion_role_gcode` have **zero** occurrences in that file. The probe is row-scoped, reports **which** clause failed, and **strips backticks before matching** — an earlier draft paired a literal-substring probe here with a `13 .coString` regex in `implementation-plan.md`, where the `.` matches a backtick, making the two mutually exclusive and closure unreachable. Both places now run this identical command: `bash -c 'python3 -c "import io; p=chr(124); b=chr(96); L=io.open(r\"docs/DEVIATION_LOG.md\",encoding=\"utf-8\").read().splitlines(); r=[l for l in L if l.startswith(p+chr(32)+\"DEV-085\")]; row=(r[0] if r else \"\").replace(b,\"\"); miss=[t for t in (\"13 coString\",\"3 coStrings\",\"filament_change_extrusion_role_gcode\") if t not in row]; print(\"PASS\" if r and not miss else (\"FAIL: no DEV-085 row\" if not r else \"FAIL: DEV-085 row still missing \"+str(miss)))"'`
