...
### Step 6: File the residual deviation, correct `DEV-085`, register `TASK-305`

- Task IDs: `TASK-305`
- Objective: maintain the residual `DEV-###` row enumerating the **eight** unresolvable macros and their canonical counterparts, the domain asymmetry, and the warn-and-pass policy; record the closed `D-<n>-ADR-0050-AMENDED` row; correct the two measured errors in the existing `DEV-085` row while leaving it `Open`; hand-add the `TASK-305` backlog row outside the generated block and regenerate that block.
...
- Postcondition: the residual row exists and names `print_time_estimate_s`, `total_layer_count`, `print_bed_max`, `num_extruders`, `max_layer_z`, `log_warn`, `reverted` and `check_placeholder_parser_failed`; the `DEV-085` row states 13 `coString` + 3 `coStrings`; `TASK-305` resolves in `docs/07_implementation_status.md` with warn-and-pass text.
- Files allowed to read, with ranges when over 300 lines:
...
- Exit condition: AC-13 and AC-14 print PASS, and the `DEV-085` correction probe passes.
