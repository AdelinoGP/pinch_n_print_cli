...
- Maps to `TASK-265` (IR + role/claim + planner emission) and `TASK-266` (traditional-support doc).
...
- Sub-agent dispatches:
  - "Locate `should_emit` (or the post-74710fa role-claim dispatcher) in `crates/slicer-sdk/src/views.rs`; return SNIPPETS ≤ 30 lines showing the `match role` arms and surrounding context."
  - "Locate `SupportPlanIR.schema_version` literal value in `crates/slicer-ir/src/slice_ir.rs`; return FACT (the SemVer value)."
  - "Locate `SupportPlanEntry`, `SupportPlanIR`, `Point3WithWidth`, `ExtrusionRole` definitions in `crates/slicer-ir/src/slice_ir.rs`; return LOCATIONS file:line."
  - "Locate `ExtrusionRole` mirror in `crates/slicer-schema/wit/deps/ir-types.wit` (if present); return SNIPPETS ≤ 20 lines."
  - "Locate the placeholder raft block in `modules/core-modules/support-planner/src/lib.rs` (was lines 381-423 pre-fca3731); return LOCATIONS ≤ 5 entries."
  - "Summarize OrcaSlicer `generate_raft_base` from `OrcaSlicerDocumented/src/libslic3r/Support/SupportCommon.cpp`; return SUMMARY ≤ 200 words."
...
- Verification: implementer can recite (a) the dispatcher location, (b) the current schema_version, (c) the placeholder block location, (d) WIT mirror status.
...
### Step 3: Add `claim:raft-fill` arm to `should_emit`; update WIT `ExtrusionRole` mirror if applicable; rebuild guests

- Task IDs: `TASK-265`
- Files allowed to read:
  - `crates/slicer-sdk/src/views.rs` — `should_emit` location
  - `crates/slicer-schema/wit/deps/ir-types.wit` — if WIT mirror exists
- Files allowed to edit (≤ 3):
  - `crates/slicer-sdk/src/views.rs`
  - `crates/slicer-schema/wit/deps/ir-types.wit` (if mirror exists)
...
- Context cost: `M`
- Verification: AC-3, AC-N2 PASS; workspace compiles; guests fresh.
- Exit condition: role/claim machinery live.

### Step 4: Replace placeholder raft block with `RaftPlan` emission in `support-planner`

- Task IDs: `TASK-265`
- Files allowed to read:
  - `modules/core-modules/support-planner/src/lib.rs` — placeholder block + `plan_for_object` context
  - existing config-read site for `support_raft_layers` and friends
- Files allowed to edit (≤ 3):
  - `modules/core-modules/support-planner/src/lib.rs`
- Files out-of-bounds: traditional-support (Step 6), docs (Step 7).
- Sub-agent dispatches:
  - "Find tests that reference the placeholder raft single-point in `modules/core-modules/support-planner/tests/`; return LOCATIONS." — purpose: migrate or invalidate.
  ...
  - "Run AC-6 grep; return FACT pass/fail." — purpose: placeholder gone.
- Context cost: `M`
- Verification: AC-6 PASS; planner compiles; existing tests pass.
- Exit condition: placeholder gone; new emission in place.

### Step 5: Author `raft_plan_emission_tdd.rs` integration tests (AC-4, AC-5, AC-N1)
...
- Sub-agent dispatches:
  ...
  - "Run `cargo xtask build-guests --check`; return FACT clean / STALE."
- Context cost: `M`
- Verification: AC-4, AC-5, AC-N1 PASS.
- Exit condition: integration coverage in place.
...
| Step 4 | M | Planner placeholder removal + new emission. |
| Step 5 | M | Integration tests. |
...
- AC-1 through AC-10 + AC-N1 + AC-N2 all PASS.
- `docs/02_ir_schemas.md` updated; AC-10 grep PASS.
- `docs/07_implementation_status.md` marks `TASK-265` and `TASK-266` `[x]` (via worker dispatch).
...
- Confirm gate commands green: `cargo xtask build-guests --check`, `cargo build --workspace`, the three new test files, `cargo clippy --workspace -- -D warnings`.
- Mark `TASK-265` and `TASK-266` `[x]`; transition `packet.spec.md` to `status: implemented`.
