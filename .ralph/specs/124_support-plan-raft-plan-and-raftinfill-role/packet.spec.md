...
- **AC-6. Given** the placeholder raft block previously at `modules/core-modules/support-planner/src/lib.rs:381-423`, **when** searched, **then** the per-region duplicated single-point at `(0.0, 0.0, raft_z)` emission is GONE — no longer present in any code path. | `! rg -q 'Point3WithWidth \{[\s\S]{0,30}x: 0\.0,[\s\S]{0,30}y: 0\.0,[\s\S]{0,30}z: raft_z' modules/core-modules/support-planner/src/lib.rs`
