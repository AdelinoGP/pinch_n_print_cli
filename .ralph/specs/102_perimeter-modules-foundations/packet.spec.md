...
- **AC-1. Given** a fresh workspace, **when** `cargo check --workspace --all-targets` runs after the new `slicer-core::perimeter_utils` module is created, **then** the module compiles and exports the symbols `build_outer_wall_flags`, `find_all_transitions`, `extract_tool_index`, `default_feature_flags`, `expolygon_to_path3d`, `generate_seam_candidates`, and the constant `BASE_SPEED`. (Per deviation **D-102-AC1-HELPER-CONSOLIDATION**, the originally-named `has_adjacent_material_change` / `find_adjacent_tool` were consolidated into `find_all_transitions`.) The grep checks **every** required symbol individually so a missing export fails the gate (not vacuous alternation). | `cargo check -p slicer-core --all-targets 2>&1 | tee target/test-output.log && for s in build_outer_wall_flags find_all_transitions extract_tool_index default_feature_flags expolygon_to_path3d generate_seam_candidates BASE_SPEED; do rg -q "pub (fn\|const) $s\b" crates/slicer-core/src/perimeter_utils.rs || { echo "MISSING $s"; exit 1; }; done`
- **AC-2. Given** both `classic-perimeters/src/lib.rs` and `arachne-perimeters/src/lib.rs`, **when** the migration to consume `slicer-core::perimeter_utils` completes, **then** neither file contains its own definition of any of `build_outer_wall_flags`, `find_all_transitions`, `extract_tool_index`, `default_feature_flags`, `expolygon_to_path3d`, or `generate_seam_candidates` (each must be a `use` import, not a local `fn`). | `! rg -q '^fn (build_outer_wall_flags|find_all_transitions|extract_tool_index|default_feature_flags|expolygon_to_path3d|generate_seam_candidates)' modules/core-modules/classic-perimeters/src/lib.rs modules/core-modules/arachne-perimeters/src/lib.rs`
- **AC-3. Given** the widened IR, **when** `WallBoundaryType::MaterialBoundary` is constructed for a polygon with three transitions across four distinct tool indices `[1, 2, 3, 1]`, **then** the variant carries a `Vec<MaterialBoundarySegment>` of length 3 in clockwise order, each segment naming `near_tool` + `far_tool` for that transition (not just the first one), and `CURRENT_SLICE_IR_SCHEMA_VERSION` bumps to `4.2.0`. (The test now also asserts the schema version and the length-3 segment case directly via `schema_version_is_4_2_0` / `three_transition_polygon_carries_three_segments`. The grep uses `rg -U` because the constant is formatted across multiple lines.) | `cargo test -p slicer-ir --test material_boundary_widening_tdd -- --nocapture 2>&1 | tee target/test-output.log && rg -U -q 'CURRENT_SLICE_IR_SCHEMA_VERSION: SemVer = SemVer \{\s*major: 4,\s*minor: 2,\s*patch: 0' crates/slicer-ir/src/slice_ir.rs`
...
- `docs/15_config_keys_reference.md` — reconcile the manifest-vs-code defaults for `wall_count`, `outer_wall_speed`, `inner_wall_speed`. Note: the doc uses a markdown-table row format (`| \`wall_count\` | int | \`<N>\` | … |`), not `key: value`. The implementer MUST replace the literal default cells for these three rows so they equal the post-reconcile values chosen per design.md §Open Questions `[FWD]` (manifest-wins default = `wall_count = 3`, `outer_wall_speed = 30.0`, `inner_wall_speed = 45.0`). Verification: confirm each of the three rows in the `classic-perimeters` and `arachne-perimeters` sections matches the chosen value via targeted `rg` greps written at implementation time.
...
- **D-102-AC1-HELPER-CONSOLIDATION** — [AC-1] Specified: `perimeter_utils` exports `has_adjacent_material_change` and `find_adjacent_tool` | Implemented: replaced both with `find_all_transitions` that returns `Vec<MaterialBoundarySegment>` covering all transitions | Reason: the old two-function API was the lossy single-transition behavior T-013/T-014 fixes. Exporting the old functions alongside the new one would create a footgun. Both modules now call `build_outer_wall_flags` which internally uses `find_all_transitions`; the old functions have zero call sites.

- **D-102-DOC-DUPLICATE-HEADING-RESOLVED** — [Doc Impact Statement] During P102 Step 4, a `## PerimeterOutputBuilder failure modes` section was added at line 541 of `docs/05_module_sdk.md`. A colliding section also added by this packet at line 1165 carried the same heading. Resolved by merging both into a single canonical section at line 541, retaining the prose intro from 541 and the table format from 1165. Verification: `[ $(rg -c '^## PerimeterOutputBuilder failure modes' docs/05_module_sdk.md) -eq 1 ]` returns 0.
