...
**FORWARD-DEP on P108:** P108/T-090 deletes the old fake `arachne-perimeters/` (a 512-line iterative-inset approximation that is NOT real Arachne). P110/T-205 then creates a FRESH `arachne-perimeters/` skeleton for real Arachne — new directory, new manifest, empty `LayerModule` impl. T-205 must not activate until the directory is confirmed absent (P108 `status: implemented`).
...
- `docs/adr/0023-arachne-port-strategy.md` (NEW) — ADR documenting boostvoronoi selection + degeneracy strategy. Cross-references D-7 CLOSED entry (already in roadmap). (Slot 0023: tree highest is 0021; P106 claims 0022.)
...
- M1 implementation packets (P102..P108) — FORWARD-DEPs; P104/P105/P108 are `status: draft`. P108 must be `status: implemented` before T-205 activates.
- M1 verification (P109) — `status: draft` — separate predecessor.
...
- `docs/specs/orca-mmu-perimeter-investigation.md` — FORWARD-DEP: this file is planned by T-P96-A0 in P105 but is ABSENT from the tree (verified). The implementer MUST verify it exists before Step 5; if absent, delegate a `MultiMaterialSegmentation.cpp` LOCATIONS dispatch to get the Arachne MMU citations directly.
...
- Cross-step invariant: Steps 1–5 are slicer-core-internal (no module changes). Step 6 CREATES the new skeleton (new workspace member). The PRECONDITION for Step 6 is that P108 is `status: implemented` and `! test -d modules/core-modules/arachne-perimeters` passes — do not proceed to Step 6 until this is confirmed.
