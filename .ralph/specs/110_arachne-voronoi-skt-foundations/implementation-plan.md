...
## Step 1 — Write ADR-0010 + Close D-7

- **Tasks:** T-200.
- **Objective:** Draft `docs/adr/0010-arachne-port-strategy.md` recording boostvoronoi as the Voronoi crate, pure-Rust constraint, degeneracy strategy, and pinned version. Close D-7 in `docs/14_deviation_audit_history.md`.
- **Precondition:** None.
- **Postcondition:** `docs/adr/0010-arachne-port-strategy.md` exists; `D-7: CLOSED` line present in deviation audit history; ADR records boostvoronoi version + license note (BSL-1.0) + the `epsilon_offset` hazard.
- **Files allowed to read:** `docs/adr/0009-perimeter-module-scope.md` (template), `docs/14_deviation_audit_history.md` (full — small file), `docs/08_coordinate_system.md` (full), `docs/01_system_architecture.md` (§slicer-core).
- **Files allowed to edit:** `docs/adr/0010-arachne-port-strategy.md` (NEW), `docs/14_deviation_audit_history.md`.
...
- **Narrow verification:** `rg -q 'D-7.*CLOSED.*boostvoronoi' docs/adr/0010-arachne-port-strategy.md && rg -q 'D-7.*CLOSED' docs/14_deviation_audit_history.md`.
- **Cheapest falsifying check:** `test -f docs/adr/0010-arachne-port-strategy.md && rg -q 'epsilon_offset' docs/adr/0010-arachne-port-strategy.md` returns success.

## Step 2 — Voronoi Wrapper (T-201)

- **Tasks:** T-201.
- **Objective:** Add `boostvoronoi` dependency; create `crates/slicer-core/src/voronoi.rs` with `voronoi_from_segments`, `Segment`, `HalfEdgeGraph`, `VoronoiError`; write 3 stress fixture tests + 1 negative test.
- **Precondition:** Step 1 done (ADR pins version).
- **Postcondition:** `cargo test -p slicer-core voronoi` green; AC-2 + AC-3 + AC-N1 falsifiable; `cargo check --workspace --all-targets` green.
- **Files allowed to read:** `docs/adr/0010-arachne-port-strategy.md` (Step 1 output), `crates/slicer-core/src/lib.rs` (current `pub mod` set), `crates/slicer-core/src/geometry.rs` (existing `Point2` shape from P103/T-045 — line range only).
...
- **Context cost:** M.
- **Authoritative docs:** `docs/adr/0010-arachne-port-strategy.md` (Step 1 output), https://docs.rs/boostvoronoi/.
...
- **Files allowed to read:** `crates/slicer-core/src/polygon_ops.rs` (existing, from P103/T-040) — for `offset2_ex`, `opening_ex`; `crates/slicer-core/src/polygon_tree.rs` (P103/T-043) — for containment; `docs/specs/orca-mmu-perimeter-investigation.md` (full ≤ 200 lines, from P102/T-P96-A0).
...
## Step 6 — `arachne-perimeters` Module Skeleton (T-205) + DAG Validation (AC-N2)

- **Tasks:** T-205 + AC-N2 test.
- **Objective:** Create `modules/core-modules/arachne-perimeters/` with Cargo.toml, `arachne-perimeters.toml` manifest (`incompatible-with` against both `classic-perimeters` AND `variable-width-perimeters`), `src/lib.rs` placeholder `run_perimeters`, and `wit-guest/` shim. Add to workspace `Cargo.toml` members. Write the AC-N2 DAG-rejection test.
- **Precondition:** Steps 1–5 done. `cargo xtask build-guests --check` reports CLEAN before this step (per CLAUDE.md §"Guest WASM Staleness").
- **Postcondition:** `cargo xtask build-guests --check` reports CLEAN after Step 6's edits and a rebuild via `cargo xtask build-guests`; `cargo test -p slicer-runtime --test contract dag_rejects_arachne_and_classic_coexistence` green; AC-8 + AC-N2 falsifiable.
- **Files allowed to read:** `modules/core-modules/classic-perimeters/Cargo.toml`, `modules/core-modules/classic-perimeters/classic-perimeters.toml`, `modules/core-modules/classic-perimeters/src/lib.rs` (lines 1–50 only — `#[slicer_module]` pattern), `crates/slicer-runtime/tests/contract/` (Glob for existing dag-validation tests).
- **Files allowed to edit:** `modules/core-modules/arachne-perimeters/Cargo.toml` (NEW), `modules/core-modules/arachne-perimeters/arachne-perimeters.toml` (NEW), `modules/core-modules/arachne-perimeters/src/lib.rs` (NEW), `modules/core-modules/arachne-perimeters/wit-guest/` (NEW), `Cargo.toml` (workspace members), `crates/slicer-runtime/tests/contract/dag_validation.rs` (EDIT or CREATE).
- **Expected sub-agent dispatches:** ONE `cargo xtask build-guests` (rebuild all guests after adding new module). ONE `cargo test -p slicer-runtime --test contract` — return FACT pass/fail.
- **Context cost:** M.
- **Authoritative docs:** `docs/03_wit_and_manifest.md` §"Module Manifest TOML" + §"incompatible-with"; `docs/05_module_sdk.md` §"#[slicer_module]".
- **OrcaSlicer refs:** None.
- **Narrow verification:** `cargo xtask build-guests --check 2>&1 | tee target/test-output.log && cargo test -p slicer-runtime --test contract dag_rejects_arachne_and_classic_coexistence 2>&1 | tee target/test-output.log`.
- **Cheapest falsifying check:** `rg -q 'incompatible-with.*classic-perimeters' modules/core-modules/arachne-perimeters/arachne-perimeters.toml && rg -q 'incompatible-with.*variable-width-perimeters' modules/core-modules/arachne-perimeters/arachne-perimeters.toml`.
...
- `cargo test -p slicer-runtime --test contract dag_rejects_arachne_and_classic_coexistence 2>&1 | tee target/test-output.log` green.
