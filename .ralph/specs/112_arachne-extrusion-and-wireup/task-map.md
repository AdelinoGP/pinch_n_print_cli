...
| T-233 | Update `docs/01_system_architecture.md` Tier-2 `Layer::Perimeters`: name the real Arachne pipeline "real Arachne (Voronoi + SkeletalTrapezoidation + BeadingStrategy stack)" citing P112 (no "iterative-inset" caveat exists to remove — P108 already cleaned it) | Phase 13 | Step 11 | pending |
| T-234 | Final full-suite run via gated `cargo xtask test --workspace` (M2 closure ceremony per CLAUDE.md §"Test Discipline" workspace-test exception; fires guest-WASM freshness check) | Phase 13 | Step 12 | pending |
