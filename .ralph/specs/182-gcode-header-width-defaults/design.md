...
- OrcaSlicer comparison: **no canonical source is consulted, but canonical is not irrelevant — the divergence is inherited, not introduced.** Upstream registers both wall-width keys as `coFloatOrPercent` with default `0` (auto → `1.125 × nozzle_diameter` = `0.45` mm at a 0.4 mm nozzle); PnP's plain-float `0.4` cannot express auto. That gap is already recorded by the **open** row `D-164-WALL-WIDTH-KEYS-NOT-FLOAT-OR-PERCENT` in `docs/DEVIATION_LOG.md`, and packet `184-classic-perimeter-flow-parity`'s `[FWD-1]` is the decision that deliberately holds the resolved fallback at `0.4` rather than adopting canonical's auto default. This packet **inherits** that decision and does not re-open it: it makes the header report the value the pipeline actually falls back to. It therefore consults no `OrcaSlicerDocumented/` source and files no new canonical-divergence row — the divergence is carried by `D-164` and by 184's `[FWD-1]` residual, and must not be re-derived or duplicated here.
