...
- `docs/adr/0020-layer-stage-commit-as-per-stage-enum.md` — **append an `## Amendment — <date> (packet 181)` section that quotes the retired clause verbatim and states the replacement** (`None` is the empty-commit case only; a missing component is `Err(LayerStageError::FatalModule)`). Follow the `D-161-ADR-0037-AMENDED` precedent exactly: ADR-0037 carries an `## Amendment — 2026-07-15 (packet 161)` section that quotes the clause it retires, and the deviation row points at it. **Do not delete or silently rewrite §Decision item 1 in place** — the amendment is a decision record, and an in-place edit destroys the evidence a future reader needs.
