...
- Amend `docs/adr/0020-layer-stage-commit-as-per-stage-enum.md` by **appending an `## Amendment — <date> (packet 181)` section** that quotes §Decision item 1's retired clause verbatim and states the replacement (`None` is the empty-commit case only; a missing component is `Err(LayerStageError::FatalModule)`). Do **not** delete or rewrite the clause in place — the `D-161-ADR-0037-AMENDED` precedent amended ADR-0037 by adding an `## Amendment — 2026-07-15 (packet 161)` section that quotes the retired clause, and this packet follows it. **Then file `D-181-ADR-0020-AMENDED`** in `docs/DEVIATION_LOG.md` quoting the contested clause and pointing at the new section (convention: `D-161-ADR-0037-AMENDED`, `D-283-ADR-0046-AMENDED`). Verify `bash -c "rg -q '^## Amendment' docs/adr/0020-layer-stage-commit-as-per-stage-enum.md && rg -q 'packet 181' docs/adr/0020-layer-stage-commit-as-per-stage-enum.md && rg -q 'is the empty/' docs/adr/0020-layer-stage-commit-as-per-stage-enum.md && rg -q 'D-181-ADR-0020-AMENDED' docs/DEVIATION_LOG.md && echo PASS || echo FAIL"` — note the clause **must still match**, because the Amendment quotes it; a `! rg -q 'is the empty/'` gate would only pass under the in-place deletion this packet forbids.
...
- Reconcile draft packets `163`/`164`, whose DEV-087 carve-outs go stale once this lands. Re-derive the exact wording at reconciliation time (`rg -n 'MissingComponent|DEV-087' .ralph/specs/163*/ .ralph/specs/164*/`) rather than trusting a quotation frozen here; as authored it is 164's `implementation-plan.md` ("byte-untouched"), 164's `design.md` ("must not be widened or narrowed here" / "do not touch"), and 163's `requirements.md` §Out of Scope. Record the note; do not edit their files.
