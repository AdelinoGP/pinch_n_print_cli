...
- In scope:
  - Define three new types in `slicer-sdk` (or thread through from `slicer-ir`) and re-export at the SDK API boundary:
    - `EntityMutation` — serializable enum with variants for every PrintEntity field a near-future PostPass module plausibly mutates. Step 0 audits packet 40 design.md `## Open Questions` and the four future-module references (`SequentialPrintOrder`, `MinLayerTimeEnforcer`, `FlushVolumeCalculator`, `PrimeTower`) to enumerate concrete variants. Working draft: `SetSpeedFactor(f32)` (path-level, mutates `ExtrusionPath3D.speed_factor`), `SetFlowFactor(f32)` (per-point, multiplies every `Point3WithWidth.flow_factor` in the path — semantically a volumetric multiplier, the correct lever for `FlushVolumeCalculator`). Final list locked at Step 0 close. A path-level `SetExtrusionWidthFactor` was considered and rejected: `ExtrusionPath3D` carries no `extrusion_width_factor` field today (only `points`, `role`, `speed_factor`), and adding one would expand IR shape outside this packet's scope; defer until a real consumer surfaces.
  ...
  - Close `DEV-041` in `docs/DEVIATION_LOG.md` (the live deviation registry; the row currently sits at `docs/DEVIATION_LOG.md:47`) with a closure note dated at packet acceptance. Note: `docs/14_deviation_audit_history.md` is an archive of retired audit artifacts and does NOT carry the live DEV-041 row.
- Out of scope:
...
- **Given** an SDK `FinalizationOutputBuilder` with a fixture layer holding three entities (ids `1, 2, 3`), entity_id `2`'s path has every `Point3WithWidth.flow_factor == 1.0`, **when** a caller invokes `builder.modify_entity(layer, 2, EntityMutation::SetFlowFactor(0.7))` and `apply_to`, **then** post-merge entry with `entity_id == 2` has every `path.points[i].flow_factor == 0.7` AND the other two entries' per-point `flow_factor` values are byte-unchanged. | `cargo test -p slicer-sdk --test finalization_builder_tdd modify_entity_set_flow_factor_applies -- --exact --nocapture`
...
- **Given** the `crates/slicer-macros/src/lib.rs` `run_finalization` drain-back loop, **when** the file is grepped for an explicit iteration over `merge_ops` (regex `for\s+\w+\s+in\s+[^{]*merge_ops` OR `merge_ops\(\)\s*\.\s*iter\(\)` OR `\.merge_ops\(\)\s*\.\s*into_iter\(\)`), **then** at least one matching iteration site exists AND no comment containing the literal substring `silently no-op` or `DEV-041` or `merge_ops` survives (the gap is closed in code, not just hidden behind a TODO; pre-implementation the file contains exactly two TODO-comment matches at `lib.rs:1212` and `:1214` which MUST be removed). The grep MUST NOT be satisfied by a struct-field reference such as `merge_ops: Vec::new()` or `merge_ops,` in a struct literal — only a real iteration shape passes. | `cargo test -p slicer-host --test finalization_mutation_roundtrip_tdd drain_back_forwards_merge_ops -- --exact --nocapture`
...
- `docs/DEVIATION_LOG.md` — `DEV-041` entry to be closed by this packet (currently at line 47). Direct read; narrow (single row). The legacy `docs/14_deviation_audit_history.md` is an archive only and does NOT carry the live DEV-041 row.
