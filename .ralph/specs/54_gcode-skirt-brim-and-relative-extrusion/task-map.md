...
- **TASK-183** (new — Track B) — "Relative-extrusion mode toggle (M82/M83) via `use_relative_e_distances`".
...
| `TASK-142a` | Step 2A — Track A fix + tests | `docs/05_module_sdk.md` (Finalization Stage, ≤ 40 lines) | ONE of {`modules/core-modules/skirt-brim/src/lib.rs`, `crates/slicer-host/src/dispatch.rs:2840-:2900`, `crates/slicer-host/src/config_schema.rs`, `modules/core-modules/skirt-brim/skirt-brim.toml`} + `crates/slicer-host/tests/gcode_skirt_brim_emission_tdd.rs` (new) + cascading mechanical updates to `modules/core-modules/skirt-brim/tests/skirt_brim_tdd.rs` and `modules/core-modules/skirt-brim/tests/finalization_live_tdd.rs` when the diagnosed fix changes `lib.rs` defaults | Brim/Print SUMMARY from Step 1 | M | Skipped if Step 1 returned ESCALATE. All Track A ACs + negative test green. |
| `TASK-183` | Step 2B — Track B schema + first test | none direct | `crates/slicer-host/src/config_schema.rs`, `crates/slicer-host/tests/gcode_relative_extrusion_tdd.rs` (new) | none | S | `config_schema_registers_bool_default_true` green; rest red. |
| `TASK-183` | Step 3B — Serializer branch | `docs/02_ir_schemas.md` (SUMMARY) | `crates/slicer-host/src/gcode_emit.rs` (`:200-:480` range), `crates/slicer-host/tests/gcode_relative_extrusion_tdd.rs` | `OrcaSlicerDocumented/src/libslic3r/GCodeWriter.cpp` (FACT ≤ 8 lines on M82/M83) | M | 5 of 6 ACs + 4 of 4 negative tests green; pipeline-threading test still red. |
| `TASK-183` | Step 4B — Pipeline threading | none | `crates/slicer-host/src/main.rs` (`PipelineConfig` construction site, ~`:230-:280`); `pipeline.rs` untouched | none | S | All Track B tests green. |
| `TASK-142a`, `TASK-183` | Step 5 — Docs hygiene | `docs/07_implementation_status.md`, `docs/DEVIATION_LOG.md`, `docs/14_deviation_audit_history.md` | docs only | none | S | TASK-142a + TASK-183 rows; DEV-009 progress entries for both subsets. |
