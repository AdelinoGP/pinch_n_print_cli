...
| Packet 64 (`64_paint-native-migration`) | This packet depends on | Migrated `paint-segmentation` to host-native `PrePass::PaintSegmentation` fallback; added `union_paint_regions_at_harvest: bool` parameter to `execute_paint_segmentation`. This packet's `support_*` piggyback was implemented alongside Packet 64's migration and runs in the host-native path. |
...
- The synthetic-fixture builder in `threemf_subtypes_synthetic_e2e_tdd.rs` uses IR struct builders (`box_mesh`, `modifier_volume_with_subtype`, `mesh_ir_with_modifier`) rather than 3MF archive parsing. This tests consumer logic directly; full 3MF parse → IR → consumers coverage lives in Packet 56's `threemf_sidecar_classification_tdd.rs` and Packet 56b's `benchy_4color_modifier_part_e2e_tdd.rs`.
