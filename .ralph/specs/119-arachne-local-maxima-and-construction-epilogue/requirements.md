# Requirements: 119-arachne-local-maxima-and-construction-epilogue
...
- **Deviation-log entry**: `D-119-LOCAL-MAXIMA-EPILOGUE` (new ID, addendum on
...
- Negative cases: `AC-N1` (N1 red tests stay green — epilogue doesn't regress
  A1).
- Cross-packet impact: unblocks `120` (E — E's `removeSmallLines` interacts
...
| `rg -q 'D-119-LOCAL-MAXIMA-EPILOGUE' docs/DEVIATION_LOG.md` | Deviation log entry present | FACT pass/fail |
...
- **Deviation-log correction uses the supersession pattern** — new
  `D-119-LOCAL-MAXIMA-EPILOGUE` + addendum on `D-113C-FAITHFUL-GRAPH-CONSTRUCTION`.
