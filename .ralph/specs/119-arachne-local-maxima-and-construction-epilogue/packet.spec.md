---
status: draft
packet: 119-arachne-local-maxima-and-construction-epilogue
task_ids:
  - none
...
# Packet Contract: 119-arachne-local-maxima-and-construction-epilogue
...
- Depends on: `118-arachne-angle-fudge-and-noncentral-regions` (C — D's `generateLocalMaximaSingleBeads` reads the normalized centrality; C's `filterNoncentralRegions` must land first so local-maxima detection runs on the corrected central set).
- Unblocks: `120-arachne-postprocess-order-and-remove-small-simplify` (E — E's `removeSmallLines` interacts with the micro-loops D emits).
...
- `docs/DEVIATION_LOG.md` — new entry `D-119-LOCAL-MAXIMA-EPILOGUE` documenting the N9+N10 fix, with an addendum on `D-113C-FAITHFUL-GRAPH-CONSTRUCTION` noting D extends 113c's `from_polygons` with the canonical epilogue. Supersession pattern.
  - `rg -q 'D-119-LOCAL-MAXIMA-EPILOGUE' docs/DEVIATION_LOG.md`
