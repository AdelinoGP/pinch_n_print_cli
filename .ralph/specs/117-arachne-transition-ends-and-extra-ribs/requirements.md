# Requirements: 117-arachne-transition-ends-and-extra-ribs
...
- **Deviation-log entry**: `D-117-TRANSITION-ENDS` (new ID, addendum on
  `D-112-PROPAGATION-ADAPT`, supersession pattern).

## Out of Scope

- **N1/N7 (junction geometry + BeadingPropagation)** — A1 (`116a`). B reads
  A1's `get_beding` for emission interpolation but does not change it.
- **N2/N4 (`perimeter_index` + `is_odd`)** — A2 (`116b`). B reads A2's junction
  fans for beading interpolation but does not change them.
- **N5 (π hack) and N6 (`filterNoncentralRegions`)** — Packet C (`118`).
...
- `docs/DEVIATION_LOG.md` `D-112-PROPAGATION-ADAPT` + `D-116A-JUNCTION-BANDS` +
  `D-116B-CONNECTJUNCTIONS-EMISSION` entries — substrate.
...
- Cross-packet impact: unblocks `118` (C — `filterNoncentralRegions` interacts
...
| `rg -q 'D-117-TRANSITION-ENDS' docs/DEVIATION_LOG.md` | Deviation log entry present | FACT pass/fail |
...
- **Deviation-log correction uses the supersession pattern** — new
  `D-117-TRANSITION-ENDS` + addendum on `D-112-PROPAGATION-ADAPT`.
