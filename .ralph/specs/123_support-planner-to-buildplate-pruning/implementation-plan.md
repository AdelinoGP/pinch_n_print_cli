...
- One atomic step at a time. Maps to `TASK-288` (renumbered from source-plan `TASK-264`).
- TDD: AC-2 through AC-4, AC-N1, AC-N2 unit tests authored RED before implementation.
...
### Step 2: Author AC-2 through AC-4, AC-N1, AC-N2 as RED

- Files allowed to edit (≤ 3): `modules/core-modules/support-planner/tests/to_buildplate_tdd.rs` (new).
- Sub-agent dispatches:
  - "Run `cargo test -p support-planner --test to_buildplate_tdd`; return FACT (expected: all five fail)."
- Context cost: `S`
...
- Sub-agent dispatches:
  ...
  - "Run `rg -B1 -A4 'support_on_build_plate_only' modules/core-modules/support-planner/support-planner.toml`; return FACT pass/fail." — purpose: AC-5 manifest.
- Context cost: `M`
- Verification: AC-1, AC-2, AC-3, AC-4, AC-5, AC-N1, AC-N2 PASS.
...
- Sub-agent dispatches:
  ...
  - "Run `rg -q 'build_plate_only_emits_no_to_model_branches' docs/specs/support-modules-orca-port.md`; return FACT pass/fail."
  - "Run `cargo xtask build-guests --check`; return FACT."
- Context cost: `M`
- Verification: AC-6, AC-7, AC-8, AC-9 PASS; doc updated.
- Exit condition: invariant active; goldens aligned.
...
- All ACs PASS; `cargo xtask build-guests --check` clean; `docs/07` marks `TASK-288` `[x]`; docs/specs invariant list extended.

## Acceptance Ceremony

- Re-dispatch every AC command; confirm gate commands green; mark `TASK-288` `[x]`; transition.
