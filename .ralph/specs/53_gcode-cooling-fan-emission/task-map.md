...
| `TASK-154` | Step 2 — Config keys + first tests | none direct | `crates/slicer-host/src/config_schema.rs`, `crates/slicer-host/tests/gcode_part_cooling_emission_tdd.rs` (new) | none | S | Two named tests green. |
| `TASK-154` | Step 3 — Scaffold cooling module | `docs/03_wit_and_manifest.md` (manifest schema) | `modules/core-modules/part-cooling/{Cargo.toml, part-cooling.toml, src/lib.rs}` (new), `crates/slicer-host/src/dispatch.rs` (range `:2840-:2900`), `modules/core-modules/build-core-modules.sh` | none | M | Build script produces `part-cooling.wasm`. Dispatcher loads the module. Split into 3a/3b if it trends to L. |
| `TASK-154` | Step 4 — Algorithm + remaining tests | `docs/02_ir_schemas.md` (SUMMARY) | `modules/core-modules/part-cooling/src/lib.rs`, `crates/slicer-host/tests/gcode_part_cooling_emission_tdd.rs` | CoolingBuffer SUMMARY from Step 1 | M | All 6 ACs + 3 negative cases green. |
