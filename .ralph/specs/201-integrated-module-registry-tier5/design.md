...
- Primary code path: `load_modules_from_roots` → `discover_manifest_paths` → `ingest_manifest` → `LoadedModuleBuilder::build` (`crates/slicer-scheduler/src/manifest.rs`); consumed by `load_live_modules_for_plan_profiled` (`crates/slicer-wasm-host/src/execution_plan_live.rs`) and by `run_slice` (`crates/slicer-runtime/src/run.rs`, two loader call sites: the main slice path and the `load_live_modules_for_plan_with_config` helper near the bottom of the file).
...
- ADR-0056 §1 "One model": the integrated tier enters at the *loader*, not the root assembler — `assemble_search_roots` keeps returning `Vec<PathBuf>` untouched; tier 5 is a post-roots ingestion phase inside `load_modules_from_roots_with_integrated`. Claims, DAG, and config machinery never branch on provenance.
- ADR-0056 §2 "Lowest search priority": integrated entries are processed **after** every disk root through the same `seen_ids` set, so first-root-wins dedup by `module.id` is literally the same code path.
...
- Selected approach — **provenance field + text-source ingestion + post-roots tier, registry in a dedicated crate**:
  1. `manifest.rs`: add `pub enum ModuleProvenance { External, Integrated }` (Copy, Eq); `LoadedModule` gains `pub(crate) provenance: ModuleProvenance`; `LoadedModuleBuilder` gains a `.provenance(...)` setter defaulting to `External` (all construction goes through the builder — fields are `pub(crate)`, so the struct-literal blast radius is the single literal inside `LoadedModuleBuilder::build`).
...
- `crates/slicer-wasm-host/src/execution_plan_live.rs` — lines 30–340 only — purpose: `LiveModuleBinding` shape, entry-point stack, compile loop.
...
- The shadow-diagnostic wording `external module 'X' shadows integrated module 'X'` is this packet's canonical contract string (aligned with ADR-0056's consequence bullet, which describes the diagnostic in prose without the quoting) and becomes a contract consumed by 203's diagnostics surfacing; changing it later requires touching 203's tests.
...
- [FWD] Feature naming for the registry crate: module directory name (`classic-perimeters`) is chosen here; if 205 prefers prefixed names (`integrated-classic-perimeters`) the rename is mechanical and confined to the registry crate + 205's edition lists. Implementer may keep the chosen names.
...
- [FWD] Whether the `run.rs` helper near line 1033 (`load_live_modules_for_plan_with_config` call with `module_dirs`) serves a non-slice path (report/dry-run); the step-5 worker must confirm both call sites' roles before swapping and leave any test-only call sites on the old entry points.
