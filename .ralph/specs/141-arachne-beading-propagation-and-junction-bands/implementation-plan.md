# Implementation Plan: 141-arachne-beading-propagation-and-junction-bands
...
- Objective: Rewrite `generate_junctions` (symbol-search for it; the `:192-334` range is pre-`9367d239`, do not trust literally) to the canonical scheme — iterate ALL edges (no centrality gate, ribs included), skip non-upward half-edges (`from.R > to.R`), skip flat/same-bead-count edges, compute ONE beading **at the peak node ALWAYS** via `get_beding` (never `edge.from` as a primary path), emit ONLY in-band beads (middle-index start, break on `bead_R < end_R`), no clamping, near-`start_R` snap, **width read directly from the resolved beading's own `bead_widths[idx]`** (never a fresh per-bead `strategy.compute()` call). **Before starting this step, read `packet.spec.md`'s "Known Implementation Hazard" section — a first attempt at this exact objective got the peak-anchor, width-source, and rib-inclusion parts backwards/missing despite this document already stating them correctly, and only discovered it via a fresh ground-truth re-read, not from this plan.** Add the AC-N1 structural test AND the AC-3 bug-regression test (`arachne_generate_junctions_canonical_regression.rs`, 3 tests). Re-baseline the affected `slicer-core` fixtures EXCEPT `toolpaths_tapered_wedge.json` (leave that one failing — see below). Add the `D-141-JUNCTION-BANDS` deviation-log entry + `D-113C` addendum.
- Precondition: Step 1 is green (the `BeadingPropagation` side table + `get_beding` exist and the structural test passes).
- Postcondition: AC-3 (the 3 isolated bug-regression tests) pass **without weakened assertions** — this is the real, achievable-now oracle for this step's own correctness. AC-N1 (upward-half-edge-only) passes. N2, N3, N4 red tests stay RED (gated by the "stays red" commands). **AC-1 and AC-2 (the N1 red tests, run through the full pipeline) are EXPECTED TO STILL FAIL after this step — this is correct, not a regression to chase.** They are blocked on Packet 142's chain-walk 3-way-junction fix (that packet's AC-4), discovered when this exact step was first (incorrectly) implemented and its bugs were later found and fixed. Do not attempt to make AC-1/AC-2 pass by re-introducing a centrality/type gate into `generate_junctions` or the domain seeding — that is the specific mistake `packet.spec.md`'s hazard section documents. `toolpaths_tapered_wedge.json` is deliberately left un-re-recorded (its drift is downstream of Packet 142's still-open gap); the other affected fixtures ARE re-baselined. `D-141-JUNCTION-BANDS` present in `docs/DEVIATION_LOG.md`, including its 2026-07-06 correction paragraph.
- Files allowed to read (with line-range hints when > 300 lines):
...
- Files allowed to edit (≤ 4):
  ...
  - `crates/slicer-core/src/arachne/pipeline.rs` (wire `populate_beading_propagation` into `run_arachne_pipeline` — N7's side table has zero production call sites otherwise)
  - `crates/slicer-core/tests/arachne_junction_upward_half_edge_only.rs` (NEW — AC-N1 structural test) and `crates/slicer-core/tests/arachne_generate_junctions_canonical_regression.rs` (NEW — AC-3, 3 bug-regression locks)
  - `docs/DEVIATION_LOG.md` (addendum only — new `D-141-JUNCTION-BANDS` entry + one-line addendum on `D-113C-FAITHFUL-GRAPH-CONSTRUCTION`; no in-place edits to 113c's narrative)
- Files explicitly out-of-bounds for this step:
  - `crates/slicer-core/src/arachne/pipeline.rs:334` and `:272-277` (Packet C's π hack / fudge)
...
- Expected sub-agent dispatches:
  ...
  - "Run `cargo test -p slicer-core --features host-algos --test arachne_generate_junctions_canonical_regression --no-fail-fast`; return FACT (pass, all 3) or SNIPPETS (fail)" — purpose: validate AC-3, the real achievable-now oracle for this step.
  - "Run `cargo test -p slicer-core --features host-algos --test arachne_parity_red_junction_bands -- n1_rectangle_outer_wall_junctions_stay_near_boundary --nocapture`; return FACT (fail is EXPECTED here — blocked on Packet 142's AC-4, not a regression) or SNIPPETS" — purpose: record AC-1's current (expected-red) state for the commit message, not to chase green.
  - "Run `cargo test -p slicer-core --features host-algos --test arachne_parity_red_junction_bands -- n1_square_outer_wall_junctions_at_outer_bead_radius --nocapture`; return FACT (fail is EXPECTED, same reason)" — purpose: record AC-2's current state.
...
- Verification:
  ...
  - `rg -q 'D-141-JUNCTION-BANDS' docs/DEVIATION_LOG.md` — FACT pass.
- Exit condition: AC-1, AC-2, AC-N1 pass; N2/N3/N4 stay red; centrality/bead_count/propagation/generate_toolpaths regression green (fixtures re-baselined); `D-141-JUNCTION-BANDS` present in deviation log; `cargo check -p slicer-core --all-targets` and `cargo clippy -p slicer-core --all-targets -- -D warnings` pass; e2e closure delta recorded in commit message (record-only).
...
- `D-141-JUNCTION-BANDS` present in `docs/DEVIATION_LOG.md` with addendum on `D-113C-FAITHFUL-GRAPH-CONSTRUCTION`.
