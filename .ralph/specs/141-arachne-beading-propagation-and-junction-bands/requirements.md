# Requirements: 141-arachne-beading-propagation-and-junction-bands
...
- **Deviation-log entry**: `D-141-JUNCTION-BANDS` (new ID, addendum on
...
- **N2 (`perimeter_index = bead_idx`) and N4 (`is_odd` semantics)** — owned by
  Packet A2 (`142-arachne-canonical-connectjunctions-emission`). A1 leaves
  both red.
...
- Cross-packet impact: unblocks `142` (A2 — needs A1's junction fans),
  `143` (B — needs A1's fans for beading interpolation),
  `144` (C — the π hack A1 leaves in place is load-bearing until C removes it
  strictly after A2).
- Refinements not captured in Given/When/Then:
  ...
  - A1 does NOT remove the π hack — it is load-bearing for the centrality-gated
    scheme until A1's rewrite lands. Packet C (`144`) removes it strictly after
    A2.
...
| `rg -q 'D-141-JUNCTION-BANDS' docs/DEVIATION_LOG.md` | Deviation log entry present | FACT pass/fail |
...
- **Deviation-log correction uses the supersession pattern** — a new
  `D-141-JUNCTION-BANDS` entry plus a one-line addendum on
