...
- **FORWARD-DEP**: `TASK-281` (`178-seam-region-aware-planning`) and `TASK-282` (`179-seam-canonical-algorithm-fidelity`) are both `status: draft` and must be implemented before this packet's continuous projection and degraded fallback can be exercised end-to-end. This packet's Step 1 can be implemented and unit-tested in isolation (with synthetic `PerimeterRegionView` fixtures mirroring packet 1's input), but AC-4's e2e test requires the full pipeline to work.
...
- Files explicitly out of bounds:
  ...
  - Packet 1 WIT/IR files
  - Packet 2 scoring/visibility/spline files
