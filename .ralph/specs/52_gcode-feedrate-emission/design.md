...
- Primary code path:
  ...
  - `crates/slicer-host/src/config_schema.rs:104-176` — `ConfigValue` enum + validation API. Eight new keys added.
- Neighboring tests or fixtures:
  - `crates/slicer-host/tests/orca_comment_contract_tdd.rs` — regression; this packet must not break the `;TYPE:` contract.
...
- Selected approach: **builder-side resolution.** Add `fn resolve_feedrate(role: &ExtrusionRole, speed_factor: f32, config: &ResolvedConfig) -> Option<f32>` inside `gcode_emit.rs`. Call it from the print-move builder and the two z-hop builders. The travel-move builder continues to use the upstream `tm.f` when set; when `tm.f` is `None`, fall back to `resolve_feedrate(&ExtrusionRole::Custom("Travel"), 1.0, config)`.
- Exact functions, traits, manifests, tests, or fixtures expected to change:
  - `crates/slicer-host/src/gcode_emit.rs`:
    - Add `fn resolve_feedrate(role, speed_factor, config) -> Option<f32>` (private).
    ...
    - Extend the builder's parameter list to receive a `&ResolvedConfig` (or whatever the existing config handle is — discovery step will confirm).
  - `crates/slicer-host/src/config_schema.rs`:
    - Register eight `ConfigField`s: `outer_wall_speed`, `inner_wall_speed`, `sparse_infill_speed`, `internal_solid_infill_speed`, `top_surface_speed`, `travel_speed`, `initial_layer_speed`, `initial_layer_travel_speed`. Each `ConfigValue::Float`. Defaults from delegated OrcaSlicer FACT lookup; record verbatim in this file once obtained.
  - `crates/slicer-host/tests/gcode_feedrate_emission_tdd.rs` — NEW; ≥ 8 tests (one per AC + each negative case).
...
- `crates/slicer-host/src/config_schema.rs` — primary edit; load directly (small, < 300 lines per reconnaissance); expected change: register eight speed keys with defaults.
...
- "Run `cargo test -p slicer-host --test orca_comment_contract_tdd`; return FACT pass/fail." — purpose: regression check after the F-token wiring lands.
...
- Unit conversion contract: `f_token_value = round(speed_mm_per_s * 60.0 * speed_factor)` as an integer. `speed_factor` clamped to `[0.05, 5.0]` to avoid pathological values (clamp threshold confirmed against OrcaSlicer in Step 1's FACT dispatch).

## Locked Assumptions and Invariants

- `ExtrusionRole` enum variants in slicer-ir map onto the eight registered config keys via an explicit `match` (no string lookup). The match arms are defined inside `gcode_emit.rs::resolve_feedrate` and listed in the implementation plan Step 2.
...
- Risk: the eight config keys may already exist under different names elsewhere in the workspace (e.g. inside a module manifest). Mitigated by Step 0's verification dispatch.
...
- None at draft time. The discovery Step 1 will confirm the existing config-view parameter shape; if the builder turns out NOT to accept a `&ResolvedConfig` today, the resolution helper will receive a borrowed map and the threading change is contained to one function signature.
