...
- In scope:
  ...
  - module computes the permutation from its `regions: &[PerimeterRegionView]` input plus a fresh start position of `(0.0, 0.0)`
...
- Depends on:
  - packet 32 (`32_layer-collection-builder-wit-surface`) — provides the `layer-collection-builder` resource, host validation, dispatch wiring, SDK builder, and macro plumbing this packet relies on
...
- `docs/02_ir_schemas.md` — `LayerCollectionIR.ordered_entities`, `PrintEntity.topo_order`, `PerimeterRegionView`
- `docs/04_host_scheduler.md` — PathOptimization stage scheduling (unchanged structurally; only the source of ordering moves)
- `docs/05_module_sdk.md` — SDK builder usage (`LayerCollectionBuilder::set_entity_order`)
