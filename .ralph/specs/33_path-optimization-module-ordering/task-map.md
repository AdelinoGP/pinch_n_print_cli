...
| --- | --- | --- | --- | --- | --- |
| `TASK-152h` (new) | Step 1 | `docs/05_module_sdk.md` | `modules/core-modules/path-optimization-default/src/lib.rs` | `OrcaSlicerDocumented/src/libslic3r/ShortestPath.cpp` | Port packet-18 NN algorithm into the module; call `collection.set_entity_order(items)`. Reversal flag stays `false`. |
...
| `TASK-152h` | Step 5 | `docs/01_system_architecture.md` | `crates/slicer-host/src/layer_executor.rs`, `crates/slicer-host/src/lib.rs` | — | Delete `order_entities_by_nearest_neighbor` and remove its re-export. Both call sites in `execute_single_layer` use raw `assemble_ordered_entities` directly. |
...
- Packet 32 (`32_layer-collection-builder-wit-surface`) — must be `implemented` before this packet activates. This packet consumes packet 32's WIT resource, host validation helper, SDK builder, and macro plumbing.
