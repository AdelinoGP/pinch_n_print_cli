...
(`crates/slicer-ir/src/slice_ir.rs:1176-1185`), but no guest module can reach it: the dispatch
...
read an *arbitrary* region's config today. Every downstream infill packet (132–136) assumes
...
- Host: `RegionKey`-matched per-region `ResolvedConfig` resolution from the `RegionMapIR`
  pool at dispatch; retire the first-match derivation (`dispatch.rs:1629-1645` region).
- WIT: config accessor on `slice-region-view` AND `perimeter-region-view`
  (`crates/slicer-schema/wit/deps/ir-types.wit`); affected world version bump (world-layer
  1.1.0 → 1.2.0; mirror on any other world exposing these views). Additive only.
...
- Positive cases: `AC-1`–`AC-4` in `packet.spec.md`. Refinements: AC-1's two densities are
  exactly 0.15/0.40 to make misrouting unmistakable; AC-2 is a structural grep (the retired
  derivation is *absent*), paired with AC-1's behavioral proof; AC-4's carve-list entries must
  be per-test (no "various e2e tests" hand-waving).
- Negative cases: `AC-N1` (single-region equivalence), `AC-N2` (wedge byte-identical — the
  strongest no-regression signal available since the wedge is single-region throughout).
...
| `rg -n 'global_layer_index == layer' crates/slicer-wasm-host/src/dispatch.rs \| wc -l` | AC-2 structural grep (expect 0) | FACT count |
| `cargo test -p slicer-runtime --test e2e -- wedge 2>&1 \| tee target/test-output.log \| grep "^test result"` | AC-N2 wedge SHA guard | FACT |
...
- Likely temptation reads: `crates/slicer-ir/src/resolved_config.rs` in full — skip; only the
  `RegionMapIR` pool types matter, and they live in `slice_ir.rs:1176-1185`.
