...
- **AC-5. Given** the same feature build and no search roots, **when** `pnp_cli module config-schema --no-default-module-paths` runs, **then** stdout JSON contains a schema entry whose name is `classic-perimeters` (`build_config_schema_json` output, `crates/slicer-scheduler/src/manifest.rs`); **and when** re-run with `--no-integrated-modules`, stdout contains no `classic-perimeters`. | `cargo test -p pnp-cli --features integrated-classic-perimeters --test integrated_provenance_tdd config_schema_includes_integrated_module`
...
- `docs/adr/0056-integrated-modules-native-dispatch.md` — short; read directly; §2 (tier/dedup) and the provenance-aware-diagnostic consequence are this packet's contract.
...
- `.ralph/specs/201-integrated-module-registry-tier5/packet.spec.md` and `.ralph/specs/202-native-adapter-and-dispatch/packet.spec.md` — direct read (FORWARD-DEP contracts); their other files only via SUMMARY dispatch.
