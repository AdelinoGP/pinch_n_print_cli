...
- Blast radius (Step 1 only; re-derive list at implementation): `crates/slicer-runtime/tests/e2e/mm_real_fixture_gcode_tdd.rs`, `tests/e2e/run_slice_api_tdd.rs`, `tests/executor/cube_4color_arachne.rs`, `tests/executor/cube_4color_gcode_output_tdd.rs` (3 sites), `tests/executor/cube_4color_ironing_per_painted_top_color_tdd.rs`, `tests/executor/cube_4color_phase5_tdd.rs`, `tests/executor/cube_4color_sparse_infill_per_painted_region_tdd.rs`, `tests/unit/cancel_flag_tdd.rs`, `tests/unit/profile_flag_tdd.rs`, `tests/visual_debug_agent_overhead_tdd.rs` — 12 construction sites across these 10 files (13 including the one in `crates/pnp-cli/src/main.rs`), each gaining one `no_integrated_modules: false` line.
...
- `.ralph/specs/201-*/packet.spec.md`, `.ralph/specs/202-*/packet.spec.md` - FORWARD-DEP contracts.

## Out-of-Bounds Files

- `.ralph/specs/200-*`, `.ralph/specs/201-*` (beyond packet.spec.md), `.ralph/specs/202-*` (beyond packet.spec.md), `.ralph/specs/194-*`…`199-*` — never modify; inspect other packet files only via SUMMARY dispatch.
...
- The `integrated-classic-perimeters` feature couples this packet's tests to 201's registry feature name, and the two strings may collide: 201 chose the bare `classic-perimeters` but carries an open `[FWD]` that 205 may prefer prefixed names (`integrated-classic-perimeters`) — the same string this packet uses for its `pnp-cli` passthrough. A same-named passthrough is legal cargo and needs no AC rename; only a *third* name forces `packet.spec.md`'s AC commands to be revisited. The binding reconciliation rule lives in `implementation-plan.md` Step 3 (FORWARD-DEP reconciliation, not a design change).
