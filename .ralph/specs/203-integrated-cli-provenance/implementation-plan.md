...
- Files explicitly out of bounds:
  ...
  - `.ralph/specs/194-*` … `.ralph/specs/202-*`, `.ralph/specs/204-*`, `docs/07_implementation_status.md`, `docs/adr/*`, `docs/specs/multi-edition-distribution-plan.md`, `CONTEXT.md` — never modify.
...
- Authoritative docs:
  ...
  - `.ralph/specs/201-integrated-module-registry-tier5/design.md` §Code Change Surface item 5 and `.ralph/specs/201-integrated-module-registry-tier5/packet.spec.md` AC-N2 - the "add an entry point, never a parameter" rule and the `&[]`-is-identity contract. **Read-only** (packet 201 is out of bounds for edits); prefer a `SUMMARY` dispatch over a full read. This rule is 201's, not an ADR's
- OrcaSlicer refs:
...
- Files explicitly out of bounds:
  ...
  - `.ralph/specs/194-*` … `.ralph/specs/202-*`, `.ralph/specs/204-*`, `docs/07_implementation_status.md`, `docs/adr/*`, `CONTEXT.md` — never modify.
...
- Authoritative docs:
  ...
  - `docs/adr/0056-integrated-modules-native-dispatch.md` §2 - tier/dedup and the provenance-aware-diagnostic consequence
- OrcaSlicer refs:
  - none.
- Verification:
...
- **Feature-name reconciliation (mandatory, do this before writing the passthrough):** read the `[features]` table of the landed `crates/slicer-integrated-modules/Cargo.toml` and use the **actual** feature name. Two names are in play and they collide by design:
  - `201/design.md` §Code Change Surface item 4 chose the bare module-directory name `classic-perimeters` for the registry crate's own feature;
  - `201/design.md` §Open Questions carries an unresolved `[FWD]`: "if 205 prefers prefixed names (`integrated-classic-perimeters`) the rename is mechanical and confined to the registry crate + 205's edition lists" — i.e. the registry crate may have landed with the *same* string this packet uses for its `pnp-cli` passthrough.
  - Decision rule: this packet's `pnp-cli` feature is named `integrated-classic-perimeters` (locked — `packet.spec.md` AC commands spell it). Its value must be `["slicer-integrated-modules/<landed-registry-feature-name>"]`. If the registry landed on `classic-perimeters`, write `integrated-classic-perimeters = ["slicer-integrated-modules/classic-perimeters"]`. **If the registry landed on `integrated-classic-perimeters`, the passthrough becomes `integrated-classic-perimeters = ["slicer-integrated-modules/integrated-classic-perimeters"]`** — a same-named passthrough is legal cargo and requires no AC rename. Only if the registry landed on some *third* name must `packet.spec.md`'s AC commands be revisited; record whichever case held in the Step 3 commit message.
...
- Files allowed to read, with ranges when over 300 lines:
  ...
  - `.ralph/specs/203-integrated-cli-provenance/packet.spec.md` §Acceptance Criteria — the verbatim asserted strings
- Files allowed to edit (at most 3):
...
- Files explicitly out of bounds:
  ...
  - `.ralph/specs/194-*` … `.ralph/specs/202-*`, `.ralph/specs/204-*` — never modify.
