...
- Guest WASM is likely to be reported STALE by the closure gate's `--check` because every Group-A/B module's `Cargo.toml` gains a dev-dep entry, which can affect build-script input hashing even if the production code is untouched. Rebuild via `cargo xtask build-guests` (without `--check`) is mandatory before the AC-11 workspace test sweep.
