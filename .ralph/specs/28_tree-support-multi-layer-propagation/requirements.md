...
- New core-module crate `modules/core-modules/support-planner/` holding claim `support-planner` on `PrePass::SupportGeneration`, implementing simplified OrcaSlicer-style propagation (no avoidance/collision/radius-tapering). Manifest reads `MeshIR`, `SurfaceClassificationIR`, `PaintRegionIR`; writes `SupportPlanIR`. `LayerPlanIR` is a host-side scheduling prerequisite (via `ensure_stage_prerequisites` for `PrePass::SupportGeneration`) but is not read at runtime — v1 is layer-height-agnostic (uniform layer height) and walks the mesh at `MeshObjectView` granularity. v1 is also single-region: every entry is emitted under the canonical `region_id = 0` bucket because `MeshObjectView` does not currently surface per-region segmentation. Both limitations are deferred to a follow-up packet that extends the prepass WIT.
