...
- New WIT prepass stage: `export run-support-geometry` with a `support-geometry-output` resource carrying `push-support-plan`, plus a `support-plan-entry` record.
...
- Host wiring: `PrepassStageOutput::SupportPlan`, a new `BlackboardPrepassSlot::SupportPlan`, a new `commit_support_plan` on `Blackboard`, and an `ensure_stage_prerequisites` entry for `PrePass::SupportGeometry`.
- New core-module crate `modules/core-modules/support-planner/` holding claim `support-planner` on `PrePass::SupportGeometry`, implementing simplified OrcaSlicer-style propagation (no avoidance/collision/radius-tapering). Manifest reads `MeshIR`, `SurfaceClassificationIR`, `PaintRegionIR`; writes `SupportPlanIR`. `LayerPlanIR` is a host-side scheduling prerequisite (via `ensure_stage_prerequisites` for `PrePass::SupportGeometry`) but is not read at runtime — v1 is layer-height-agnostic (uniform layer height) and walks the mesh at `MeshObjectView` granularity. v1 is also single-region: every entry is emitted under the canonical `region_id = 0` bucket because `MeshObjectView` does not currently surface per-region segmentation. Both limitations are deferred to a follow-up packet that extends the prepass WIT.
...
1. `wit/world-prepass.wit` declares `PrePass::SupportGeometry` via `run-support-geometry` + `support-geometry-output` + `support-plan-entry`.
...
3. `slicer-host` schedules `PrePass::SupportGeometry` with prerequisite slots `SurfaceClassification` and `LayerPlan`, commits its output to a new `BlackboardPrepassSlot::SupportPlan`, and audits the read/write masks in the same shape as `PrePass::SeamPlanning`.
