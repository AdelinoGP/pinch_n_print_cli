...
7. **`crates/slicer-host/src/wit_host.rs` / prepass runtime dispatcher** — extend `WasmRuntimeDispatcher` (or wherever prepass dispatch lives; see `execute_prepass`'s `runner.run_stage`) to implement the `run-support-generation` export: feed `list<MeshObjectView>` + the output resource, collect pushed entries into `SupportPlanIR`, return as `PrepassStageOutput::SupportPlan`.
8. **`modules/core-modules/support-planner/`** — new crate. Layout mirrors `seam-planner-default/`:
   ...
   - `src/lib.rs` (`#[slicer_module] impl PrepassModule for SupportPlanner { fn run_support_generation(...) }`)
...
- **PrePass is sequential, per-stage; per-layer tier is rayon-parallel.** `PrePass::SupportGeneration` runs once, globally, before any per-layer work. `Layer::Support` runs per-layer in rayon. Propagation across layers must happen in the PrePass stage — a per-layer reader can never propagate because it has no cross-layer view guaranteed-consistent under parallelism.
...
- **The SDK trait for prepass modules must support the new stage.** Inspect `PrepassModule` in `crates/slicer-sdk/src/traits.rs`; add `fn run_support_generation(...)` with a default-unimplemented body so existing prepass modules continue to compile.
...
### Files modified
- `wit/world-prepass.wit` — add `run-support-generation` export + `support-generation-output` resource + `support-plan-entry` record.
...
- `crates/slicer-host/src/wit_host.rs` — prepass dispatcher implements the new `run-support-generation` call path (mirrors `run-seam-planning`).
- `crates/slicer-sdk/src/traits.rs` — add `fn run_support_generation(...)` to `PrepassModule` (default `Err(ModuleError::unimplemented(...))`).
...
- **Q3: Claim layout.** *Resolved —* new claim `support-planner` on `PrePass::SupportGeneration`. `support-generator` on `Layer::Support` is unchanged.
...
5. The `#[slicer_module]` macro already handles `PrepassModule` with stage routing to the correct WIT export; adding `run_support_generation` to the trait and macro keyed-stage map is mechanical.
