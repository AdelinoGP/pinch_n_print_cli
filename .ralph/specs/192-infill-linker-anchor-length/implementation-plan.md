...
- Task IDs: `TASK-311`
- Objective: regenerate `docs/15`, close and correct the `DEV-089` row, file the two residual deviation rows with freshly derived IDs, rewrite ADR-0025's stale section, flip the two `ORCA_CONFIG_REFERENCE` rows, correct the `resolved_config.rs` doc comment, register `TASK-311`, and update the plan's queue row.
