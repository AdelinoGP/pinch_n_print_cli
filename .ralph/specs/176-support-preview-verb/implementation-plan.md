...
- Task IDs: `TASK-280`
- Objective: determine which fixture (`resources/bridge_support_enforcers.3mf` or `resources/bridge.obj` + explicit config) plus which config keys (`support_enabled`, support type, etc.) cause prepass to commit a non-empty `SupportGeometryIR`; record the pair as the AC-1/AC-2 test input.
...
- Expected sub-agent dispatches:
  - Question: full test-run result; scope: `cargo test -p pnp-cli --test support_preview_tdd`; return: `FACT` pass/fail + failing names
- Context cost: `M`
- Authoritative docs:
...
- OrcaSlicer refs:
  - none
- Verification:
  - `mkdir -p target && cargo test -p pnp-cli --test support_preview_tdd 2>&1 | tee target/test-output.log | grep -E "^test result|FAILED"` — FACT pass/fail
