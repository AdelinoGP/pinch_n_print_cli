...
| `crates/pnp-cli/src/main.rs` (or its slice subcommand module) | **EDIT** | At the top of the slice subcommand body, call `let mesh = Arc::new(slicer_model_io::load_model(&args.model)?);` and set `SliceRunOptions { mesh, .. }` (replacing the prior `SliceRunOptions { model_path: args.model, .. }` field). |
| `crates/slicer-runtime/src/cli.rs` | **EDIT** | In `SliceRunOptions` (definition at line 251), replace the `pub model_path: PathBuf` field with `pub mesh: Arc<MeshIR>`. All other fields are unchanged. Update any constructor / builder that populated `model_path` to take a `mesh` instead. |
...
| `crates/pnp-cli/src/main.rs` (and any submodules under `crates/pnp-cli/src/`) | Identify the slice subcommand body to know where to insert `load_model`. | Search for `fn slice` or `Commands::Slice` match arm. |
| `crates/slicer-runtime/src/cli.rs` | Confirm the current `SliceRunOptions` struct layout (definition at line 251). | Lines 249–271 (the struct + its doc comments). |
