...
- `crates/slicer-core/src/perimeter_utils.rs`: `expolygon_to_path3d` gains a previous-layer-boundary parameter and stamps `overhang_distance_mm` beside the quartile it already stamps; a new `signed_distance_to_boundary(x, y, &[ExPolygon]) -> f32` helper returning a raw signed f32 distance in mm; the stamping sites map "no boundary" to `None`.
