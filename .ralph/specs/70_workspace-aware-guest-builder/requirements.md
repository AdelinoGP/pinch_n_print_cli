...
- Add a new `xtask/` workspace member (workspace-relative, conventional `xtask` pattern). `xtask/Cargo.toml` declares `[package] name = "xtask"`, dev-mode workflow only. Dependencies: `cargo_metadata`, `serde`, `serde_json`, plus a thin CLI parser (clap or hand-rolled — no heavy dep needed since the surface is `build-guests` + `--check` + optional `--list`). Use `[workspace.metadata.scripts]` or root-level `.cargo/config.toml` `[alias]` so `cargo xtask` resolves to `cargo run -p xtask --`.
- Implement `cargo xtask build-guests`: discover guests via `cargo_metadata::MetadataCommand`, filter workspace members whose manifest path starts with `modules/core-modules/` and ends in `wit-guest/Cargo.toml` (core-module convention) OR starts with `test-guests/` and ends in `Cargo.toml` directly (test-guest convention). For each discovered guest: invoke `cargo build --target wasm32-unknown-unknown --release -p <name>`, then run `wasm-tools component new <core_wasm_path> -o <component_output_path>` where the output path follows the per-tree convention (`modules/core-modules/<dir>/<dir>.wasm` for core-modules, `test-guests/<crate-name>.component.wasm` for test-guests).
...
- The two bash scripts (~420 lines total) are the largest read of this packet. The implementer loads each in full (necessary to understand the freshness logic exactly) but extracts the rules into Rust code without copying bash idioms — the Rust implementation uses `std::fs::metadata` + `walkdir` (or `ignore::WalkBuilder`) for source discovery, and `cargo_metadata` for workspace introspection. After step 4 deletes the scripts, they should not be re-read.
