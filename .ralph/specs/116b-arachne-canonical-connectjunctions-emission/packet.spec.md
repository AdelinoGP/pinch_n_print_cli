---
status: draft
packet: 116b-arachne-canonical-connectjunctions-emission
task_ids:
  - none
...
# Packet Contract: 116b-arachne-canonical-connectjunctions-emission
...
- Depends on: `116a-arachne-beading-propagation-and-junction-bands` (A1 — needs
  A1's correct upward-half-edge junction fans for the `perimeter_index`
  pop-back merge to be implementable).
- Unblocks: `117-arachne-transition-ends-and-extra-ribs` (B — beading
  interpolation reads the canonical junction fans); `118-arachne-angle-fudge-and-noncentral-regions` (C — the π hack is load-bearing for A1's centrality-gated
...
- `docs/DEVIATION_LOG.md` `D-113C-FAITHFUL-GRAPH-CONSTRUCTION` entry — read
  ...
  `D-116A-JUNCTION-BANDS` addendum; A2 adds `D-116B-CONNECTJUNCTIONS-EMISSION`).
...
- `docs/DEVIATION_LOG.md` — new entry `D-116B-CONNECTJUNCTIONS-EMISSION`
  ...
  `D-116A-JUNCTION-BANDS` noting A2 supersedes A1's junction *metadata* layer
  (A1 owns the junction *geometry*; A2 owns the metadata + emission). Supersession
  pattern (new ID + addendum, no in-place edits).
  - `rg -q 'D-116B-CONNECTJUNCTIONS-EMISSION' docs/DEVIATION_LOG.md`
