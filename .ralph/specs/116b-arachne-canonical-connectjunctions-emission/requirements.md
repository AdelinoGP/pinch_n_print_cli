# Requirements: 116b-arachne-canonical-connectjunctions-emission
...
- Grouped task IDs: **none** (provenanced by the second-pass Arachne parity
  ...
  `docs/DEVIATION_LOG.md`'s `D-116A-JUNCTION-BANDS` entry, which A2 supersedes
...
Packet 116a (A1) fixed junction *geometry* — canonical `generateJunctions`
...
This packet supersedes `D-116A-JUNCTION-BANDS` for the junction-metadata +
...
- **Fixture re-baseline (this packet's own stage only)**:
  ...
  by `is_odd`).
- **Deviation-log entry**: `D-116B-CONNECTJUNCTIONS-EMISSION` (new ID, addendum
  on `D-116A-JUNCTION-BANDS`, supersession pattern — no in-place edits to
  A1's narrative).

## Out of Scope

- **N1 (junction geometry) and N7 (`BeadingPropagation`)** — owned by Packet A1
  (`116a`). A2 builds on A1's upward-half-edge junction fans.
- **N3 (transition ends) and N8 (`generateExtraRibs`)** — Packet B.
- **N5 (π hack) and N6 (`filterNoncentralRegions`)** — Packet C (`118`), strictly
...
- `docs/DEVIATION_LOG.md` `D-113C-FAITHFUL-GRAPH-CONSTRUCTION` and
  `D-116A-JUNCTION-BANDS` entries — read full; purpose: substrate + A1's
...
- Cross-packet impact: unblocks `117` (B — beading interpolation reads canonical
  junction fans), `118` (C — π hack removal strictly after A2).
...
| `rg -q 'D-116B-CONNECTJUNCTIONS-EMISSION' docs/DEVIATION_LOG.md` | Deviation log entry present | FACT pass/fail |
...
- **Deviation-log correction uses the supersession pattern** — new
  `D-116B-CONNECTJUNCTIONS-EMISSION` + addendum on `D-116A-JUNCTION-BANDS`. No
