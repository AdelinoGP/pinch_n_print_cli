...
- Does `core_module_ir_access_contract_tdd.rs` fully enumerate all 17 modules, or does it only cover a subset? If subset, confirm the full list.
- Are there any core modules that are host-built-in (not WASM) and therefore do not need manifest declarations?
- Should `path-optimization-default` declare read access to `PerimeterIR`, `InfillIR`, and `SupportIR` per the Stage I/O Contract?