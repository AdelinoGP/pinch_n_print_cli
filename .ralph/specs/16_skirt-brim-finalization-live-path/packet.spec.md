...
- **Given** a live host finalization dispatch using the canonical `skirt-brim` module, **when** the host merges finalization output into `LayerCollectionIR`, **then** the resulting finalized layers contain prepended `ExtrusionRole::Skirt` entities without calling the legacy `process()` path. | `cargo test -p slicer-host --test finalization_live_tdd live_finalization_dispatch_merges_skirt_brim_entity_pushes -- --exact --nocapture`
