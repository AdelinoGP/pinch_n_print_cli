# Requirements: 120-arachne-postprocess-order-and-remove-small-simplify
...
- **Deviation-log entry**: `D-120-POSTPROCESS-ORDER` (new ID, addendum on
...
- `docs/DEVIATION_LOG.md` `D-112-SIMPLIFY-DP` + `D-116B-CONNECTJUNCTIONS-EMISSION`
...
- Cross-packet impact: unblocks `121` (F — F's e2e closure gate depends on E's
...
| `rg -q 'D-120-POSTPROCESS-ORDER' docs/DEVIATION_LOG.md` | Deviation log entry present | FACT pass/fail |
...
- **Deviation-log correction uses the supersession pattern** — new
  `D-120-POSTPROCESS-ORDER` + addendum on `D-112-SIMPLIFY-DP`.
