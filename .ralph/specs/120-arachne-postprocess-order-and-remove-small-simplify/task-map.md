# Task Map: 120-arachne-postprocess-order-and-remove-small-simplify
...
| M2 — P116a/P116b/P117/P118/P119 (no TASK-###) | A1/A2/B/C/D | draft → implemented | E depends on D strictly (D's `is_odd` micro-loops interact with E's `removeSmallLines`). |
...
| `D-120-POSTPROCESS-ORDER` (NEW) | — | E creates this entry documenting the N11+N12+N13 fix. |
| `D-116B-CONNECTJUNCTIONS-EMISSION` | Closed (A2) | Untouched by E (E doesn't change emission; A2's `is_odd` fix is what makes E's `removeSmallLines` correct). |
