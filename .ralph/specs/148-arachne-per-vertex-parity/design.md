...
- **Exact functions, traits, manifests, tests, or fixtures expected to change:**
  ...
  - `modules/core-modules/arachne-perimeters/arachne-perimeters.toml`:
    ...
    - Add `[config.schema.wall_sequence]` (copy classic's at `classic-perimeters.toml:81`) — required by the AC-8 gate (`precise_outer_wall && wall_sequence==InnerOuter`); the key is absent from the arachne manifest today, so the gate cannot read it without this registration.
  - `crates/slicer-sdk/src/test_support/fixtures.rs`:
    - Add an `overhang_quartile_polygons(self, bands: Vec<QuartileBand>) -> Self` setter to `SliceRegionViewBuilder`, following the `bridge_areas` setter pattern at fixtures.rs:355. Verified gap: the builder exposes `bridge_areas` (:355) and `overhang_areas` (:387) but no quartile-band setter — AC-5's unit-test fixture is unbuildable without it. Test-support-only surface; no production code affected.
  - `modules/core-modules/arachne-perimeters/src/lib.rs`:
    ...
    - `run_perimeters` (236-352):
      ...
      - In the same loop, for each path point, look up the overhang quartile from `overhang_bands` (a `&[QuartileBand]` where each `QuartileBand` has `quartile: u8, polygons: &[ExPolygon]`). Filter the bands whose polygons contain the point and take the max quartile among matches (the shape `expolygon_to_path3d` actually implements). If no band contains the point, `overhang_quartile = None`. This mirrors the classic path's `expolygon_to_path3d` logic (`perimeter_utils.rs:316-331`). The per-vertex lookup operates in mm against `overhang_bands` (mm via `SliceRegionView`); the helper `point_in_polygon_winding` lives in `crates/slicer-ir/src/polygon_predicate.rs` and is wasm-compatible.
...
- `modules/core-modules/arachne-perimeters/arachne-perimeters.toml` — role: module manifest; expected change: three new `[config.schema.*]` sections (`precise_outer_wall`, `seam_candidate_angle_threshold_deg`, `wall_sequence`).
- `crates/slicer-sdk/src/test_support/fixtures.rs` — role: test-support fixtures; expected change: `SliceRegionViewBuilder` gains an `overhang_quartile_polygons(...)` setter (AC-5 fixture prerequisite).
...
- Aggregate (sum across all steps): M (9 steps × S/S/M/M/M/M/S/M/S; unit tests are written TDD-first inside Steps 2-6, not in a trailing batch).
- Largest single step: M (Step 4: TDD tests + feature_flags + overhang-band lookup in the construction loop; touches multiple IR fields and bridges). Steps 3, 5, 6, and 8 (old-test deletion/rewrite) are also M.
...
- `[FWD]` **ThinWall predicate in `classify_line`.** Current code maps ALL `is_odd` lines to `GapFill` (lib.rs:207-208). The candidate predicate for AC-2/AC-3 is `line.is_odd && line.inset_idx == 0 && print_thin_walls` → ThinWall (the widened center-line bead of a region thinner than one bead), with deeper odd lines staying GapFill. The implementer resolves this mid-flight via the Step 3 OrcaSlicer dispatch (WallToolPaths.cpp odd-toolpath classification) before writing the arm; if OrcaSlicer uses a different signal, follow OrcaSlicer and note the predicate in the test's doc comment.
