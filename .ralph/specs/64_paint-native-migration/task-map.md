...
| New task row (this packet) | All 11 steps | `docs/04_host_scheduler.md`, `docs/07_implementation_status.md` | 22 files (see `design.md` §Files in Scope) | M | This packet creates a new task row in `docs/07_implementation_status.md` with status `[x]` upon completion. The row references `64_paint-native-migration` and summarizes the consolidation. |

The `TASK-136` row in `docs/07` should have its notes updated to reflect that the code 504 warning path is now exercised by the always-on host annotator (not just in E2E progress-event tests). The new task row for this packet should appear after the latest entry in `docs/07`.
