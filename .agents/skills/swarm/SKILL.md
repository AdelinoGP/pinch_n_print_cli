...
- The packet has independent steps or disjoint file surfaces to justify sub-agents parallelism.
...
**3.1 Tooling.** Use your environment's sub-agent or task delegation mechanism to dispatch workers. Use research sub-agents for read-only research; use coding sub-agents for code or packet edits. Launch independent workers in parallel when their write surfaces are disjoint. Workers are synchronous and stateless: each invocation must receive all required packet context up front and returns one final report.
...
**4.2 Review.** After the implementation pass, dispatch a single review worker bound by `.agents/skills/spec-review/SKILL.md` with the same context-discipline reminders.
...
- The sub-agent / task delegation mechanism of your environment
