...
**Do not paraphrase.** The `1 unit = 100 nm` factor is the single most common source of OrcaSlicer-port bugs; the wording must match the canonical reference in the project instructions and `docs/08_coordinate_system.md` so a reader can grep across them.
